...
export const CACHE_TAGS = {
  ...
  seoDocument: (urlOrKey: string) => `seo-document:${urlOrKey}`,
  sitemap: "sitemap",
} as const;
...
export function revalidateLocales(localePrefixes?: string[]) {
  safeRevalidateTag(CACHE_TAGS.locales);
  revalidateAllLocaleLayoutPaths(localePrefixes);
}
...
export function revalidateUiMessages(languageCode: string) {
  ...
  safeRevalidateTag(CACHE_TAGS.translations);
  revalidateAllLocaleLayoutPaths();
}

/**
 * Invalidate App Router full-route cache for locale layout shells.
 * Prefer passing enabled url prefixes; falls back to common public prefixes.
 */
export function revalidateAllLocaleLayoutPaths(localePrefixes?: string[]): void {
  const prefixes =
    localePrefixes?.length && localePrefixes.length > 0
      ? localePrefixes
      : ["en", "ar"];
  for (const prefix of prefixes) {
    const trimmed = prefix.trim();
    if (!trimmed) continue;
    safeRevalidatePath(`/${trimmed}`, "layout");
  }
}
...
/** Invalidate the public Google Shopping product feed. */
export function revalidateGoogleShoppingFeed() {
  safeRevalidatePath("/feeds/google-shopping.xml");
}
...
export function revalidateSeoDocument(urlOrKey: string) {
  safeRevalidateTag(CACHE_TAGS.seoDocument(urlOrKey));
}

export function revalidateSitemap() {
  safeRevalidateTag(CACHE_TAGS.sitemap);
  safeRevalidatePath("/sitemap.xml");
  safeRevalidatePath("/sitemap_index.xml");
}
