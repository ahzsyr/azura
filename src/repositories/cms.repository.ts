...
export const cmsRepository = {
  ...
  getPageBySlug(slug: string, publishedOnly = false) {
    return prisma.cmsPage.findFirst({
      ...
      include: { seoMeta: true, author: true },
    });
  },
  ...
  getPageById(id: string) {
    return prisma.cmsPage.findUnique({
      where: { id },
      include: { seoMeta: true, author: true, revisions: { orderBy: { version: "desc" }, take: 15 } },
    });
  },
  ...
  listTags() {
    return prisma.postTag.findMany({ orderBy: { slug: "asc" } });
  },
