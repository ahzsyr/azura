...
export type SeoMetaWriteData = {
  ...
  jsonLd?: Prisma.InputJsonValue | typeof Prisma.DbNull | typeof Prisma.JsonNull;
...
import {
  ...
  SEO_APPEARANCE_NAMESPACE,

  SEO_INTEGRATIONS_NAMESPACE,

  SEO_SITEMAP_NAMESPACE,

  SEO_STRUCTURED_NAMESPACE,

  SEO_TRACKING_NAMESPACE,

} from "@/features/seo/constants";
import { normalizeStructuredConfig, prepareStructuredConfigForSave } from "@/features/seo/platform/schema-pipeline/structured-config";
...
export const seoRepository = {
  ...
  async resolveMeta(params: {
    ...
    if (params.pageKey) {
      ...
      const pageKeyMeta = await this.getByPageKey(params.pageKey);

      if (pageKeyMeta) return pageKeyMeta;
  ...
  async upsertMetaByCmsPage(cmsPageId: string, data: SeoMetaWriteData) {
    ...
    if (wiredPageKey) {
      const meta = await this.upsertMetaByPageKey(wiredPageKey, data);
  ...
  upsertRedirect(
    ...
    return prisma.seoRedirect.upsert({
      ...
      create: { fromPath, toPath, type, isActive },

      update: { toPath, type, isActive },
  ...
  async getAppearanceConfig(): Promise<SeoAppearanceConfig> {

    const row = await prisma.jsonStore.findUnique({

      where: {

        namespace_key: { namespace: SEO_APPEARANCE_NAMESPACE, key: "config" },

      },

    });

    return (row?.data as SeoAppearanceConfig) ?? {};

  },



  async upsertAppearanceConfig(config: SeoAppearanceConfig) {
    ...
    return prisma.jsonStore.upsert({
      ...
      create: {
        ...
        namespace: SEO_APPEARANCE_NAMESPACE,
  ...
  async getSitemapConfig(): Promise<SeoSitemapConfig> {

    const row = await prisma.jsonStore.findUnique({

      where: {

        namespace_key: { namespace: SEO_SITEMAP_NAMESPACE, key: "config" },

      },

    });

    return (row?.data as SeoSitemapConfig) ?? {};

  },



  async upsertSitemapConfig(config: SeoSitemapConfig) {
    ...
    return prisma.jsonStore.upsert({
      ...
      create: {
        ...
        namespace: SEO_SITEMAP_NAMESPACE,
  ...
  async getStructuredConfig(): Promise<SeoStructuredConfig> {
    ...
    return normalizeStructuredConfig((row?.data as SeoStructuredConfig) ?? {});
  ...
  async upsertStructuredConfig(config: SeoStructuredConfig) {
    ...
    const normalized = prepareStructuredConfigForSave(config);
    ...
    return prisma.jsonStore.upsert({
      ...
      update: { data: normalized as Prisma.InputJsonValue },
  ...
  async getTrackingConfig(): Promise<SeoTrackingConfig> {

    const row = await prisma.jsonStore.findUnique({

      where: {

        namespace_key: { namespace: SEO_TRACKING_NAMESPACE, key: "config" },

      },

    });

    return (row?.data as SeoTrackingConfig) ?? {};

  },



  async upsertTrackingConfig(config: SeoTrackingConfig) {
    ...
    return prisma.jsonStore.upsert({
      ...
      create: {
        ...
        namespace: SEO_TRACKING_NAMESPACE,
  ...
  async getSealedIntegrationsConfig(): Promise<SeoIntegrationsConfig> {
    ...
    return (row?.data as SeoIntegrationsConfig) ?? {};
  ...
  async getIntegrationsConfig(): Promise<SeoIntegrationsConfig> {
    ...
    return unsealIntegrationsConfig(await this.getSealedIntegrationsConfig());
  ...
  async listSearchMetricTotalsByUrl(params: {
    source: string;
    since: Date;
    until: Date;
    ...
    const rows = await seoSearchMetricDelegate().findMany({
      where: {
        ...
        date: { gte: params.since, lt: params.until },
      },
      take: 10_000,
    });
    const grouped = new Map<string, { url: string; clicks: number; impressions: number }>();
    for (const row of rows) {
      if (!row.url) continue;
      const current = grouped.get(row.url) ?? { url: row.url, clicks: 0, impressions: 0 };
      current.clicks += row.clicks;
      current.impressions += row.impressions;
