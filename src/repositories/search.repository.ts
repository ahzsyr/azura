...
import { SEARCH_PERF_LIMITS } from "@/capabilities/search/engine/performance/search-performance-limits";
import { toBooleanModeQuery, tokenize } from "@/capabilities/search/core/text";
...
const POSTGRES_SEARCH_VECTOR_SQL = Prisma.sql`
  ...
  if (params.types?.length) {
    return prisma.$queryRaw<FullTextRow[]>`
      SELECT id, "entityType", "entityId", locale, title, "urlPath", metadata,
        ...
        ts_rank(
          COALESCE("search_vector", ${POSTGRES_SEARCH_VECTOR_SQL}),
          websearch_to_tsquery('simple', ${params.tsQuery})
        ) AS relevance
      FROM "SearchDocument"
      WHERE locale = ${params.locale}
        AND "entityType" IN (${Prisma.join(params.types)})
        AND COALESCE("search_vector", ${POSTGRES_SEARCH_VECTOR_SQL})
          @@ websearch_to_tsquery('simple', ${params.tsQuery})
      ORDER BY relevance DESC
      LIMIT ${params.limit}
    `;
...
export const searchRepository = {
  ...
  async listDocuments(params: {
    ...
    return rows.map((r) => {
      const body = r.body.length > BODY_PREVIEW ? r.body.slice(0, BODY_PREVIEW) : r.body;
      return mapLeanRow({ ...r, body });
    });
  },
};
