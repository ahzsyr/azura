...
export const productRepository = {
  ...
  async listLocalizedSlugs(localeCode: string): Promise<string[]> {
    ...
    const products = await prisma.product.findMany({
      select: { id: true, canonicalSlug: true, status: true },
      orderBy: { canonicalSlug: "asc" },
    });
    const published = products.filter((product) => isProductPublishedForSearch(product.status));
    if (published.length === 0) return [];

    const publishedIds = published.map((product) => product.id);
    ...
    return published.map((r) => r.canonicalSlug);
  },
