...
export const searchEnterKeyActionSchema = z.enum(["search-page", "first-result"]);
export type SearchEnterKeyAction = z.infer<typeof searchEnterKeyActionSchema>;
...
export const SEARCH_ENTER_KEY_ACTION_OPTIONS: {
  value: SearchEnterKeyAction;
  label: string;
  description: string;
}[] = [
  {
    ...
    description: "Pressing Enter opens the full search results page.",
  },
  {
    value: "first-result",
    label: "Open first result",
    description: "Pressing Enter navigates directly to the first matching result.",
  },
];
...
export const searchGeneralSchema = z.object({
  ...
  enterKeyAction: z.preprocess(emptyToUndefined, searchEnterKeyActionSchema.default("search-page")),
