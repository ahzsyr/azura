...
import { migrateFiltersRaw } from "@/capabilities/search/settings/resolve-search-filters";
import { searchSemanticProviderSchema } from "@/capabilities/search/settings/search-smart.schema";
...
export function resolveAdminSearchSettings(site: Record<string, unknown>): AdminSearchSettings {
  ...
  const enterKeyAction =
    generalRaw.enterKeyAction === "search-page" || generalRaw.enterKeyAction === "first-result"
      ? generalRaw.enterKeyAction
      : s.enterKeyAction === "search-page" || s.enterKeyAction === "first-result"
        ? s.enterKeyAction
        : undefined;
      ...
      modalElements: (() => {
        const appearance =
          s.appearance && typeof s.appearance === "object" && !Array.isArray(s.appearance)
            ? (s.appearance as Record<string, unknown>)
            : {};
        const raw = appearance.modalElements;
        return raw && typeof raw === "object" && !Array.isArray(raw) ? raw : {};
      })(),
    },
    smart: (() => {
