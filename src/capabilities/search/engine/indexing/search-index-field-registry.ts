...
import {
  ...
  fieldWeight,
  isFieldActive,
} from "@/capabilities/search/engine/indexing/search-index-profile";
import {
  extractFacetValues,
  extractSearchableAttributeText,
  type SearchableFieldDefinition,
} from "@/capabilities/search/engine/schema/search-field-schema";
import {
  flattenJsonToText,
  normalizeTags,
  stringifyIndexValue,
} from "@/capabilities/search/engine/indexing/search-text-utils";
import type {
...
import { isCustomFieldKey } from "@/capabilities/search/engine/indexing/search-index-field-keys";
...
function extractSeoText(seo: SearchIndexSeoSnapshot | null | undefined, urlPrefix: string): string {
  if (!seo) return "";
  const title = getLocalizedField({ title: seo.title ?? {} }, "title", urlPrefix);
  const description = getLocalizedField(
    { description: seo.description ?? {} },
    "description",
    urlPrefix
  );
  const og = getLocalizedField({ title: seo.ogTitle ?? {} }, "title", urlPrefix);
