import type { SearchAnalyticsEvent } from "@/capabilities/search/engine/types";
...
function persistenceSink(event: SearchAnalyticsEvent): void {
  ...
  void import("@/capabilities/search/analytics/search-analytics-store.service").then((store) => {
    if (event.type === "query") {
...
if (typeof process !== "undefined") {
  searchAnalytics.addSink((event) => {
    ...
    if (runtimeConfig.persistEvents) return;
    void import("@/capabilities/search/analytics/search-analytics-store.service").then((m) =>
