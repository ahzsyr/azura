...
} from "@/capabilities/search/engine/providers/catalog-providers";
import type { SearchIndexer } from "@/capabilities/search/engine/indexer/search-indexer";
...
export async function syncCatalogSearchIndexes(
  ...
  for (const { urlPrefix, code } of resolved.indexerLocales) {
    ...
    if (resolved.siteCatalog.products && !isEntityReadContentEnabled()) {
  ...
  for (const { urlPrefix } of resolved.indexerLocales) {
    ...
    if (resolved.siteCatalog.products) {
      ...
      for (const record of records) {
        if (!catalogProductSearchProvider.shouldIndex(record)) continue;
        valid.add(
          catalogDocKey(
            "CATALOG_PRODUCT",
...
export async function reconcileCatalogSearchIndexes(
  ...
  if (resolved.siteCatalog.products && !isEntityReadContentEnabled()) {
    catalogTypes.push("CATALOG_PRODUCT");
  }
...
export async function upsertCatalogProductRecord(
  ...
  if (!catalogProductSearchProvider.shouldIndex(source)) {
    await removeCatalogProduct(indexer, urlPrefix, record.slug);
    return;
  }
...
async function syncCatalogProducts(
  ...
  if (records.length > 0) {
    ...
    for (const record of records) {
      ...
      if (!catalogProductSearchProvider.shouldIndex(source)) continue;
...
async function syncCatalogCategories(
  ...
  const records = await loadListingRecords(urlPrefix);
  // Full taxonomy (incl. hidden) so Category facet sync drops orphans / hidden nodes
  const collections = orderCollectionsHierarchy(
    await collectionsDataService.loadAll({ localePrefix: urlPrefix }),
  );
