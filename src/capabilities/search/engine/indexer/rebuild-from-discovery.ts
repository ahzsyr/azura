...
import type { ContentItemDbShape } from "@/capabilities/search/engine/indexing/content-item-index-pipeline";
...
export async function rebuildFromDiscovery(indexer: SearchIndexer): Promise<RebuildResult> {
  ...
  const [contentItems, faqItems, testimonials, pages, posts, media, teamMembers, partners] =
    await Promise.all([
    ...
    prisma.teamMember.findMany({
      where: {
        isPublished: true,
        directory: { isPublished: true },
      },
      include: {
        directory: { select: { id: true, slug: true } },
      },
    }),
    ...
    await runWithConcurrency(
      ...
      async (member) => {
        try {
          ...
          captureError(e, `team member ${member.id}`);
        }
      },
      perf.indexConcurrency
    );
    ...
    await runWithConcurrency(
      ...
      async (partner) => {
        try {
          ...
          captureError(e, `partner ${partner.id}`);
        }
      },
      perf.indexConcurrency
    );
