...
import { pickCollectionCardImage } from "@/features/collections/collection-navigation";
import { collectRuleLeaves, upgradeLegacyRuleSet } from "@/features/categories/matching";
...
export const catalogCollectionSearchProvider = defineSearchProvider<CatalogCollectionIndexSource>({
  ...
  buildRecords(col, ctx) {
    ...
    const card: SearchCardPayload = {
      slug: col.slug,
      imageUrl: pickCollectionCardImage({
    ...
    return [
      {
        ...
        body: [
          col.description,
          col.slug,
          ...collectRuleLeaves(upgradeLegacyRuleSet(col.conditions)).flatMap((r) => [
