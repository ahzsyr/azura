...
/** Locale-prefixed public path, falling back to unprefixed default locale. */
export function localizedPublicPath(
  localePrefix: string,
  path: string | null,
  fallbackSegment = "content",
  defaultPrefix: string = getFallbackDefaultLocalePrefix(),
): string {
  const cleaned = path?.trim() || `/${fallbackSegment}`;
  const withSlash = cleaned.startsWith("/") ? cleaned : `/${cleaned}`;
  return publicLocalePath(localePrefix, withSlash === "/" ? "/" : withSlash, defaultPrefix);
}
