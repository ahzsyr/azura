...
import {
  SEARCH_MODE_OPTIONS,
  SEARCH_ENTER_KEY_ACTION_OPTIONS,
        ...
        <div className="space-y-8">
          ...
          <SettingsSection title="Query behavior" description="Limits and timing for search requests.">
            ...
            <SelectField
              ...
              description='Choose what Enter does in search bar and search modal.'
              value={s.general.enterKeyAction}
              onChange={(v) =>
                patchNested(
                  "general",
                  "enterKeyAction",
                  v as AdminSearchSettings["general"]["enterKeyAction"]
                )
              }
              options={SEARCH_ENTER_KEY_ACTION_OPTIONS.map((option) => ({
                value: option.value,
                label: option.label,
              }))}
            />
