...
export function resolveProductCardFields(
  ...
  const ctaEnabled = isActionTypeEnabled(design, "cta");
  const linkCtx = productLinkContextFromProduct({
    productTitle: product.name,
    name: product.name,
    slug: product.slug,
    mpn: product.buy_now_slug,
  });
  const buyNowHref =
    buyNowEnabled && cardDisplay.showBuyNow
      ? buildBuyNowHref(buyNow, product.slug, product.buy_now_slug, linkCtx)
      : null;
  const showProductCta = ctaEnabled && cardDisplay.showProductCta;
  const showBuyNow = Boolean(buyNowHref);
  const ctaLayout = productCta.cardLayout ?? "floating_corner";
