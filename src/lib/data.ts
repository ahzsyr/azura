...
export async function getDashboardStats() {
  return safe(
    async () => {
      const [packages, formSubmissions, testimonials, gallery] = await Promise.all([
      ...
      return { packages, newFormSubmissions: formSubmissions, testimonials, gallery };
    },
    { packages: 0, newFormSubmissions: 0, testimonials: 0, gallery: 0 }
  );
}
