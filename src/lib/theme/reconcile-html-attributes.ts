...
const APPEARANCE_DATA_ATTRS = new Set(["data-theme", "data-theme-mode"]);
...
export function reconcileSiteHtmlAttributes(
  ...
  for (const [key, value] of Object.entries(ssrAttributes)) {
    ...
    // Boot / ThemeEngine own appearance — never overwrite with SSR defaults.
    if (APPEARANCE_DATA_ATTRS.has(key)) continue;
    const datasetKey = key
      .slice(5)
      .replace(/-([a-z])/g, (_, c: string) => c.toUpperCase());
    if (html.dataset[datasetKey] === value) continue;
