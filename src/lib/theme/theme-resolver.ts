...
import { resolveThemeFonts } from "@/lib/theme/font-registry";
import {
  themeConfigToTokens,
  tokensToThemeConfig,
  type ThemeConfig,
} from "./migration-adapters";
import { mergeThemeSources, type ThemeSourceInput } from "./theme-source-merge";

export type { ThemeConfig } from "./migration-adapters";
export { themeConfigToTokens, tokensToThemeConfig } from "./migration-adapters";
export type { ThemeSourceInput } from "./theme-source-merge";
export { mergeThemeSources } from "./theme-source-merge";
...
export function generateHtmlAttributes(input: HtmlAttributesInput): Record<string, string> {
  ...
  const presetId = presetVisual?.presetId ?? config.presets.siteDefaultPresetId;
  ...
  const fonts = resolveThemeFonts(config.typography);
  if (fonts.classNames) {
...
export function buildResolvedThemeSync(
  ...
  return {
    ...
    borderStyle,
    preset: {
      presetId: presetVisual?.presetId ?? config.presets.siteDefaultPresetId,
