...
export function mergeThemeSources(sources: ThemeSourceInput): ThemeConfig {
  ...
  const provenance = baseTokens.themeProvenance;
  const shouldApplySitePreset =
    Boolean(sources.preset) &&
    !(
      provenance?.customizedAfterPreset &&
      provenance?.presetLockMode !== "baseline-reset"
    );
