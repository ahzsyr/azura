...
export function detectDeviceCapabilities(): DeviceCapabilities {
  ...
  // Phones often report concurrency=4 / omit deviceMemory — do not treat that as low-end alone.
  const lowMemory = deviceMemoryGb != null && deviceMemoryGb <= 2;
...
export function buildCapabilityPolicy(capabilities: DeviceCapabilities): CapabilityPolicy {
  ...
  return {
    // Custom cursors stay desktop-only; backgrounds use allowAnimatedBackground + medium.
    allowHeavy: !prefersReducedMotion && !lowEndDevice && !mobileLike,
    // Medium must stay available on phones — smallScreen alone used to kill all medium effects.
    allowMedium: !prefersReducedMotion && !lowEndDevice,
