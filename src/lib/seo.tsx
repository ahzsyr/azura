...
import { isLocalhostHost } from "@/features/seo/site-url-utils";
import type { Locale } from "@/i18n/routing";
import type { PublicLocale } from "@/i18n/locale-config";
import { buildHreflangAlternates, buildCanonicalUrl, normalizeStoredCanonicalUrl } from "@/i18n/seo-helpers";
...
function resolveMetadataSiteUrl(origin?: string): string {
  const candidates = [
    origin?.trim(),
  ...
  return (candidates[0] ?? getSiteUrl()).replace(/\/$/, "");
}

/**
 * @deprecated Use `seoService.resolveMetadata()` → `documentToMetadata()` for public routes.
 * Retained for JsonLd helpers and legacy contributor tests only.
 */
export function buildMetadata({
  ...
  const siteUrl = resolveMetadataSiteUrl(origin);
  const defaultPrefix =
    enabledLocales?.find((l) => l.isDefault)?.urlPrefix ?? getFallbackDefaultLocalePrefix();
  const defaultUrl = buildCanonicalUrl(
    siteUrl,
    locale,
    path,
    slugByLocale?.[locale],
  ...
  const rawCanonical =
    normalizeStoredCanonicalUrl(canonicalUrl, siteUrl, defaultPrefix) ?? defaultUrl;
  ...
  const displayTitle = composeDocumentTitle(trimmedTitle, resolvedSiteName);
  const trimmedOgTitle = ogTitle?.trim();
  const socialTitle = trimmedOgTitle
    ? composeDocumentTitle(trimmedOgTitle, resolvedSiteName)
    : displayTitle;
  ...
  const languages =
    enabledLocales && enabledLocales.length > 0
      ...
      : {
          en:
            sanitizeMetadataAbsoluteUrl(
              buildCanonicalUrl(siteUrl, locale, path, slugByLocale?.[locale], defaultPrefix),
              siteUrl,
            ) ??
            buildCanonicalUrl(siteUrl, locale, path, slugByLocale?.[locale], defaultPrefix),
        };
