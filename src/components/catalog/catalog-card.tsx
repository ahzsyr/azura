...
export function CatalogCard({
  ...
  const name = getLocalizedField(item, "name", locale) || getLocalizedField(item, "title", locale);
  const excerpt = getLocalizedField(item, "excerpt", locale) || getLocalizedField(item, "description", locale);
  const image = item.images[0]?.url ?? item.images[0]?.url ?? DEFAULT_MEDIA_PLACEHOLDER;
        ...
        <div className={cn("relative overflow-hidden bg-muted", variant === "compact" ? "aspect-[16/9]" : "aspect-[4/3]")}>
          ...
          {item.source === "packages" && settings.showCategory && item.category && (
            <Badge className="absolute start-4 top-4 bg-accent text-accent-foreground">
              {getLocalizedField(item.category, "name", locale)}
            </Badge>
          )}
