...
import { isShellVisible, whenShellReady } from "@/lib/motion/shell-ready";
...
export function SiteBackgroundLayer({ tokens: tokensProp, immediate = false }: Props) {
  ...
  const mountBackground = useCallback(
    (options?: { force?: boolean }) => {
      ...
      // Never restore appearance colors here — ThemeEngineProvider owns surfaces.
  ...
  useEffect(() => {
    const onThemeChange = (event: Event) => {
      ...
      // Appearance already applied by engine — remounting flashes the page.
      if (detail?.appearanceOnly) return;
      // Capability/policy change or preset: remount canvas only (no color restore).
      mountBackground({ force: true });
    };
