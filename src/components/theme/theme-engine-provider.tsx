...
export function ThemeEngineProvider({
  ...
  useEffect(() => {
    ...
    // Defer meta update slightly so CSS vars are applied before we read them.
    const raf = requestAnimationFrame(() => syncThemeColorMeta(resolved));
  ...
  const setAppearanceMode = useCallback(
    (mode: AppearanceMode, options?: { animate?: boolean }) => {
      ...
      requestAnimationFrame(() => syncThemeColorMeta(resolved));
    },
