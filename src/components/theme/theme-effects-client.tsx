...
/**
 * Binds cursor/text/background visual effects only.
 * Never restores appearance CSS vars — ThemeEngineProvider owns those.
 */
export function ThemeEffectsClient({
  ...
  const applyResolved = useCallback(
    (options?: { immediate?: boolean; colorsOnly?: boolean; force?: boolean }) => {
      if (!resolveEffectAppearance(resolvedTheme) && !effectAppearance) return;
      const experience = resolveExperience();
      if (!experience) return;
      // Effect binding only — never restorePresetColorsFromStorage.
  ...
  useEffect(() => {
    ...
    const runApply = () => {
      // Always full-bind on mount. colorsOnly is for appearance refreshes only —
      // using it here skipped tagHeroHeadings / initTextEffects when the bg canvas
      // was already ready, and the coordinator then blocked later full applies.
      applyResolved({ immediate: true });
    };
    ...
    if (isShellVisible()) {
      ...
      stopShell = whenShellReady(runApply);
    }
  ...
  useEffect(() => {
    const onThemeChange = (event: Event) => {
      ...
      // Appearance already applied by engine — skip rebind flash on toggle.
      if (detail?.appearanceOnly) return;
      // capabilitiesOnly / preset: rebind effects without restoring colors.
      applyResolved({ immediate: true });
    };
  ...
  useEffect(() => {
    if (!applyOnMount || !baseResolved || !tokens || !effectAppearance) return;

    const onRouteContentReady = () => {
      applyResolved({ immediate: true });
    };

    document.addEventListener(ROUTE_CONTENT_READY_EVENT, onRouteContentReady);
    return () => document.removeEventListener(ROUTE_CONTENT_READY_EVENT, onRouteContentReady);
  }, [applyOnMount, applyResolved, baseResolved, tokens, effectAppearance]);
