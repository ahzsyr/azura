"use client";

import { usePathname } from "@/i18n/navigation";
import { PageLoadingSkeleton } from "@/components/layout/page-loading-skeleton";
import {
  firstRouteSkeleton,
  isBuildShell,
  isRouteSkeleton,
} from "@/lib/navigation/is-route-skeleton";
import { containsPartialRouteContent } from "@/lib/navigation/is-partial-route-content";
import { emitRouteContentReady } from "@/lib/motion/shell-ready";
import { recordNavigationEnd } from "@/lib/performance/runtime-metrics";
import { clearSharedElementHandoff } from "@/lib/navigation/shared-elements";
import { PUBLIC_MOTION } from "@/lib/motion/public-motion";
import { postDebugSessionLog } from "@/lib/debug-session-log.client";
import {
  isValidElement,
  useLayoutEffect,
  useRef,
  useState,
  useEffect,
  type ReactNode,
} from "react";

type Props = {
  children: ReactNode;
};

function isRealContent(node: ReactNode): boolean {
  if (node == null || typeof node === "boolean") return false;
  if (isRouteSkeleton(node)) return false;
  if (isBuildShell(node)) return false;
  if (containsPartialRouteContent(node)) return false;
  if (Array.isArray(node)) return node.some((child) => isRealContent(child));
  return isValidElement(node);
}

function prefersReducedMotion(): boolean {
  if (typeof window === "undefined") return false;
  return window.matchMedia("(prefers-reduced-motion: reduce)").matches;
}

function firstLoadFallback(children: ReactNode): ReactNode {
  if (typeof document !== "undefined") {
    if (document.documentElement.classList.contains("site-preloading")) {
      return null;
    }
  }
  const skeleton = firstRouteSkeleton(children);
  if (skeleton) return skeleton;
  return <PageLoadingSkeleton variant="home" />;
}

function isShellPreloading(): boolean {
  if (typeof document === "undefined") return false;
  return document.documentElement.classList.contains("site-preloading");
}

/**
 * Stale-page hold during navigation — keeps outgoing page visible until real RSC
 * content arrives, then crossfades via CSS (not document.startViewTransition).
 */
export function MarketingPageTransition({ children }: Props) {
  const pathname = usePathname();
  const committedRef = useRef<ReactNode>(null);
  const committedPathRef = useRef(pathname);
  const hasCommittedRef = useRef(false);
  const prevPathnameRef = useRef(pathname);
  const previousRenderSnapshotRef = useRef<string>("");
  const renderLogBudgetRef = useRef(4);
  const enterTimeoutRef = useRef<number | null>(null);
  const [layerState, setLayerState] = useState<"idle" | "stale" | "entering">("idle");

  const skeletonActive = isRouteSkeleton(children);
  const realContent = isRealContent(children);
  const pending = skeletonActive || !realContent;
  const isNavigating =
    hasCommittedRef.current && committedPathRef.current !== pathname;
  const showStaleHold =
    isNavigating && pending && hasCommittedRef.current && committedRef.current != null;

  const clearEnterTimeout = () => {
    if (enterTimeoutRef.current == null) return;
    window.clearTimeout(enterTimeoutRef.current);
    enterTimeoutRef.current = null;
  };

  useEffect(() => {
    const snapshot = JSON.stringify({
      pathname,
      pending,
      realContent,
      skeletonActive,
      hasCommitted: hasCommittedRef.current,
      committedPath: committedPathRef.current,
      isNavigating,
      showStaleHold,
      layerState,
    });

    if (previousRenderSnapshotRef.current === snapshot) return;
    previousRenderSnapshotRef.current = snapshot;

    if (renderLogBudgetRef.current > 0) {
      renderLogBudgetRef.current -= 1;
      // #region agent log H1
      postDebugSessionLog({
        runId: "post-fix",
        hypothesisId: "H1",
        location: "marketing-page-transition.tsx:render",
        message: "Marketing transition snapshot changed",
        data: {
          pathname,
          pending,
          realContent,
          skeletonActive,
          hasCommitted: hasCommittedRef.current,
          committedPath: committedPathRef.current,
          isNavigating,
          showStaleHold,
          layerState,
          renderLogsRemaining: renderLogBudgetRef.current,
        },
      });
      // #endregion
    }
  }, [isNavigating, layerState, pathname, pending, realContent, showStaleHold, skeletonActive]);

  useLayoutEffect(() => {
    if (typeof window === "undefined") return;
    if (window.location.hash) return;
    window.scrollTo({ top: 0, behavior: prefersReducedMotion() ? "auto" : "auto" });
  }, [pathname]);

  useLayoutEffect(() => {
    if (prevPathnameRef.current === pathname) return;

    // #region agent log H4
    postDebugSessionLog({
      runId: "post-fix",
      hypothesisId: "H4",
      location: "marketing-page-transition.tsx:path-change",
      message: "Pathname changed inside transition effect",
      data: {
        from: prevPathnameRef.current,
        to: pathname,
        pending,
        hasCommitted: hasCommittedRef.current,
        committedPath: committedPathRef.current,
      },
    });
    // #endregion

    prevPathnameRef.current = pathname;
    clearEnterTimeout();

    if (!hasCommittedRef.current) return;
    setLayerState("stale");
  }, [pathname, pending]);

  useLayoutEffect(() => {
    if (pending) return;

    if (committedRef.current === children && committedPathRef.current === pathname) {
      setLayerState("idle");
      return;
    }

    const commit = () => {
      const isFirstCommit = !hasCommittedRef.current;
      // #region agent log H1
      postDebugSessionLog({
        runId: "post-fix",
        hypothesisId: "H1",
        location: "marketing-page-transition.tsx:commit-start",
        message: "Committing route content",
        data: {
          isFirstCommit,
          pathname,
          pending,
          previousCommittedPath: committedPathRef.current,
          layerState,
          showStaleHold,
          routeNavigatingClass: document.documentElement.classList.contains("route-navigating"),
          sitePreloadingClass: document.documentElement.classList.contains("site-preloading"),
        },
      });
      // #endregion

      committedRef.current = children;
      committedPathRef.current = pathname;
      hasCommittedRef.current = true;
      if (isFirstCommit) {
        setLayerState("idle");
      } else {
        setLayerState("entering");
        clearEnterTimeout();
        enterTimeoutRef.current = window.setTimeout(() => {
          // #region agent log H3
          postDebugSessionLog({
            runId: "post-fix",
            hypothesisId: "H3",
            location: "marketing-page-transition.tsx:enter-timeout",
            message: "Enter timeout fired",
            data: {
              pathnameAtTimeout: pathname,
              committedPath: committedPathRef.current,
              routeNavigatingClass: document.documentElement.classList.contains("route-navigating"),
              sitePreloadingClass: document.documentElement.classList.contains("site-preloading"),
            },
          });
          // #endregion

          enterTimeoutRef.current = null;
          setLayerState("idle");
        }, PUBLIC_MOTION.routeEnterClearMs);
      }
      emitRouteContentReady();
      recordNavigationEnd(pathname, { success: true });
    };

    // Route commits must stay outside document.startViewTransition — wrapping React
    // DOM updates there races with reconciliation and causes insertBefore NotFoundError.
    commit();
    clearSharedElementHandoff();
  }, [children, pending, pathname]);

  useEffect(() => () => clearEnterTimeout(), []);

  let activeContent: ReactNode;

  if (!pending) {
    activeContent = children;
  } else if (hasCommittedRef.current && committedRef.current != null) {
    activeContent = committedRef.current;
  } else if (isShellPreloading()) {
    activeContent = children;
  } else {
    activeContent = firstLoadFallback(children);
  }

  const activeLayerClass =
    layerState === "entering"
      ? "route-page-layer route-page-layer--active"
      : "route-page-layer route-page-layer--idle";

  return (
    <div className="marketing-page-content">
      {showStaleHold ? (
        <div
          key={`stale-${committedPathRef.current}`}
          className="route-page-layer route-page-layer--stale"
          aria-busy
          aria-label="Loading page content"
        >
          {committedRef.current}
        </div>
      ) : (
        <div
          key={pathname}
          className={activeLayerClass}
          aria-busy={pending && !hasCommittedRef.current ? true : undefined}
          aria-label={
            pending && !hasCommittedRef.current ? "Loading page content" : undefined
          }
          data-route-content-ready={!pending ? "true" : undefined}
        >
          {activeContent}
        </div>
      )}
    </div>
  );
}
