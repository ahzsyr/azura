...
export function SitePreloader({ settings }: Props) {
  ...
  useEffect(() => {
    ...
    if (preloaderShowsOnInitialLoad(settings.mode)) {
      ...
      /**
       * Keep the SSR boot overlay (already tinted with --primary).
       * Mounting a second client PreloaderView flashes the default logo color.
       */
      showStartRef.current = performance.now();
      setShellPreloading(true);
      if (maxTimerRef.current) clearTimeout(maxTimerRef.current);
    ...
    removeBootPreloader();
    document.dispatchEvent(new CustomEvent(SHELL_READY_EVENT));
  }, [mounted, scheduleDismiss, settings.enabled, settings.mode, settings.maxDurationMs, setShellPreloading, hidePreloader]);
  ...
  if (!settings.enabled || !mounted || !visible) return null;

  return createPortal(
    <PreloaderView settings={settings} hidden={!visible} logoUrl={settings.resolvedLogoUrl} />,
    document.body,
  );
}
