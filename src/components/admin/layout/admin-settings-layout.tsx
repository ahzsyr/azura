          ...
          return (
            <div key={tab.id} className="flex flex-col gap-0.5">
              ...
              {isActive && children.length > 0 ? (
                <div
                  className="ml-2 mb-0.5 flex flex-col gap-0.5 border-l border-border/50 pl-1.5"
                  role="group"
                  aria-label={`${tab.label} sections`}
                >
                  {children.map((child) => {
                    const childActive = activeSubTab === child.id;
                    return (
                      <button
                        key={child.id}
                        type="button"
                        className={cn(
                          "w-full rounded-md px-2.5 py-1.5 text-left text-xs font-medium transition-colors duration-150",
                          childActive
                            ? "bg-primary/10 text-primary"
                            : "text-muted-foreground hover:bg-muted/50 hover:text-foreground",
                        )}
                        aria-current={childActive ? "page" : undefined}
                        onClick={() => onSubTabChange?.(tab.id, child.id)}
                      >
                        {child.label}
                      </button>
                    );
...
type AdminSettingsLayoutProps = {
  ...
  /** Active nested sidebar child (section) id when using sidebar layout. */
  activeSubTab?: string;
  /** Nested sidebar child click — (parentTabId, childId). */
  onSubTabChange?: (tabId: string, subTabId: string) => void;
