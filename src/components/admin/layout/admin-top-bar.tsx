...
import { AdminSearchCommand } from "@/capabilities/search/components/search-command";
...
import { AdminActionDevIndicator } from "./admin-action-dev-indicator";
...
function SaveStatusIndicator() {
  ...
  const labels = {
    saved:
      pageActions.onPublish && publishStatus === "live" ? "Live" : "Saved",
    unsaved: "Unsaved changes",
    saving: pageActions.onPublish && publishStatus === "publishing" ? "Publishing…" : "Saving…",
    error: publishStatus === "error" ? "Publish failed" : "Save failed",
  };

  const colors = {
    saved:
      pageActions.onPublish && publishStatus === "live"
...
export function AdminTopBar() {
  ...
  const handlePublish = useCallback(async () => {
    if (!pageActions.onPublish || saveStatus === "saving" || publishStatus === "publishing") return;
    setPublishStatus("publishing");
    try {
      await pageActions.onPublish();
      markPublished();
      setSaveStatus("saved");
    } catch (e) {
      if (isRedirectError(e)) throw e;
