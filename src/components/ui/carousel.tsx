...
function readElementDirection(el: Element | null): "ltr" | "rtl" {
  // Prefer an ancestor so our own `dir` prop does not mask page/preview direction.
  const fromAncestor = el?.parentElement?.closest("[dir]")?.getAttribute("dir");
  const value =
    fromAncestor ??
...
/** Resolve Embla `direction` from opts, nearest `[dir]`, or `<html dir>`. */
function useCarouselDirection(
  rootRef: React.RefObject<HTMLElement | null>,
  explicit?: "ltr" | "rtl",
): "ltr" | "rtl" {
  const [direction, setDirection] = React.useState<"ltr" | "rtl">(
    () => explicit ?? readElementDirection(null),
  );
  ...
  React.useLayoutEffect(() => {
    ...
    const sync = () => setDirection(readElementDirection(rootRef.current));
    sync();
...
function Carousel({
  ...
  const direction = useCarouselDirection(rootRef, opts?.direction);
  ...
  return (
    <CarouselViewportContext.Provider value={viewportContextValue}>
      <div ref={rootRef} className={cn("relative", className)} {...props} dir={direction}>
        {children}
      </div>
