...
import {
  ProductCardMediaFlip,
  isMediaFlipStyle,
} from "@/features/products/card-design/components/product-card-media-flip";
...
export function ProductCardTemplate({ viewModel, cardStyle }: Props) {
  ...
  const useMediaFlip =
    isMediaFlipStyle(ctx.design.style) && theme?.listingViewMode !== "list";
  ...
  const card = (
    <ProductCardShell ctx={ctx} cardStyle={cardStyle}>
      ...
      {useMediaFlip ? <ProductCardMediaFlip ctx={ctx} /> : <ProductCardMedia ctx={ctx} />}
      <ProductCardContent ctx={ctx} suppressFlipSlots={useMediaFlip} />
      <ProductCardActionBar ctx={ctx} />
    </ProductCardShell>
  );

  if (!useMediaFlip) return card;

  return <TooltipProvider delayDuration={250}>{card}</TooltipProvider>;
}
