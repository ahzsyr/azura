...
/** Refresh access token if expired or within this window of expiry. */
const TOKEN_REFRESH_SKEW_MS = 5 * 60 * 1000;

export async function getUnsealedAccessToken(connectionId: string) {
  const credential = await prisma.marketingCredential.findUnique({
    where: { connectionId },
    include: { connection: true },
  });
  if (!credential) return undefined;

  const accessToken = unsealSecret(credential.accessToken);
  ...
  const refreshToken = unsealSecret(credential.refreshToken);
  if (!refreshToken) {
  ...
  try {
    const providerId = credential.connection.providerId;
    const { getProviderAppCredentials } = await import(
    ...
    if (providerId === "google-ads") {
      ...
      token = await refreshGoogleAdsAccessToken({
        refreshToken,
        clientId: creds.clientId,
        clientSecret: creds.clientSecret,
      });
    } else {
      // Generic OAuth refresh for other providers that use Google-style token endpoint
    ...
    const nextExpires = token.expires_in
      ? new Date(Date.now() + token.expires_in * 1000)
      : null;

    await prisma.marketingCredential.update({
      where: { connectionId },
      data: {
        accessToken: sealSecret(token.access_token) ?? "",
        ...(token.refresh_token
          ? { refreshToken: sealSecret(token.refresh_token) ?? null }
          : {}),
    ...
    await prisma.marketingConnection.update({
      where: { id: connectionId },
      data: { status: "connected", lastHealthAt: new Date() },
    });

    return token.access_token;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await prisma.marketingCredential
      .update({
        where: { connectionId },
        data: { refreshStatus: "failed" },
      })
      .catch(() => undefined);
    await prisma.marketingConnection.update({
      where: { id: connectionId },
    ...
    console.error(`[marketing] token refresh failed for ${connectionId}:`, message);
    return undefined;
  }
}

export async function refreshConnectionToken(connectionId: string) {
  // Force near-expiry path by clearing effective validity window via getUnsealedAccessToken
  // after temporarily treating token as expired when refresh is requested explicitly.
  const credential = await prisma.marketingCredential.findUnique({ where: { connectionId } });
  if (!credential) return { ok: false as const, message: "No credential" };
  if (credential.expiresAt && credential.expiresAt.getTime() > Date.now() + TOKEN_REFRESH_SKEW_MS) {
    // Still valid — but force refresh for scheduled job by bumping expiresAt into the past temporarily
    await prisma.marketingCredential.update({
      where: { connectionId },
      data: { expiresAt: new Date(Date.now() - 1000) },
    });
  }
  const token = await getUnsealedAccessToken(connectionId);
  return token
    ? { ok: true as const }
