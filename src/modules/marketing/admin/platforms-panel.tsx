...
function credentialsReady(platformId: string, config?: PublicMarketingProviderAppConfig) {
  if (platformId === "google-ads") {
    // SEO owns Ads credentials — Platforms treats SEO-backed health as readiness, not Marketing app config.
    return true;
  }
...
function adsOperational(
  platformId: string,
  config?: PublicMarketingProviderAppConfig,
  googleAdsHealth?: GoogleAdsHealthSnapshot | null,
) {
  if (platformId !== "google-ads") return credentialsReady(platformId, config);
  if (googleAdsHealth?.readiness) return googleAdsHealth.readiness === "operational";
  return Boolean(googleAdsHealth?.operational);
}
...
function platformTabStatus(
  ...
  if (providerId === "google-ads") {
    if (googleAdsHealth?.readiness === "operational") return "connected";
    if (googleAdsHealth?.readiness === "setup_incomplete") return "warning";
    if (googleAdsHealth?.oauthConnected) return "warning";
    return "setup";
  }
...
function StatusBadge({
  ...
  if (isConnectedStatus(status) && !readiness) {
    return (
      <Badge className="border-transparent bg-emerald-600 text-white hover:bg-emerald-600">
        Connected
      </Badge>
  ...
  if (!ready || readiness === "not_connected") {
    return (
      <Badge variant="outline" className="text-sky-700 dark:text-sky-300">
        {readiness === "not_connected" ? "Not connected" : "Setup needed"}
      </Badge>
    );
  }
...
function PlatformOverviewCard({
  ...
  const ready =
    platform.id === "google-ads"
      ? Boolean(googleAdsHealth?.oauthConnected || googleAdsHealth?.operational)
      : credentialsReady(platform.id, config);
  const status = platform.connection?.status;
      ...
      <Card className="h-full border-border/70 shadow-sm transition-[border-color,box-shadow,transform] duration-150 group-hover:border-border group-hover:shadow-md group-active:scale-[0.99]">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <StatusBadge
                status={status}
                ready={ready}
                readiness={platform.id === "google-ads" ? googleAdsHealth?.readiness : undefined}
              />
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
            ...
            <span>
              {platform.id === "google-ads"
                ? "Configured in SEO → Google"
                : ready
                  ? "Credentials saved"
                  : "Credentials incomplete"}
            </span>
            {platform.id === "google-ads" &&
            googleAdsHealth?.readiness === "setup_incomplete" ? (
              <span className="text-amber-700 dark:text-amber-300">
                Finish Ads setup under SEO → Google → Ads
              </span>
            ) : null}
          </div>
        </CardContent>
...
function PlatformDetail({
  ...
  const ready =
    providerId === "google-ads"
      ? Boolean(googleAdsHealth?.oauthConnected || googleAdsHealth?.operational)
      : credentialsReady(providerId, config);
  const operational = adsOperational(providerId, config, googleAdsHealth);
  const status = platform?.connection?.status;
      ...
      <Card className="border-border/70 shadow-sm">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div>
                <CardTitle className="flex flex-wrap items-center gap-2 text-base">
                  ...
                  <StatusBadge
                    status={status}
                    ready={ready}
                    readiness={providerId === "google-ads" ? googleAdsHealth?.readiness : undefined}
                  />
                </CardTitle>
                <CardDescription className="mt-1">
                  {providerId === "google-ads"
                    ? "Status only — configure OAuth and Ads API under SEO → Google → Ads."
                    : platform
            ...
            {providerId === "google-ads" ? (
              <div className="flex flex-wrap gap-2">
                <Button asChild>
                  <a href="/admin/seo/google?tab=ads">
                    {googleAdsHealth?.oauthConnected ? "Manage in SEO Google" : "Connect in SEO Google"}
                  </a>
                </Button>
                <Button asChild variant="outline">
                  <a href="/admin/marketing/ad-accounts">Sync Ad Accounts</a>
                </Button>
              </div>
            ) : ready ? (
              <Button asChild>
                <a href={`/api/marketing/oauth/${providerId}/start`}>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-3">
            <Metric
              icon={isConnectedStatus(status) || googleAdsHealth?.oauthConnected ? CheckCircle2 : Unplug}
              label="Connection"
              value={
                providerId === "google-ads"
                  ? googleAdsHealth?.oauthConnected
                    ? "Connected (SEO)"
            ...
            <Metric
              ...
              label="Health"
              value={
                googleAdsHealth?.summary ??
                platform?.runtime?.healthSummary ??
                (ready ? "Ready to connect" : "Needs setup")
              }
            />
          </div>
          {providerId === "google-ads" ? (
            <div className="rounded-lg border border-border/60 bg-muted/10 p-3">
              <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
                  Google Ads operational checks
                </p>
                <a
                  href="/admin/help#topic-seo-google"
              ...
              <p className="mb-2 text-xs text-muted-foreground">
                Credentials and OAuth live under SEO → Google → Ads. Use Ad Accounts to sync
                inventory for the scoreboard.
              </p>
              {googleAdsHealth?.legacySeoCustomerIdHint &&
              (platform?.connection?.accountCount ?? 0) === 0 ? (
                <p className="mb-2 text-xs text-amber-700 dark:text-amber-300">
                  SEO customer {googleAdsHealth.legacySeoCustomerIdHint} is configured —{" "}
                  {googleAdsHealth.oauthConnected ? (
                    <>
                      <a
                      ...
                      to import customers from your Google Ads connection.
                    </>
                  ) : (
                    <>
                      <a
              ...
              <ul className="grid gap-1.5 sm:grid-cols-2">
                {(googleAdsHealth?.items ?? []).map((item) => (
                  <li
                    key={item.id}
                  ...
                  >
                    ...
                    {!item.ok && item.href ? (
                      <Button asChild size="sm" variant="outline" className="shrink-0">
                        <a href={item.href}>{item.actionLabel ?? "Configure"}</a>
                      </Button>
                    ) : null}
                  </li>
                ))}
              ...
              {!operational ? (
                <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">
                  Ads is operational when SEO Google has OAuth + developer token + numeric MCC +
                  customer, then Ad Accounts are synced.
                </p>
              ) : null}
            </div>
          ) : null}
...
export function MarketingPlatformsPanel({
  ...
  const providerTabs = useMemo(() => {
    ...
    return ids.map((id) => {
      ...
      return {
        ...
        status: platformTabStatus(
          platform,
          config,
          id,
          id === "google-ads" ? googleAdsHealth : null,
        ),
      } satisfies SettingsRibbonTab;
    });
  }, [platforms, appConfigs, googleAdsHealth]);
  ...
  const connectedCount = platforms.filter((p) =>
    p.id === "google-ads"
      ? googleAdsHealth?.readiness === "operational" ||
        isConnectedStatus(p.connection?.status)
      : isConnectedStatus(p.connection?.status),
  ).length;
  const setupCount = platforms.filter((p) => {
    if (p.id === "google-ads") {
      return googleAdsHealth?.readiness !== "operational";
    }
    return !credentialsReady(p.id, appConfigs.find((c) => c.providerId === p.id));
  }).length;
    ...
    <div className="space-y-5">
      {activeTab !== "google-ads" ? (
        <AdminServerFormTopBarActions
          ownerKey="marketing-platforms"
          formId={
            activeTab === "linkedin"
              ? "marketing-platforms-linkedin-form"
              : "marketing-platforms-meta-form"
          }
      ...
      <AdminPageHeader
        ...
        description="Connect Meta and LinkedIn here. Google Ads OAuth and credentials are configured under SEO → Google → Ads; this page shows status and links to Ad Accounts."
      />
        ...
        {(tab) => {
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    {platforms.map((platform) => (
                      <PlatformOverviewCard
                        ...
                        googleAdsHealth={
                          platform.id === "google-ads" ? googleAdsHealth : null
                        }
          ...
          return (
            <PlatformDetail
              ...
              googleAdsHealth={tab === "google-ads" ? googleAdsHealth : null}
            />
          );
        }}
      </AdminSettingsLayout>
