...
function MetaCredentialsForm({ config }: { config?: PublicMarketingProviderAppConfig }) {
  return (
    <Card className="border-border/70 shadow-sm">
      ...
      <form
        id="marketing-platforms-meta-form"
        data-admin-save-form=""
        action={upsertProviderAppConfigAction}
      >
...
function LinkedInCredentialsForm({ config }: { config?: PublicMarketingProviderAppConfig }) {
  return (
    <Card className="border-border/70 shadow-sm">
      ...
      <form
        id="marketing-platforms-linkedin-form"
        data-admin-save-form=""
        action={upsertProviderAppConfigAction}
      >
        <CardContent>
...
function GoogleAdsCredentialsForm({ config }: { config?: PublicMarketingProviderAppConfig }) {
  return (
    <Card id="google-ads-credentials" className="scroll-mt-24 border-border/70 shadow-sm">
      <CardHeader className="pb-4">
        <CardTitle className="text-base">Google Ads connection</CardTitle>
        <CardDescription>
          OAuth, developer token, MCC, and Ads customer are configured under SEO → Google → Ads.
          Marketing consumes that connection for Ad Accounts and campaign sync.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3 text-sm">
        <p className="text-muted-foreground">
          Status snapshot (from last sync / health): developer token{" "}
          {config?.hasDeveloperToken ? "saved historically" : "not on Marketing store"}
          {config?.hasLoginCustomerId
            ? ` · MCC hint ${config.loginCustomerIdMasked}`
            : ""}
          . Prefer the SEO Google Ads tab as the source of truth.
        </p>
        <Button asChild>
          <a href="/admin/seo/google?tab=ads">Configure in SEO → Google → Ads</a>
        </Button>
      </CardContent>
    </Card>
  );
