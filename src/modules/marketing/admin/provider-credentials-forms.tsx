      ...
      <form action={upsertProviderAppConfigAction}>
        <CardContent className="space-y-4">
          ...
          <FieldGroup
            ...
            description="Optional. Prefer Marketing → Tracking to paste Meta Pixel base code (Events Manager install-by-code). Pixel ID here stays in sync for CAPI."
          ...
          >
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                For full code setup (paste base code + auto-install), use{" "}
                <a href="/admin/marketing/tracking" className="text-primary underline-offset-2 hover:underline">
                  Tracking
                </a>
                .
              </p>
  ...
  return (
    <Card className="border-border/70 shadow-sm">
      <CardHeader className="pb-4">
        ...
        <CardDescription>
          OAuth app credentials for Company Pages and Ads. Required to connect. Secrets are encrypted at rest.
        </CardDescription>
      </CardHeader>
      <form action={upsertProviderAppConfigAction}>
        <CardContent>
          ...
          <FieldGroup
            title="OAuth app"
            description="Used to connect LinkedIn organization pages and ad accounts."
            requirement="required"
            icon={KeyRound}
          >
            <div className="space-y-1.5">
...
function GoogleAdsCredentialsForm({ config }: { config?: PublicMarketingProviderAppConfig }) {
  return (
    <Card className="border-border/70 shadow-sm">
      <CardHeader className="pb-4">
        <CardTitle className="text-base">Google Ads app credentials</CardTitle>
        <CardDescription>
          OAuth client plus developer token and login customer ID (stored in provider metadata).
        </CardDescription>
      </CardHeader>
      <form action={upsertProviderAppConfigAction}>
        <CardContent className="space-y-4">
          ...
          <FieldGroup
            title="OAuth app"
            description="Google Cloud OAuth client for Ads API."
            requirement="required"
            icon={KeyRound}
          >
            <div className="space-y-1.5">
              <FieldLabel htmlFor="ga-client-id" required>
                Client ID
              </FieldLabel>
              <Input
                id="ga-client-id"
            ...
            <div className="space-y-1.5">
              <FieldLabel htmlFor="ga-client-secret" required>
                Client secret
              </FieldLabel>
              <Input
                id="ga-client-secret"
                name="clientSecret"
                type="password"
                placeholder={config?.hasClientSecret ? "••••••••" : "Google OAuth client secret"}
                autoComplete="new-password"
              />
              <SecretHint saved={Boolean(config?.hasClientSecret)} />
            </div>
          ...
          <FieldGroup
            title="Ads API"
            description="Required for campaign sync via GAQL."
            requirement="required"
            icon={Megaphone}
          >
            <div className="space-y-1.5">
              <FieldLabel htmlFor="ga-developer-token" required>
                Developer token
              </FieldLabel>
              <Input id="ga-developer-token" name="developerToken" type="password" placeholder="Google Ads developer token" />
            </div>
            <div className="space-y-1.5">
              <FieldLabel htmlFor="ga-login-customer" required>
                Login customer ID
              </FieldLabel>
              <Input
                id="ga-login-customer"
        ...
        <CardFooter className="border-t border-border/60 bg-muted/10 px-6 py-4">
          <Button type="submit">Save Google Ads credentials</Button>
        </CardFooter>
      </form>
    </Card>
  );
...
export function MarketingProviderCredentialsForms({
  ...
  const googleAds = configs.find((c) => c.providerId === "google-ads");
  ...
  if (providerId === "google-ads") return <GoogleAdsCredentialsForm config={googleAds} />;

  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <MetaCredentialsForm config={meta} />
      <LinkedInCredentialsForm config={linkedin} />
      <GoogleAdsCredentialsForm config={googleAds} />
    </div>
  );
}
