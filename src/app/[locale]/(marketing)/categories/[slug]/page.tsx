...
import { CollectionDetailHero } from "@/features/collections/components/collection-detail-hero";
import { CollectionSubcollectionsBrowser } from "@/features/collections/components/collection-subcollections-browser";
...
export async function generateMetadata({ params }: Props) {
  ...
  const title = collection?.seo?.metaTitle || collection?.name || "Category";
  const description =
    collection?.seo?.metaDescription || collection?.description || "Browse category products.";
...
export default async function CategoryDetailPage({ params }: Props) {
  ...
  return (
    <div className="col-detail-wrap" dir={pageDir}>
      <CatalogTopNavigation locale={locale} surface="categoryDetail" categorySlug={slug} />
      <nav className="col-detail-breadcrumb text-xs text-muted-foreground mb-4 flex flex-wrap gap-1" aria-label="Breadcrumb">
        <Link href={publicLocalePath(locale, "/categories")} className="hover:underline">
          Categories
        </Link>
        {trail.map((item) => (
      ...
      <CollectionSubcollectionsBrowser
