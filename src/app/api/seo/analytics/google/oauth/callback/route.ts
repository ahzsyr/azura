...
export async function GET(request: NextRequest) {
  ...
  const clientId = oauthClient.clientId?.trim() || google.clientId?.trim() || "";
  const clientSecret = oauthClient.clientSecret?.trim() || google.clientSecret?.trim() || "";
  if (!clientId || !clientSecret) {
    return oauthAdminRedirect(request, {
      googleOAuth: "error",
      message: "Missing Google OAuth client credentials. Save them in Admin → SEO → Google.",
    }, targetTab);
  }
