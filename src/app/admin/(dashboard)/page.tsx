...
export default async function AdminDashboardPage() {
  let stats = { packages: 0, newFormSubmissions: 0, testimonials: 0, gallery: 0 };
  let platform = { pages: 0, posts: 0, media: 0, packages: 0, newFormSubmissions: 0 };
  let recentSubmissions: Awaited<
    ReturnType<
      typeof prisma.formSubmission.findMany<{
        include: { template: { select: { name: true } } };
      }>
    >
  > = [];
  let customerCount = 0;
  let branding = { brandName: "Site", brandShort: "SI", tagline: "", logoUrl: "" };

  try {
    const [statsResult, platformResult, brand, submissions, customers] = await Promise.all([
      getDashboardStats(),
