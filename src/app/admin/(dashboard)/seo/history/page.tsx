...
export default async function AdminSeoAuditHistoryPage() {
  ...
  return (
    <div className="space-y-8">
      <AdminPageHeader
        title="Audit History"
        description="Past site audit snapshots. Open one to view Site Audit, Technical Audit, or Tasks for that run."
      />
                ...
                <tr key={row.id} className="border-b last:border-0">
                  ...
                  <td className="px-3 py-2 space-x-2">
                    <Link
                      ...
                      href={`/admin/seo/audit?snapshotId=${encodeURIComponent(row.id)}`}
                    >
                      Overview
                    </Link>
                    <Link
                    ...
                    <Link
                      className="text-primary underline-offset-4 hover:underline"
                      href={`/admin/seo/tasks?snapshotId=${encodeURIComponent(row.id)}`}
                    >
                      Tasks
                    </Link>
                  </td>
