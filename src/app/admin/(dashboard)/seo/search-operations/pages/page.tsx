...
export default async function SearchOpsPagesWorkspace() {
  ...
  const latestInspect = platform.operations
    ...
    .find(
      (op) =>
        op.definitionId === "page.inspect_url" &&
        op.status === "completed" &&
        op.result &&
        String(op.result.url ?? "") === url,
    );
  const inspector = latestInspect?.result
    ? {
        url,
        indexed: Boolean(latestInspect.result.indexed),
        lastCrawledLabel: String(latestInspect.result.lastCrawledLabel ?? "—"),
        canonical: String(latestInspect.result.canonical ?? url),
        richResults: (latestInspect.result.richResults as "valid" | "invalid" | "unknown") ?? "unknown",
        breadcrumbValid: Boolean(latestInspect.result.breadcrumbValid),
        faqValid: Boolean(latestInspect.result.faqValid),
        mobileFriendly: Boolean(latestInspect.result.mobileFriendly),
        cwv: (latestInspect.result.cwv as "good" | "needs_improvement" | "poor") ?? "needs_improvement",
      }
    : platform.inspectUrl(url);
    ...
    <div className="space-y-8 max-w-6xl">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {latestInspect
              ? `Latest live inspection · ${new Date(latestInspect.completedAt ?? latestInspect.updatedAt).toLocaleString()}`
              : "Run Inspect URL to pull live Search Console data (placeholder shown until then)."}
          </CardDescription>
        </CardHeader>
