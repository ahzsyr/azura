...
export default async function SearchOpsMonitoringWorkspace() {
  ...
  const pageSpeedDataSource = latestPageSpeed?.result?.dataSource;
  const useCrux = pageSpeedDataSource === "crux";
  const cwvSample = {
    url: platform.siteOrigin,
    lcpMs:
      typeof latestPageSpeed?.result?.lcpMs === "number" && useCrux
        ? latestPageSpeed.result.lcpMs
        : 0,
    cls:
      typeof latestPageSpeed?.result?.cls === "number" && useCrux ? latestPageSpeed.result.cls : 0,
    inpMs:
      typeof latestPageSpeed?.result?.inpMs === "number" && useCrux
            ...
            {latestPageSpeed?.result ? (
              <div className="rounded-lg border p-3">
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {typeof latestPageSpeed.result.dataSource === "string"
                    ? ` · ${latestPageSpeed.result.dataSource === "crux" ? "CrUX field" : "lab fallback"}`
                    : ""}
                </p>
                <Link
