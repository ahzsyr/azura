...
export async function requestPasswordReset(input: {
  ...
  const resetLink = `${getSiteBaseUrl()}${accountPublicPath(input.locale, "reset-password")}?token=${encodeURIComponent(rawToken)}`;
  const vars = {
    name: user.name,
    resetLink,
...
export async function requestAdminPasswordReset(input: {
  ...
  const resetLink = `${getSiteBaseUrl()}${accountPublicPath(locale, "reset-password")}?token=${encodeURIComponent(rawToken)}`;
