...
import { UnifiedPageRowActions } from "@/features/pages/components/unified-page-row-actions";
...
function cmsRowForEntry(
  page: UnifiedPageEntry,
  cmsById: Map<string, CmsPageListRow>,
  cmsBySlug: Map<string, CmsPageListRow>,
): CmsPageListRow | undefined {
  const cmsPageId = page.meta?.cmsPageId;
  if (cmsPageId) {
    const byId = cmsById.get(cmsPageId);
    if (byId) return byId;
  }
  return cmsBySlug.get(page.slug);
}

function UnifiedPagesTable({
  pages,
  cmsRows,
  localePrefix,
}: {
  ...
  const cmsById = useMemo(
    () => new Map(cmsRows.map((row) => [row.id, row])),
    [cmsRows],
  );
  const cmsBySlug = useMemo(
    () => new Map(cmsRows.map((row) => [row.slug, row])),
    [cmsRows],
  );
  ...
  return (
    <AdminList>
      {pages.map((page) => {
        const cmsRow = cmsRowForEntry(page, cmsById, cmsBySlug);
        return (
              ...
              <AdminListMetaSmall>
                ...
                {page.layoutAssignmentLabel ||
                  (page.layoutTemplate
                    ...
                    : page.meta?.templateKey
                      ? `Template: ${page.meta.templateKey}`
                      : "layout: —")}
              </AdminListMetaSmall>
            </div>
            <UnifiedPageRowActions
              page={page}
              cmsRow={cmsRow}
              localePrefix={localePrefix}
            />
