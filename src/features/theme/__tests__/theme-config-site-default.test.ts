...
describe("siteThemeToTokens preset ownership", () => {
  ...
  it("parses effect and motion settings from site theme", () => {
    const tokens = siteThemeToTokens(
      makeTheme({
        cursorEffectSettings: { intensity: 1.2, opacity: 0.8, speed: 1.1 },
        textEffectSettings: { intensity: 0.9, opacity: 0.7 },
        motionSettings: { intensity: 1.1, opacity: 0.95 },
      }),
    );
    assert.equal(tokens.cursorEffectSettings.intensity, 1.2);
    assert.equal(tokens.cursorEffectSettings.opacity, 0.8);
    assert.equal(tokens.textEffectSettings.intensity, 0.9);
    assert.equal(tokens.motionSettings.intensity, 1.1);
  });
});
