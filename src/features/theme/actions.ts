...
export async function saveThemeDraft(formData: FormData) {
  ...
  const raw = {
    ...
    siteDefaultPresetId:
      (formData.get("siteDefaultPresetId") as string) ||
      (formData.get("activePresetId") as string) ||
      null,
  ...
  await themeService.saveDraft({
    ...
    siteDefaultPresetId: parsed.siteDefaultPresetId ?? parsed.activePresetId ?? null,
    ...
    themeProvenance:
      ...
      ({ sourcePresetId: null, appliedAt: null, customizedAfterPreset: false, presetLockMode: "unlocked" } as object),
  });
...
export async function saveThemeDraftPatch(changes: Record<string, unknown>) {
  ...
  fd.set("siteDefaultPresetId", merged.siteDefaultPresetId ?? merged.activePresetId ?? "");
