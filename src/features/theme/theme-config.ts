...
const VALID_IOS_STATUS_BAR_STYLES: IosStatusBarStyle[] = [
  "default",
  "black",
  "black-translucent",
];

export function parseMobileBrowserConfig(raw: unknown): MobileBrowserConfig {
  const r = asRecord(raw);
  const iosStyle = r.iosStatusBarStyle;
  return {
    syncWithTheme: r.syncWithTheme !== false,
    browserThemeColorLight:
      typeof r.browserThemeColorLight === "string" ? r.browserThemeColorLight : null,
    browserThemeColorDark:
      typeof r.browserThemeColorDark === "string" ? r.browserThemeColorDark : null,
    browserBackgroundColor:
      typeof r.browserBackgroundColor === "string" ? r.browserBackgroundColor : null,
    iosStatusBarStyle:
      typeof iosStyle === "string" && VALID_IOS_STATUS_BAR_STYLES.includes(iosStyle as IosStatusBarStyle)
        ? (iosStyle as IosStatusBarStyle)
        : "default",
  };
}
...
export function siteThemeToTokens(theme: SiteTheme): ThemeTokens {
  ...
  return {
    ...
    cursorEffectSettings: parseVisualEffectSettings(
      "cursorEffectSettings" in theme
        ? (theme as SiteTheme & { cursorEffectSettings?: unknown }).cursorEffectSettings
        : {},
    ),
    textEffectSettings: parseVisualEffectSettings(
      "textEffectSettings" in theme
        ? (theme as SiteTheme & { textEffectSettings?: unknown }).textEffectSettings
        : {},
    ),
    motionSettings: parseMotionSettings(
      "motionSettings" in theme
        ? (theme as SiteTheme & { motionSettings?: unknown }).motionSettings
        : {},
    ),
    ...
    mobileBrowserConfig: parseMobileBrowserConfig(theme.mobileBrowserConfig),
  };
}
