...
import { applyPresetColors } from "./colors";
import { clearVisitorThemeDomOverrides } from "./visitor-dom-cleanup";
...
export function buildLiveVisualExperience(
  ...
  return {
    ...
    cursorEffectSettings:
      useLive && liveEffects?.cursorEffectSettings != null
        ? parseVisualEffectSettings(liveEffects.cursorEffectSettings)
        : site.cursorEffectSettings,
    textEffectSettings:
      useLive && liveEffects?.textEffectSettings != null
        ? parseVisualEffectSettings(liveEffects.textEffectSettings)
        : site.textEffectSettings,
    motionSettings:
      useLive && liveEffects?.motionSettings != null
        ? parseMotionSettings(liveEffects.motionSettings)
        : site.motionSettings,
...
export function restorePresetColorsFromStorage(resolvedAppearance: ResolvedAppearance): boolean {
  const visitorPresetId = readStoredPresetId();
  if (!visitorPresetId) return false;
