...
export function useThemeStudio(base: SiteTheme) {
  ...
  const setState = useCallback((updater: ThemeTokens | ((prev: ThemeTokens) => ThemeTokens)) => {
    setStateRaw((prev) => {
      ...
      const shouldMarkCustomized =
        Boolean(next.siteDefaultPresetId) &&
        !(next.themeProvenance?.presetLockMode === "baseline-reset" && next.themeProvenance?.customizedAfterPreset === false);
      const normalizedNext = shouldMarkCustomized
        ? {
            ...next,
            themeProvenance: {
              sourcePresetId: next.themeProvenance?.sourcePresetId ?? next.siteDefaultPresetId,
              appliedAt: next.themeProvenance?.appliedAt ?? prev.themeProvenance?.appliedAt ?? null,
