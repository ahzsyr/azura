...
function runApply(
  ...
  // colorsOnly refreshes CSS vars / appearance paints — it must not claim a full
  // bind, or later text/cursor tagging is skipped when signatures already match.
  if (applyOptions.colorsOnly) {
    if (lastEffectSignature !== null) {
