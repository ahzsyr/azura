...
import { mountSectionBackgroundSync } from "./section-runtime";
import { unmountSiteBackground } from "./site-runtime";
...
export function downgradeSiteBackgroundForPolicy(
  ...
  // Capable mobiles: allowHeavy is false (cursors/desktop), but animated site
  // backgrounds should still run — only collapse when motion/paint budget is gone.
  if (tier === "heavy" && !policy.allowHeavy) {
    if (policy.allowAnimatedBackground && policy.allowMedium) {
      return normalized;
    }
    return resolveConstrainedSiteEffect(normalized);
  }
...
/**
 * Legacy global site background entry — SiteBackgroundLayer owns mounting.
 * Updates body dataset and tracks active effect for policy/downgrade consumers.
 * Does not restore appearance colors — ThemeEngineProvider owns surfaces.
 */
...
export function mountSectionAnimatedBackground(
  ...
  const runtimeEffect = isConstrainedBackgroundEnvironment()
    ...
    : effect;
  return mountSectionBackgroundSync(container, runtimeEffect as BackgroundEffectId);
}
