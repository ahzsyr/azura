    ...
    <div className={cn("pb-partners", `pb-partners--${layout}`, showMap && "pb-partners--with-map")}>
      ...
      {showSearch && (
        <Input
          ...
          placeholder={locale.startsWith("ar") ? "بحث..." : "Search partners..."}
          value={query}
        ...
        {partners.map((partner) => (
          <li key={partner.id} className="pb-partners__card rounded-xl border p-4">
            ...
            {partner.websiteUrl && (
              ...
              >
                {locale.startsWith("ar") ? "الموقع" : "Website"}
              </a>
            )}
          </li>
        ))}
      ...
      {showMap && layout === "map" && (
        <p className="text-sm text-muted-foreground mt-6 text-center">
          {locale.startsWith("ar") ? "خريطة الشركاء قريباً." : "Partner map view coming soon."}
        </p>
      )}
    </div>
  );
