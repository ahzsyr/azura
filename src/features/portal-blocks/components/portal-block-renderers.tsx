...
export async function PricingCalculatorBlockRenderer({ locale, props, loc }: RenderCtx) {
  ...
  const subtitle = loc("subtitle") || pickLocale(p, "subtitle", asLocale(locale));
  return (
    <Section>
  ...
  return (
    <Section>
      <KnowledgeBaseView
        locale={asLocale(locale)}
        knowledgeBase={kb}
        title={loc("title") || pickLocale(p, "title", asLocale(locale))}
        subtitle={loc("subtitle") || pickLocale(p, "subtitle", asLocale(locale))}
        layout={p.layout}
