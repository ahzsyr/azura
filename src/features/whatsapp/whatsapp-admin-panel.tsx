...
const MESSAGE_KEYS: { key: WhatsAppMessageTemplateKey; label: string; placeholders: string }[] = [
  {
    key: "message.default",
...
export function WhatsAppAdminPanel({
  ...
  const { markDirty } = useDesignHubSettingsActions({
    ...
    saveLabel: "Save",
    publishEntityType: "app-settings",
  ...
  function patchMessageTemplate(key: WhatsAppMessageTemplateKey, locale: string, value: string) {
    if (!settings) return;
    markDirty();
    setSettings({
      ...settings,
  ...
  const previewMessage =
    settings.messages["message.default"]?.[activeLocale] ||
          ...
          tab === "overview" ? (
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Customize locale-specific WhatsApp messages here. Empty fields fall back to{" "}
                  <code className="text-xs">messages/{"{locale}"}.json</code> under the{" "}
                  <code className="text-xs">whatsapp</code> namespace.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-2">
                ...
                <div className="space-y-4">
                  {MESSAGE_KEYS.map((item) => {
                    const current =
                      settings.messages[item.key]?.[activeLocale] ?? fileFallbacks[item.key] ?? "";
                    return (
                      <AdminCollapsibleSection
                        key={item.key}
                        title={item.label}
                      ...
                      >
                        <Textarea
                          value={current}
                          onChange={(e) => patchMessageTemplate(item.key, activeLocale, e.target.value)}
                          rows={3}
                          placeholder={fileFallbacks[item.key] ?? ""}
                          className="min-h-[88px] resize-y"
                        />
                        <p className="text-xs text-muted-foreground">
                          Fallback key: <code>whatsapp.{item.key}</code>
                        </p>
                      </AdminCollapsibleSection>
                    );
                  })}
