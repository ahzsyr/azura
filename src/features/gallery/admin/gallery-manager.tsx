...
function SortableAlbumCard({
  ...
  const handleDelete = () => {
    if (!confirm(`Delete gallery "${album.displayTitle}" and all its media?`)) return;
    startTransition(async () => {
  ...
  return (
    ...
    >
      <div className="relative aspect-video bg-muted">
        {thumb ? (
          <img src={thumb} alt={album.displayTitle} className="h-full w-full object-cover" />
        ) : (
      ...
      <div className="space-y-3 p-4">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="font-medium">{album.displayTitle}</h3>
            {!album.isPublished && (
...
export function GalleryManager({ albums: initialAlbums, locales, initialCreateOpen = false }: Props) {
  ...
  return (
    <div>
      ...
      <GalleryCreateModal open={createOpen} onOpenChange={setCreateOpen} locales={locales} />
    </div>
  );
}
