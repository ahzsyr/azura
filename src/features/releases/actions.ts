...
export async function upsertReleaseSet(formData: FormData) {
  ...
  const slug = slugInput
    ...
    : await uniqueSlug(prisma.releaseSet, titleEn, id, "releases");
  ...
  for (let i = 0; i < releases.length; i++) {
    ...
    for (let j = 0; j < entries.length; j++) {
      ...
      const category = Object.values(ReleaseEntryCategory).includes(categoryRaw as ReleaseEntryCategory)
        ? (categoryRaw as ReleaseEntryCategory)
        : ReleaseEntryCategory.FEATURES;
      const entryData = {
        releaseId: release.id,
        category,
        textEn: str(entry.textEn),
        textAr: str(entry.textAr),
      ...
      if (entryId) {
        ...
        const created = await prisma.releaseEntry.create({ data: entryData });
        keptEntryIds.add(created.id);
      }
    }
