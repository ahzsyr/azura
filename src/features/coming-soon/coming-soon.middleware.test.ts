...
test("isComingSoonExemptPage exempts localized account login", () => {
  assert.equal(isComingSoonExemptPage("/account/login", false, LOCALES), true);
  assert.equal(isComingSoonExemptPage("/ar/account/login", false, LOCALES), true);
  assert.equal(isComingSoonExemptPage("/account/verify-email", false, LOCALES), true);
  assert.equal(isComingSoonExemptPage("/account/accept-invite", false, LOCALES), true);
  assert.equal(isComingSoonExemptPage("/account", false, LOCALES), false);
  assert.equal(isComingSoonExemptPage("/account/register", false, LOCALES), false);
});
