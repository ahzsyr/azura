...
import type { categoryExplorerPropsSchema } from "@/features/discovery-blocks/schemas/discovery-blocks";
...
export async function loadCategoryExplorerNodes(
  ...
  switch (config.source) {
    ...
    case "postCategories": {
      ...
      return cats.map((cat) => ({
        slug: cat.slug,
        name: cat.nameEn || cat.nameAr || cat.slug,
    ...
    case "contentCollections": {
      ...
      return rows.map((col) => ({
        slug: col.slug,
        name: col.nameEn || col.nameAr || col.slug,
...
function manualToNode(
  ...
  const label = n.labelEn || n.labelAr || n.id;
