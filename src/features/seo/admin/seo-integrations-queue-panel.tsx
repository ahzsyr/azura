...
export function IntegrationsQueuePanel({ metrics, health, sitemapUrl }: IntegrationsQueuePanelProps) {
  ...
  const runAction = (
    ...
    action: () => Promise<SeoActionResult>,
    options?: { refreshAfter?: boolean },
  ) => {
    startTransition(async () => {
      try {
        const result = await runAdminAction(runningMessage, action);
        setFeedback(result);
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <WorkflowStep
            ...
            onClick={() =>
              runAction("Syncing analytics…", runSeoAnalyticsIngestionAction, { refreshAfter: false })
            }
          />
          <WorkflowStep
            step={2}
            ...
            buttonVariant="outline"
            onClick={() => runAction("Queuing sitemap submission…", enqueueSitemapSubmissionAction)}
          />
          <WorkflowStep
            step={3}
            title="Run queue now"
            ...
            onClick={() => runAction("Running submission queue…", runSeoSubmissionQueueAction)}
          />
          ...
          <div className="rounded-lg border border-dashed p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Button
                type="button"
                disabled={pending}
                onClick={() => runAction("Submitting sitemap to search engines…", submitSitemapAndRunAction)}
                className="w-full shrink-0 sm:w-auto"
              >
                {pending ? "Working…" : "Submit sitemap to search engines"}
              </Button>
            </div>
          </div>
