...
export function IntegrationsQueuePanel({ metrics, health, sitemapUrl }: IntegrationsQueuePanelProps) {
  ...
  const configuredProviders = health.filter((item) => item.enabled && item.configured);
  const setupComplete = configuredProviders.length > 0;

  const runAction = (action: () => Promise<SeoActionResult>) => {
        ...
        <CardContent className="space-y-4">
          <ul className="space-y-2">
            {health.map((item) => (
              <PrerequisiteRow
                key={item.provider}
                ok={item.enabled && item.configured}
                label={`${item.provider}: ${item.enabled ? (item.configured ? item.message : "needs credentials and site URL") : "disabled"}`}
              />
            ))}
            {health.length === 0 ? (
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap gap-3">
          ...
          <Button
            ...
            variant="outline"
            disabled={pending}
            onClick={() => runAction(runSeoAnalyticsIngestionAction)}
          >
            Run analytics sync
          </Button>
        </div>
