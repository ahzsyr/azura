...
function withLocalePrefix(localePrefix: string, path: string, defaultPrefix: string): string {
  const normalized = path === "/" || path === "" ? "/" : path.startsWith("/") ? path : `/${path}`;
  return publicLocalePath(localePrefix, normalized, defaultPrefix);
}
...
export async function listPublicRouteCatalog(): Promise<RouteCatalogEntry[]> {
  ...
  for (const page of STATIC_SEO_PAGES) {
    const path = withLocalePrefix(defaultLocale, page.path || "/", defaultLocale);
    add({
      path: page.path === "" ? "/" : path,
      label: page.label,
      source: "wired",
  ...
  for (const [slug, wiredPath] of Object.entries(CMS_WIRED_MARKETING_SLUGS)) {
    const path = withLocalePrefix(defaultLocale, wiredPath || "/", defaultLocale);
    add({
      path: wiredPath === "" || wiredPath === "/" ? "/" : path,
      label: slug,
      source: "wired",
    });
  }
  ...
  try {
    ...
    for (const page of cmsPages) {
      ...
      add({
        path: withLocalePrefix(defaultLocale, publicPath || "/", defaultLocale),
        label: page.slug,
    ...
    for (const post of posts) {
      add({
        path: withLocalePrefix(defaultLocale, `/blog/${post.slug}`, defaultLocale),
        label: post.slug,
        source: "blog",
      });
    }
    ...
    for (const faq of faqSets) {
      add({
        path: withLocalePrefix(defaultLocale, `/faq/${faq.slug}`, defaultLocale),
        label: faq.slug,
        source: "faq",
      });
    }
    ...
    for (const item of contentItems) {
      ...
      add({
        path: withLocalePrefix(
          defaultLocale,
          `/${item.contentType.routePrefix}/${item.slug}`,
          defaultLocale,
        ),
        label: item.slug,
        source: "content",
