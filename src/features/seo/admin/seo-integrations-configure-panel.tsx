...
export function IntegrationsConfigurePanel({
  ...
  useEffect(() => {
    if (savePending) {
      ...
      useAdminActionToastStore.getState().show("running", "Saving search engine connections…");
      return;
    }
    ...
    if (saveState?.ok) {
      useAdminActionToastStore
        .getState()
        .show("success", saveState.message?.trim() || "Search engine connections saved.");
      markSaved();
      ...
      useAdminActionToastStore
        .getState()
        .show("error", saveState.message?.trim() || "Save failed.");
      setSaveStatus("error");
    }
  }, [
    savePending,
