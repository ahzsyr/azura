export function gscPropertyKind(siteUrl: string): "domain" | "url_prefix" {
  return normalizeGscSiteUrl(siteUrl).startsWith("sc-domain:") ? "domain" : "url_prefix";
}
...
/** Prefer the verified GSC property string; otherwise normalize the configured URL. */
export function resolvePreferredGscSiteUrl(params: {
  matchedGscSiteUrl?: string | null;
  configuredUrl?: string | null;
}): string {
  const matched = params.matchedGscSiteUrl?.trim();
  if (matched) return matched;
  return normalizeGscSiteUrl(params.configuredUrl ?? "");
}
