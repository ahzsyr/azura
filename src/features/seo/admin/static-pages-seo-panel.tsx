...
function metaStatus(
  ...
  if (!translations?.metaTitleEn?.trim() && !translations?.metaTitleAr?.trim()) return "empty";
  const { score } = scoreSeoMeta(meta, translations);
...
export function StaticPagesSeoPanel({ metaByKey, translationsByKey }: Props) {
  ...
  const pageList = (
    <ul className="space-y-0.5 p-1">
      {filteredPages.map((page) => {
        ...
        const status = metaStatus(metaByKey[page.pageKey], translationsByKey[page.pageKey]);
        return (
          <li key={page.pageKey}>
            <button
              type="button"
      ...
      <Card className="min-w-0">
        ...
        <CardContent>
          <SeoMetaForm
            ...
            meta={selectedMeta}
            translations={translationsByKey[selectedPageKey]}
