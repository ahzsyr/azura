...
import {
  loadPageKeySeoFormAction,
  loadStaticPageSeoFormAction,
...
function resolveSelectedPageKey(pageParam: string | null): string {
  if (!pageParam?.trim()) return STATIC_SEO_PAGES[0].pageKey;
  if (isStaticSeoPageKey(pageParam)) return pageParam;
  // Non-static keys (e.g. product:slug) are loaded via loadPageKeySeoFormAction.
...
function labelForPageKey(pageKey: string): string {
  if (isStaticSeoPageKey(pageKey)) {
    return getStaticSeoPage(pageKey)?.label ?? pageKey;
  }
  if (pageKey.startsWith("product:")) {
    return `Product: ${pageKey.slice("product:".length)}`;
  }
  return pageKey;
}
...
function pathForPageKey(pageKey: string): string {
  if (isStaticSeoPageKey(pageKey)) {
    return getStaticSeoPage(pageKey)?.path || "/";
  }
  if (pageKey.startsWith("product:")) {
    return `/products/${pageKey.slice("product:".length)}`;
  }
  return `/${pageKey}`;
}
...
export function StaticPagesSeoPanel({ contextsByKey }: Props) {
  ...
  const isStaticSelected = isStaticSeoPageKey(selectedPageKey);
  const selectedLabel = labelForPageKey(selectedPageKey);
  const selectedPath = pathForPageKey(selectedPageKey);
  const serverContext = isStaticSelected ? contextsByKey[selectedPageKey] : undefined;
  ...
  useEffect(() => {
    ...
    const loader = isStaticSeoPageKey(selectedPageKey)
      ? loadStaticPageSeoFormAction(selectedPageKey)
      : loadPageKeySeoFormAction(selectedPageKey);
  ...
  const formProps =
    ...
    loadedFormProps ??
    (isStaticSelected
      ? buildMinimalStaticPageFormProps(selectedPageKey)
      : {
          pageKey: selectedPageKey,
          meta: null,
          translations: {},
          savedTranslations: {},
          defaultTitleEn: selectedLabel,
          defaultTitleAr: selectedLabel,
          defaultDescEn: "",
          defaultDescAr: "",
          previewOrigin: "",
          publicPath: selectedPath,
        });
  ...
  const previewPath = `/${routing.defaultLocale}${selectedPath === "/" ? "" : selectedPath}`;
  ...
  const pageList = (
    <ul className="space-y-0.5 p-1">
      {!isStaticSelected ? (
        <li>
          <div className="mb-1 rounded-md border border-primary/30 bg-primary/5 px-2.5 py-2 text-sm">
            <span className="block font-medium leading-tight">{selectedLabel}</span>
            <span className="mt-0.5 block truncate text-xs text-muted-foreground">{selectedPageKey}</span>
          </div>
        </li>
      ) : null}
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-[minmax(200px,240px)_1fr] md:gap-6">
      <div className="md:hidden">
        ...
        <select
          ...
          value={isStaticSelected ? selectedPageKey : ""}
          onChange={(e) => {
        ...
        <CardHeader className="pb-3">
          <div className="flex flex-wrap items-start justify-between gap-2">
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              {selectedPageKey.startsWith("product:") ? (
                <Link
                  href={`/admin/products?product=${encodeURIComponent(selectedPageKey.slice("product:".length))}#seo`}
        ...
        <CardContent>
          {isLoadingForm && !serverFormProps && !loadedFormProps ? (
            ...
            <SeoMetaForm
              ...
              embedded
              useTopBarActions
              entityType={selectedPageKey.startsWith("product:") ? "product" : undefined}
              entityId={
                selectedPageKey.startsWith("product:")
                  ? selectedPageKey.slice("product:".length)
                  : undefined
              }
            />
          )}
