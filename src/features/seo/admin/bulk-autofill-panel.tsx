...
import { runAdminAction, useAdminActionToastStore } from "@/components/admin/layout/admin-action-toast";
import {
  bulkAutoFillAction,
  countBulkSeoAction,
...
export function BulkAutoFillPanel() {
  ...
  const runSimulation = useCallback(() => {
    startTransition(async () => {
      ...
      await runAdminAction("Simulating auto-fill…", async () => {
        const result = await bulkAutoFillAction(fd);

        if (isSegmentRequiredError(result)) {
          const message = `${result.total.toLocaleString()} processing units require segment selection (max ${BULK_SEGMENT_THRESHOLD} per request).`;
        ...
        if ("dryRun" in result && result.dryRun) {
          ...
          return {
            ok: true,
            message: `Simulation ready: ${result.estimatedChanges} will update.`,
          };
        }

        return { ok: false, message: "Unexpected simulation response." };
      });
    });
  }, [
    target,
  ...
  const runBulk = useCallback(() => {
    startTransition(async () => {
      setErrorMessage(null);
      try {
        const result = await runAdminAction("Applying auto-fill…", async () => {
          const next = await runBulkSegment(requiresSegmentation ? selectedSegment : null);
          return {
            ...next,
            ok: next.failed === 0,
            message: `Updated ${next.changed} · skipped ${next.skipped} · failed ${next.failed}`,
          };
        });
  ...
  const runAllSegments = useCallback(() => {
    ...
    startTransition(async () => {
      ...
      show("running", "Running bulk auto-fill…");
      let totalChanged = 0;
      let totalSkipped = 0;
      let totalFailed = 0;
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        show("running", `Running segment ${i + 1} / ${segments.length}…`);
        setQueueProgress({
          current: i + 1,
        ...
        try {
          ...
          setQueueProgress(null);
          show("error", `Stopped at segment ${i + 1}: ${message}`);
          return;
        }
      }

      setQueueProgress(null);
      const done = `All segments complete — updated ${totalChanged} · skipped ${totalSkipped} · failed ${totalFailed}`;
      setRunResult(done);
      setSimulation(null);
      show("success", done);
    });
