...
export function SeoMetaForm({
  ...
  const suggestedCanonicalUrl = useMemo(
    () => buildAutoCanonicalUrl(canonicalOrigin, activeLocale.urlPrefix, publicPath),
    [canonicalOrigin, activeLocale.urlPrefix, publicPath],
  );
  ...
  useEffect(() => {
    ...
    if (!suggestedCanonicalUrl) return;
    setCanonicalUrl(suggestedCanonicalUrl);
  }, [canonicalUrl, suggestedCanonicalUrl]);
  ...
  const applyAutofillToForm = useCallback(
    (changeSet: SeoChangeSet, selected: ReadonlySet<string>) => {
      for (const field of changeSet.fields) {
        ...
        switch (field.field) {
          ...
          case "ogTitle":
            ...
            break;
          case "focusKeywords":
            setFocusKeywords(field.next);
            break;
          case "canonicalUrl":
            setCanonicalUrl(field.next);
            break;
          case "robots":
          ...
          case "jsonLd":
            try {
              ...
              setJsonLdStr(JSON.stringify(parsed, null, 2));
            } catch {
              setJsonLdStr(field.next);
            }
            break;
          default:
            break;
      ...
      if (meta) {
        if (selected.has("focusKeywords") && meta.focusKeywords && !changeSet.fields.some((f) => f.field === "focusKeywords")) {
          setFocusKeywords(meta.focusKeywords);
        }
        if (selected.has("canonicalUrl") && meta.canonicalUrl && !changeSet.fields.some((f) => f.field === "canonicalUrl")) {
          setCanonicalUrl(meta.canonicalUrl);
        }
        if (selected.has("robots") && meta.robots && !changeSet.fields.some((f) => f.field === "robots")) {
          setRobots(meta.robots);
        }
        if (selected.has("ogImageUrl") && meta.ogImageUrl && !changeSet.fields.some((f) => f.field === "ogImageUrl")) {
          setOgImageUrl(meta.ogImageUrl);
        }
        if (selected.has("twitterCard") && meta.twitterCard && !changeSet.fields.some((f) => f.field === "twitterCard")) {
          setTwitterCard(meta.twitterCard);
        }
        if (selected.has("jsonLd") && meta.jsonLd != null && !changeSet.fields.some((f) => f.field === "jsonLd")) {
          setJsonLdStr(
            typeof meta.jsonLd === "string"
              ? meta.jsonLd
  ...
  }, [initialByLocale, meta, defaultOgImageUrl]);
