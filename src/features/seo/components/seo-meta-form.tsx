...
function buildAutoCanonicalUrl(origin: string, localePrefix: string, publicPath?: string): string {
  ...
  return publicLocaleAbsoluteUrl(origin, localePrefix, path);
}
  ...
  const handleEmbeddedSubmit = () => {
    startTransition(async () => {
      const result = await runAdminAction(
        "Saving metadata…",
        () => upsertSeoMetaAction(buildFormData()),
        "Metadata saved.",
      );
          ...
          {activeTab === "edit" && resolvedEntityId ? (
            <Button
              ...
              onClick={() => {
                startAutoFill(async () => {
                  const result = await runAdminAction(
                    "Generating auto-fill suggestions…",
                    () =>
        ...
        <>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder={suggestedCanonicalUrl || "https://yoursite.com/page"}
                />
                <SeoFieldHint
            ...
            <GoogleRichResultPreview
              title={previewTitle}
              description={previewDesc}
              url={previewOrigin?.includes("://") ? previewOrigin : `https://${previewOrigin ?? "brt-me.com"}`}
