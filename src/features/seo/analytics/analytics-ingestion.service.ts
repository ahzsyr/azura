...
async function refreshGoogleToken(config: SeoIntegrationProviderConfig) {
...
export const seoAnalyticsIngestionService = {
  async run(days = 3) {
    ...
    if (config.google?.analyticsEnabled) {
      try {
        const token = await refreshGoogleToken(config.google);
        const rows = await fetchGoogleSearchMetrics(config.google, days);
        await seoRepository.upsertSearchMetrics(rows);
        const richIssues = token ? await importGoogleRichResults(config.google, token, rows) : 0;
        results.push({ provider: "google", imported: rows.length + richIssues, ok: true });
      } catch (error) {
        results.push({
          provider: "google",
          imported: 0,
