...
import {
  aggregateSearchMetricRowsByUrl,
  evaluateTrafficDropCandidates,
} from "@/features/seo/analytics/traffic-drop.service";
import { inspectionSucceeded } from "@/features/seo/analytics/inspection-cap";
...
async function matchedSearchConsoleSiteUrl(): Promise<string | null> {
  const platform = await getGooglePlatformState().catch(() => null);
  return platform?.services.search_console?.connection?.matchedGscSiteUrl ?? null;
}
...
async function fetchGoogleSearchMetrics(config: SeoIntegrationProviderConfig, days: number) {
  ...
  const gscSiteUrl = await resolveConfiguredGscSiteUrl(
    config,
    token,
    await matchedSearchConsoleSiteUrl(),
  );
...
async function enqueueTrafficDropInspections(
  currentRows: SeoSearchMetricInput[],
  days: number,
): Promise<void> {
  const priorUntil = dateOnly(days);
  const priorSince = dateOnly(days * 2);
  const priorStored = await seoRepository.listSearchMetricTotalsByUrl({
    source: "google",
    since: priorSince,
    until: priorUntil,
  });
  if (priorStored.length === 0) return;

  const current = aggregateSearchMetricRowsByUrl(currentRows);
  const prior = aggregateSearchMetricRowsByUrl(priorStored);
  const { enqueueUrlInspection } = await import("@/features/seo/analytics/url-inspection-queue");
  for (const candidate of evaluateTrafficDropCandidates(prior, current)) {
    await enqueueUrlInspection(candidate.url, "traffic_drop");
  }
}
...
async function importGoogleRichResults(
  ...
  const { selectDeltaInspectionUrls, markUrlsInspected, urlInspectionSpacingMs } = await import(
    "@/features/seo/analytics/url-inspection-queue"
  );
  const gscSiteUrl = await resolveConfiguredGscSiteUrl(
    config,
    token,
  ...
  const candidateUrls = [...new Set(rows.map((row) => row.url).filter(Boolean))];
  const urls = await selectDeltaInspectionUrls(candidateUrls);
  if (urls.length === 0) return 0;
  const issues: Parameters<typeof seoRepository.replaceRichResultIssues>[0] = [];
  const successfulUrls: string[] = [];
  const spacingMs = urlInspectionSpacingMs();
  for (const [index, url] of urls.entries()) {
    ...
    if (!inspectionSucceeded(response)) continue;
  ...
  if (successfulUrls.length > 0) {
    await markUrlsInspected(successfulUrls, "traffic_drop");
  }
    ...
    if (config.google?.analyticsEnabled) {
      ...
      if (config.google.siteUrl?.trim()) {
        try {
          ...
          await enqueueTrafficDropInspections(gscRows, days);
