...
import type { SeoTrackingMode } from "./types";
...
} from "./tracking/resolve-tracking";
import {
  extractGtmContainerIdFromSnippet,
  extractMeasurementIdFromSnippet,
} from "./tracking/parse-tracking-snippets";
...
export async function upsertSeoTrackingAction(formData: FormData) {
  ...
  const mode: SeoTrackingMode = modeRaw === "gtm" ? "gtm" : "gtag";

  const hasPerServiceFlags =
    formData.has("gtagEnabled") || formData.has("gtmEnabled");
  const legacyEnabled = formData.get("enabled") === "true";

  let gtagEnabled: boolean;
  let gtmEnabled: boolean;

  if (hasPerServiceFlags) {
    gtagEnabled = formData.get("gtagEnabled") === "true";
    gtmEnabled = formData.get("gtmEnabled") === "true";
  } else {
  ...
  const measurementIdRaw =
    formData.has("measurementId")
      ? ((formData.get("measurementId") as string)?.trim() ?? "")
      : (existing.measurementId ?? "");
  const gtmContainerIdRaw =
    formData.has("gtmContainerId")
      ? ((formData.get("gtmContainerId") as string)?.trim() ?? "")
      : (existing.gtmContainerId ?? "");

  const gtagHeadSnippet =
    formData.has("gtagHeadSnippet")
      ? ((formData.get("gtagHeadSnippet") as string)?.trim() ?? "")
      : (existing.gtagHeadSnippet ?? "");
  const gtmHeadSnippet =
    formData.has("gtmHeadSnippet")
      ? ((formData.get("gtmHeadSnippet") as string)?.trim() ?? "")
      : (existing.gtmHeadSnippet ?? "");
  const gtmBodySnippet =
    formData.has("gtmBodySnippet")
      ? ((formData.get("gtmBodySnippet") as string)?.trim() ?? "")
...
export async function runSeoAnalyticsIngestionAction(): Promise<SeoActionResult> {
  await requireAdmin();
  const result = await seoAnalyticsIngestionService.run(7);
  revalidateIntegrationsPaths();
  if (result.processed === 0) {
    return {
      ok: false,
      message:
        "No analytics providers ran. Enable Google or Bing analytics ingestion on the Configure tab.",
      processed: 0,
    };
  }
  ...
  if (failures.length > 0) {
    return {
      ok: false,
      message: `Imported ${result.imported} metrics with errors: ${failures.map((item) => item.error).join("; ")}`,
      processed: result.imported,
    };
  }
  return {
    ok: true,
    message: `Imported ${result.imported} search metrics from ${result.processed} provider(s).`,
    processed: result.imported,
  };
}
