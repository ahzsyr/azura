...
import { CACHE_TAGS, revalidateSeoDocument, revalidateSeoMeta, revalidateSitemap } from "@/services/cache";
...
import type { SeoIntegrationsConfig, SeoAppearanceConfig } from "./types";
...
export async function upsertSeoAppearanceAction(
  config: SeoAppearanceConfig,
): Promise<{ ok: true } | { ok: false; message: string }> {
  try {
    await requireAdmin();
    await seoRepository.upsertAppearanceConfig(config);
    revalidateSeoDocument("seo-document");
    await enqueueSitemapSubmission("metadata");
    revalidatePath("/admin/seo/templates");
    revalidateSitemap();
    return { ok: true };
  } catch (error) {
    console.error("[seo-appearance] save failed:", error);
...
export async function upsertStructuredDataAction(formData: FormData) {
  ...
  const { buildConfigFromFormData, StructuredDataSaveError } = await import(
    "@/features/seo/platform/schema-pipeline/save-validation"
  );
  const { prepareStructuredConfigForSave } = await import(
    "@/features/seo/platform/schema-pipeline/structured-config"
  );

  let config;
  try {
    config = prepareStructuredConfigForSave(buildConfigFromFormData(formData));
  } catch (error) {
    if (error instanceof StructuredDataSaveError) {
      throw error;
    }
    throw error;
  }
