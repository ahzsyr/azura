...
import type { SeoTemplateType } from "@/features/seo/core/seo-templates";
import type { SeoHttpStatus } from "@/features/seo/core/seo-document";
...
export type SeoResolveInput = {
  ...
  /** HTTP status override for error-page SEO (404, etc.). */
...
export type SeoAppearanceTemplateEntry = {
  titleTemplate?: string;
  descriptionTemplate?: string;
};

/** Admin Search Appearance patterns (JsonStore `seo-appearance`). */
export type SeoAppearanceConfig = Partial<Record<SeoTemplateType, SeoAppearanceTemplateEntry>>;
...
export type SeoStructuredConfig = {
  ...
  entityType?: string;
  /** When true, Business Profile must not overwrite entityType. */
  entityTypePinned?: boolean;
