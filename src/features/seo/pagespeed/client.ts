...
import {
  parseCruxFieldMetrics,
  type LoadingExperience,
  type PageSpeedDataSource,
} from "./crux-metrics";

export type { PageSpeedDataSource } from "./crux-metrics";
export { CRUX_THRESHOLDS, parseCruxFieldMetrics } from "./crux-metrics";
...
export async function runPageSpeedInsights(input: {
  ...
  const crux = parseCruxFieldMetrics(body.loadingExperience ?? body.originLoadingExperience);
  const labLcp = metricMs(audits, "largest-contentful-paint");
  const labCls =
    typeof audits["cumulative-layout-shift"]?.numericValue === "number"
      ? Number(audits["cumulative-layout-shift"].numericValue.toFixed(3))
      : null;
  const labInp = metricMs(audits, "interaction-to-next-paint") ?? metricMs(audits, "max-potential-fid");
  ...
  return {
    ...
    lcpMs: crux.dataSource === "crux" ? crux.lcpMs ?? labLcp : labLcp,
    cls: crux.dataSource === "crux" ? crux.cls ?? labCls : labCls,
    inpMs: crux.dataSource === "crux" ? crux.inpMs ?? labInp : labInp,
    fieldLcpMs: crux.lcpMs,
    fieldInpMs: crux.inpMs,
    fieldCls: crux.cls,
    dataSource: crux.dataSource,
    cwvWarnings: crux.warnings,
    opportunities,
    live: true,
