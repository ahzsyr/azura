...
} from "@/features/seo/admin/google-gsc-site-url";
...
export type GoogleIntegrationVerification = {
  ...
  propertyKind?: "domain" | "url_prefix";
  gscError?: string;
...
export async function resolveConfiguredGscSiteUrl(
  ...
  const preferred = resolvePreferredGscSiteUrl({
    matchedGscSiteUrl,
    configuredUrl: null,
  });
...
async function verifyGoogleIntegrationAccessInner(
  ...
  const result: GoogleIntegrationVerification = {
    ...
    matchedGscSiteUrl,
    propertyKind: gscPropertyKind(matchedGscSiteUrl),
  };
