...
import { stripDefaultLocalePublicPrefix } from "@/i18n/url-helpers";
...
function normalizeIndexNowPageUrl(pageUrl: string): string {
  const wired = normalizeWiredCmsAbsoluteUrl(pageUrl);
  try {
    const parsed = new URL(wired);
    parsed.pathname = stripDefaultLocalePublicPrefix(parsed.pathname);
    const normalized = `${parsed.origin}${parsed.pathname}`;
    return parsed.pathname === "/" ? `${parsed.origin}/` : normalized.replace(/\/$/, "");
  } catch {
    return wired;
  }
}
...
export function buildIndexNowBatchPayload(
  ...
  const normalizedUrls = pageUrls
    .map((pageUrl) => normalizeIndexNowPageUrl(pageUrl))
    .filter(Boolean);
