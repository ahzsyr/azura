...
export function shouldEnqueueProviderJob(
  ...
  // Indexing API is not a generic URL ping — eligibility is enforced in publishUrlToIndexingApi.
  if (kind === "URL" && providerId === "google_indexing") return false;
...
export function skippedProviderJobMessage(
  ...
  if (kind === "URL" && providerId === "google_indexing") {
    return "Skipped: Google Indexing API is limited to JobPosting and BroadcastEvent pages";
  }
