...
export const googleProvider: SeoIntegrationProvider = {
  ...
  async submitSitemap(config, input) {
    ...
    try {
      const { getGooglePlatformState } = await import(
        "@/features/seo/google-platform/persistence"
      );
      const platform = await getGooglePlatformState().catch(() => null);
      const matchedGscSiteUrl =
        platform?.services.search_console?.connection?.matchedGscSiteUrl ?? null;
      token = (await refreshGoogleToken(config))?.trim() ?? token;
      if (token) {
        siteUrl = await resolveConfiguredGscSiteUrl(config, token, matchedGscSiteUrl);
      } else if (matchedGscSiteUrl?.trim()) {
        siteUrl = normalizeGscSiteUrl(matchedGscSiteUrl);
      }
    } catch {
