...
describe("getCmsPageSeoPageKey", () => {
  it("returns pageKey for wired marketing slugs", () => {
    ...
    assert.equal(getCmsPageSeoPageKey("faqs"), "faqs");
  });
...
describe("isSavedSeoTranslation", () => {
  ...
  it("returns true for focusKeywords and jsonLd locale keys", () => {
    assert.equal(
      isSavedSeoTranslation({ focusKeywordsAr: "شبكات" }, "focusKeywords", "ar"),
      true,
    );
    assert.equal(
      isSavedSeoTranslation(
        { jsonLdEn: '{"@type":"WebPage"}' },
        "jsonLd",
        "en",
      ),
      true,
    ...
    assert.equal(
      isSavedSeoTranslation(
        { canonicalUrlAr: "https://example.com/ar" },
        "canonicalUrl",
        "ar",
      ),
      true,
...
describe("coalesceTranslationRecords with focusKeywords and jsonLd", () => {
  it("merges localized focusKeywords and jsonLd with pageKey winning", () => {
    const merged = coalesceTranslationRecords(
      {
        focusKeywordsEn: "pageKey keywords",
        jsonLdEn: '{"name":"pageKey"}',
        canonicalUrlEn: "https://example.com/en",
      },
      {
        focusKeywordsEn: "cms keywords",
        focusKeywordsAr: "كلمات cms",
        jsonLdAr: '{"name":"cms"}',
        canonicalUrlAr: "https://example.com/ar",
      },
    );

    assert.equal(merged.focusKeywordsEn, "pageKey keywords");
    assert.equal(merged.focusKeywordsAr, "كلمات cms");
    assert.equal(merged.jsonLdEn, '{"name":"pageKey"}');
    assert.equal(merged.jsonLdAr, '{"name":"cms"}');
    assert.equal(merged.canonicalUrlEn, "https://example.com/en");
    assert.equal(merged.canonicalUrlAr, "https://example.com/ar");
  });
});
