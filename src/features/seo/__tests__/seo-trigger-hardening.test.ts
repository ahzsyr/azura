...
describe("SEO trigger hardening", () => {
  it("queues URL and sitemap jobs for published content", async (t) => {
    ...
    assert.deepEqual(
      ...
      [
        ["indexnow", "URL", "publish", "http://localhost:3000/en/about"],
      ],
    );
  });

  it("normalizes legacy /pages/home paths when enqueueing", async (t) => {
    const { seoRepository, seoTriggerService } = await loadSeoModules();
    const batches: Array<Parameters<typeof seoRepository.enqueueSubmissionJobs>[0]> = [];
    t.mock.method(seoRepository, "getIntegrationsConfig", async () => ({
      indexnow: { enabled: true, apiKey: "test-key" },
    ...
    const jobs = batches.flat();
    assert.equal(jobs.length, 1);
    assert.equal(jobs[0]?.url, "http://localhost:3000");
  });
  ...
  it("queues old URL, new URL, and sitemap jobs for slug changes", async (t) => {
    ...
    t.mock.method(seoRepository, "getIntegrationsConfig", async () => ({
      indexnow: { enabled: true, apiKey: "test-key" },
      bing: { enabled: true, apiKey: "bing-key", siteUrl: "https://example.com" },
    }));
    t.mock.method(seoRepository, "enqueueSubmissionJobs", async (jobs) => {
      batches.push(jobs);
    });
    ...
    const jobs = batches.flat();
    const byProvider = {
      indexnow: jobs.filter((j) => j.provider === "indexnow"),
      bing: jobs.filter((j) => j.provider === "bing"),
    };
    assert.deepEqual(
  ...
  it("queues URL and sitemap jobs for metadata updates", async (t) => {
    const { seoRepository, seoTriggerService } = await loadSeoModules();
    const batches: Array<Parameters<typeof seoRepository.enqueueSubmissionJobs>[0]> = [];
    t.mock.method(seoRepository, "getIntegrationsConfig", async () => ({
      indexnow: { enabled: true, apiKey: "test-key" },
    ...
    await seoTriggerService.handle({
      ...
      entityType: "SITE",
      entityId: "home",
      paths: ["/en/about", "/ar/about"],
    });

    const jobs = batches.flat();
    assert.ok(jobs.some((job) => job.kind === "URL"));
