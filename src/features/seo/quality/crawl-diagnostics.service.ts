...
export const crawlDiagnosticsService = {
  async analyze(limit = 50): Promise<SeoQualityIssue[]> {
    ...
    if (notFoundUrls.length > 0) {
      try {
        const { enqueueUrlInspection } = await import(
          "@/features/seo/analytics/url-inspection-queue"
        );
        for (const url of notFoundUrls.slice(0, 20)) {
          await enqueueUrlInspection(url, "not_found");
        }
      } catch {
        // Inspection enqueue must not block crawl diagnostics.
      }
    }
