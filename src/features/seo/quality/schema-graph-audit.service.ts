import { satisfiesOrganizationFamily } from "@/features/seo/platform/schema-pipeline/type-model";
...
function findOrganizationId(nodes: SchemaNode[]): string | undefined {
  for (const node of nodes) {
    if (satisfiesOrganizationFamily(node["@type"])) return nodeId(node);
  }
  return undefined;
}
...
function validateRelationships(
  ...
  const orgNodes = nodes.filter((n) => satisfiesOrganizationFamily(n["@type"]));
...
function pageExpectedTypes(ctx: SchemaContext): string[] {
  const base = ["Brand", "WebSite", "WebPage", "ImageObject"];
  const entityType = ctx.site.structuredConfig.entityType ?? "Organization";
  base.unshift(entityType);
...
export function auditSchemaGraph(
  ...
  const orgNode = nodes.find((n) => satisfiesOrganizationFamily(n["@type"]));
...
export function eligibleSchemaTypesForPage(ctx: SchemaContext): string[] {
  const entityType = ctx.site.structuredConfig.entityType ?? "Organization";
  const types: string[] = [entityType, "Brand", "WebSite", "WebPage"];
