  ...
  for (const [index, item] of objects.entries()) {
    ...
    if (type === "Product" && !item.name) {
      const productSuggestion = isProductPageKey
        ? "Remove incomplete custom JSON-LD or add name. The product schema pipeline generates valid Product JSON-LD automatically when the override is cleared."
        : "Product JSON-LD should include name.";
      issues.push(
        withResolvedFix({
    ...
    if (type === "Product" && !item.offers) {
      issues.push(
        withResolvedFix({
          id: `jsonld-product-quote-${sourceLabel}-${index}`,
          title: "Product without offers",
          severity: "info",
          message:
            "Quote-only products may omit offers. Merchant listing requires genuine offer data.",
          source: publicPath ?? sourceLabel,
          href,
        }),
      );
