...
function inspectJsonLd(source: string, url: string, value: unknown) {
  ...
  for (const [index, item] of asObjects(value).entries()) {
    ...
    if (type === "Product") {
      ...
      if (!offers) {
        issues.push({
          issueKey: `${issuePrefix}-quote-only-product`,
          type,
          category: "WARNING",
          url,
          ...
          details: {
            message:
              "Product structured data without offers is valid for quote-only products. Merchant listing requires genuine offer data.",
          },
        });
    ...
    if (type === "Organization") {
      if (!item.name) {
        issues.push({
          ...
          url,
          eligibility: "NOT_ELIGIBLE",
          details: { message: "Organization schema requires name." },
        });
      }
      if (!item.logo) {
        issues.push({
          issueKey: `${issuePrefix}-missing-logo`,
          type,
          category: "WARNING",
          url,
          eligibility: "ELIGIBLE_WITH_WARNINGS",
          details: { message: "Organization schema should include logo ImageObject reference." },
        });
      }
      if (!item.sameAs || (Array.isArray(item.sameAs) && item.sameAs.length === 0)) {
        issues.push({
          issueKey: `${issuePrefix}-missing-same-as`,
          type,
          category: "WARNING",
          url,
          eligibility: "ELIGIBLE_WITH_WARNINGS",
          details: { message: "Organization schema should include sameAs social profile URLs." },
        });
      }
    }

    if (type === "WebSite" && !item.url) {
      issues.push({
        issueKey: `${issuePrefix}-missing-url`,
        type,
        category: "ERROR",
        url,
        eligibility: "NOT_ELIGIBLE",
        details: { message: "WebSite schema requires url." },
      });
    }

    if (type === "BreadcrumbList" && !item.itemListElement) {
      issues.push({
        issueKey: `${issuePrefix}-missing-breadcrumb-items`,
        type,
        category: "WARNING",
        url,
        eligibility: "ELIGIBLE_WITH_WARNINGS",
        details: { message: "BreadcrumbList should include itemListElement." },
      });
    }

    if (type === "ImageObject" && !item.url) {
      issues.push({
        issueKey: `${issuePrefix}-missing-image-url`,
        type,
        category: "ERROR",
        url,
        eligibility: "NOT_ELIGIBLE",
        details: { message: "ImageObject requires url." },
      });
    }
