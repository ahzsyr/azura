...
const adsLandingAnalyzer: ContentAnalyzer = {
  id: "ads-landing-relevance",
  async analyze(ctx, draft: ContentSnapshotDraft) {
    const meta = { ...(draft.metadata ?? {}) };
    let campaignName = typeof meta.adsCampaignName === "string" ? meta.adsCampaignName : "";
    let adGroupName = typeof meta.adsAdGroupName === "string" ? meta.adsAdGroupName : "";
    const pathHint =
      (typeof ctx.metadata?.publicPath === "string" && ctx.metadata.publicPath.trim()) ||
      (typeof ctx.metadata?.path === "string" && ctx.metadata.path.trim()) ||
      "";

    if (!campaignName && pathHint) {
      try {
        const { prisma } = await import("@/lib/prisma");
        const campaigns = await prisma.marketingCampaign.findMany({
          where: {
            landingPagePath: { contains: pathHint },
          },
          select: { name: true },
          take: 8,
        });
        campaignName = campaigns.map((campaign) => campaign.name).join(" ");
      } catch {
        // Analyzer must stay best-effort.
      }
    }

    const { adsLandingRelevanceScore } = await import("./analyzers/ads-landing-relevance");
    const h1 = draft.headings.find((heading) => heading.level === 1)?.text ?? "";
    return {
      metadata: {
        ...meta,
        adsLandingRelevance: adsLandingRelevanceScore({
          campaignName,
          adGroupName,
          title: draft.title,
          h1,
...
export const defaultTemplates = [
  ...
  },
  {
    ...
    pattern: "{Title} — {SiteName}. {Tagline}",
  },
  {
    id: "product-title",
...
export function registerPlatformDefaults(): void {
  ...
  pluginSdk.registerAnalyzer(adsLandingAnalyzer);
