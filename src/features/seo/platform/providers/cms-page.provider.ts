...
async function buildCmsLikeSnapshot(
  ...
  const postSlug =
    entityType === "Post" && row && "slug" in row
      ? (row.slug as string | undefined)?.trim()
      : undefined;
  if (postSlug) {
    draft = {
      ...draft,
  ...
  const publishedAt = row && "publishedAt" in row ? (row.publishedAt as Date | null) : null;
  const authorName = row && "author" in row && row.author ? (row.author as { name: string }).name : null;
