...
describe("post canonical URL normalization", () => {
  it("fills unprefixed English canonical from publicPath metadata", () => {
    const ctx = createExecutionContext({
      entityType: "Post",
    ...
    assert.equal(normalized.canonicalUrl, "https://brt-me.com/blog/wireless-tips");
  });
  ...
  it("rewrites localhost absolute canonicals to the public site origin", () => {
    ...
    assert.equal(normalized.canonicalUrl, "https://brt-me.com/blog/my-post");
  });
});
