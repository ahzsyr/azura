...
function pathFromRoutingKey(routingKey: string): string | undefined {
  ...
  if (prefix === "collection" || prefix === "category") return `/categories/${slug}`;
...
function inferPublicPath(
  ...
  const fromSnapshot =
    typeof snapshot?.metadata?.publicPath === "string"
      ? snapshot.metadata.publicPath.trim()
      : "";
  if (fromSnapshot) {
    return fromSnapshot.startsWith("/") ? fromSnapshot : `/${fromSnapshot}`;
  }
  ...
  if (normalizedType.includes("collection") || normalizedType.includes("category")) return `/categories/${entityId}`;
  if (normalizedType.includes("category")) return `/categories/${entityId}`;
  if (normalizedType.includes("tag")) return `/tags/${entityId}`;
  // Post / CmsPage entityId is a UUID — never use it as a path segment.
  return undefined;
}

function withLocalePrefix(path: string, locale?: string): string {
  const clean = path.startsWith("/") ? path : `/${path}`;
  const prefix = (locale ?? "").trim();
  if (!prefix) return clean;
  return publicLocalePath(prefix, clean);
}
...
function normalizeCanonicalUrl(
  ...
  if (/^https?:\/\//i.test(value)) {
    try {
      ...
      const pathname = stripDefaultLocalePublicPrefix(url.pathname, defaultPrefix);
      if (url.hostname === "localhost" || url.hostname === "127.0.0.1") {
        const site = new URL(siteUrl);
        return `${site.origin}${pathname}${url.search}${url.hash}`;
      }
  ...
  const path = withLocalePrefix(value.startsWith("/") ? value : `/${value}`, locale);
...
export function normalizeSeoSuggestionWithContext(
  ...
  const inferredCanonicalPath = inferPublicPath(ctx, snapshot);
