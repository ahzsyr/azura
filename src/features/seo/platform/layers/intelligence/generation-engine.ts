...
import { buildMetaDescriptionFromSnapshot, extractPageText } from "../content/page-text";
...
import {
  ...
  enrichMetaTitle,
} from "./meta-enrichment";

function extractKeywords(snapshot: ContentSnapshot): string {
  const fromBody = extractPageText(snapshot);
  const fromTitle = snapshot.title ?? "";
  const words = `${fromTitle} ${fromBody}`
    .toLowerCase()
    .split(/\W+/)
    .filter((w) => w.length >= 4);
...
export async function generateRuleBasedSuggestion(
  ...
  let metaDescription = buildMetaDescriptionFromSnapshot(snapshot);
  if (!metaDescription) {
    metaDescription =
      (descTemplate?.pattern
        ? templates.find((t) => t.id === descTemplate.id)?.resolved
        : undefined) || `${snapshot.title} — ${brand}`;
  }

  metaTitle = enrichMetaTitle(metaTitle, snapshot, brand, tagline);
  metaDescription = enrichMetaDescription(metaDescription, snapshot, brand, tagline);
  const focusKeywords = enrichFocusKeywords(extractKeywords(snapshot), snapshot, brand);
