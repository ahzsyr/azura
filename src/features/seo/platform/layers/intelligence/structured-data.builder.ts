...
function entitySchemaType(ctx: SeoExecutionContext, snapshot: ContentSnapshot): string {
  const kind = ctx.entityType.toLowerCase();
  if (kind.includes("product")) return "Product";
  if (kind.includes("post")) return "Article";
  if (kind === "content_item" && snapshot.metadata?.authorName) return "Article";
  if (kind === "cms_page" && snapshot.metadata?.authorName) return "Article";
...
export function buildStructuredDataObject(input: {
  ...
  const type = entitySchemaType(input.ctx, input.snapshot);
  ...
  if (type === "Article") {
    ...
    const publishedAt = input.snapshot.metadata?.publishedAt;
    if (publishedAt && typeof publishedAt === "string") {
      base.datePublished = publishedAt;
    }
    const authorName = input.snapshot.metadata?.authorName;
