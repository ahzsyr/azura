...
import { resolveSeoTriggerPaths } from "@/features/seo/triggers/resolve-seo-trigger-paths";
import type { SeoEntityType } from "@/features/seo/triggers/events";
...
export async function runPublishing(
  ...
  if (ctx.trigger === "page_save" || ctx.source === "publish") {
    await seoTriggerService.handle({
      ...
      entityType: ctx.entityType as SeoEntityType,
      entityId,
      paths: await resolveSeoTriggerPaths({
        entityType: ctx.entityType as SeoEntityType,
        entityId,
        locale: ctx.locale,
      }),
...
export async function commitChangeSet(
  ...
  await seoTriggerService.handle({
    type: "seo.metadataUpdated",
    entityType: persistCtx.entityType as SeoEntityType,
    entityId: persistCtx.entityId,
    paths: await resolveSeoTriggerPaths({
      entityType: persistCtx.entityType,
      entityId: persistCtx.entityId,
      locale: persistCtx.locale,
      pageKey: wt.pageKey,
    }),
  });
