...
test("home graph includes canonical business entity, WebSite, WebPage, Brand", () => {
...
test("product graph includes Offer node linked by @id", () => {
  const result = SchemaPipeline.build(createProductContextFixture());
  const product = result.graph["@graph"].find((n) => n["@type"] === "Product");
  const offer = result.graph["@graph"].find((n) => n["@type"] === "Offer");
  assert.equal(offer?.["@id"], "https://example.com/products/industrial-router/#offer");
  assert.equal((product?.offers as { "@id"?: string })?.["@id"], offer?.["@id"]);
...
test("default faqBuilder excludes FAQPage on CMS pages with FAQ blocks", () => {
  const result = SchemaPipeline.build(createCmsFaqBlocksContextFixture());
  const faqPages = result.graph["@graph"].filter((node) => node["@type"] === "FAQPage");
