...
export type GoogleConnectionSnapshot = {
  ...
  message?: string;
  /** Search Console property string actually used for API calls. */
...
export type BusinessProfileDiscoverySnapshot = {
...
export type GoogleIntegrationContext = {
  ...
  /**
   * Marketing module owns the Google Ads OAuth + customer connection.
   * SEO Ads integration reads this instead of a second credential store.
   */
  marketingGoogleAds?: {
    /** True when Ads is fully operational (SEO OAuth + token + MCC + customer). */
    connected: boolean;
