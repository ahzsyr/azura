...
export async function buildOperationalCards(
  ...
  for (const def of googleIntegrationRegistry.list()) {
    ...
    cards.push({
      ...
      connected:
        def.id === "ads"
          ? connection.state === "connected" || Boolean(ctx.marketingGoogleAds?.operational)
          : connection.state === "connected" || def.isConfigured(ctx),
...
export function buildContext(input: {
  ...
  marketingGoogleAds?: GoogleIntegrationContext["marketingGoogleAds"];
}): GoogleIntegrationContext {
  return {
    ...
    env: input.env,
    marketingGoogleAds: input.marketingGoogleAds,
  };
}
