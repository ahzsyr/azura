...
export async function upsertGoogleServiceConfigAction(
  ...
  try {
    ...
    if (integrationId === "ads") {
      const { parseGoogleAdsCustomerId } = await import(
        "@/modules/marketing/providers/google-ads/sdk/api"
      );
      for (const key of ["customerId", "managerAccountId"] as const) {
        const raw = configuration[key];
        if (typeof raw !== "string" || !raw.trim()) continue;
        const parsed = parseGoogleAdsCustomerId(raw);
        if (!parsed) {
          return {
            ok: false,
            message: `${key === "managerAccountId" ? "Manager Account (MCC)" : "Customer ID"} must be a numeric Google Ads ID (hyphens allowed). Emails and names are not valid.`,
          };
        }
...
export async function testGoogleIntegrationAction(
  ...
  try {
    ...
    if (integrationId === "business_profile") {
      const { bootstrapBusinessProfileDiscoveryIfNeeded } = await import(
        "@/features/seo/google-live/business-profile"
      );
      await bootstrapBusinessProfileDiscoveryIfNeeded({ dryRun });
    ...
    let verifiedOk = result.ok;
    if (integrationId === "business_profile" && !verifiedOk) {
      const { isBusinessProfileRateLimitError } = await import(
        "@/features/seo/google-live/business-profile-cache"
      );
      if (isBusinessProfileRateLimitError(result.message)) {
        verifiedOk = true;
      }
    }
    manager.markVerified(integrationId, verifiedOk, result.message);
    if (integrationId === "search_console") {
      try {
        const google = ctx.legacyIntegrations?.google;
        if (google) {
          const { verifyGoogleIntegrationAccess } = await import(
            "@/features/seo/integrations/google-verify"
          );
          const verification = await verifyGoogleIntegrationAccess(google);
        ...
        // Property detection must not block Test Connection.
      }
    }
