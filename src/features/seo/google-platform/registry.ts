...
export function createGoogleIntegrationRegistry(
  ...
  async function monitoringFor(
    ...
    let errors = stored?.errors ?? (health.authentication === "error" ? 1 : 0);
    let warnings = stored?.warnings ?? 0;
    if (id === "business_profile") {
      const {
        isBusinessProfileRateLimited,
        isBusinessProfileRateLimitError,
        readBusinessProfileDiscovery,
      } = await import("@/features/seo/google-live/business-profile-cache");
      ...
      } else if (!discovery.lastError || isBusinessProfileRateLimitError(discovery.lastError)) {
        // Cleared cooldown / stale rate-limit lastError must not keep a red sidebar.
        errors = 0;
        if (!discovery.lastError) warnings = 0;
      }
    }
    return {
      health,
