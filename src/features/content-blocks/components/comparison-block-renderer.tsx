...
import { ManualComparisonView } from "@/features/content-blocks/components/manual-comparison-view";
import { ManualComparisonOverflow } from "@/features/content-blocks/components/manual-comparison-overflow";
import { ComparisonCatalogOverflow } from "@/features/content-blocks/components/comparison-catalog-overflow";
...
export async function ComparisonBlockRenderer({
  ...
  if (source === "contentType" && contentTypeSlug) {
    ...
    items = await fetchCatalogSnapshots(catalogSource, itemIds);
    if (items.length >= 2) {
      const keys =
  ...
  const specLabel = title ?? (locale.startsWith("ar") ? "المقارنة" : "Comparison");
