...
test("non-API merge caches durable complete status for a long TTL", () => {
  const merged = mergeSetupStatusWithEnvOverrides({
    setupComplete: true,
    registrationEnabled: true,
    comingSoonEnabled: false,
...
test("non-API merge keeps short TTL when coming soon is env-enabled", () => {
  process.env.COMING_SOON_ENABLED = "true";
...
test("setupStatusFromCookieFallback is confident setup-complete without forcing coming soon", () => {
