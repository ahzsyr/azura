...
export const contentTypeLandingSearchProvider = defineSearchProvider<ContentTypeIndexSource>({
  ...
  buildRecords(type, ctx) {
    ...
    const title = getLocalizedField(
      { titleEn: type.labelPluralEn, titleAr: type.labelPluralAr },
      "title",
      ctx.urlPrefix
    );
    return [
      {
        ...
        title,
        body: [type.nameEn, type.nameAr, type.slug].join(" "),
...
export const contentCollectionSearchProvider = defineSearchProvider<ContentCollectionIndexSource>({
  ...
  buildRecords(col, ctx) {
    ...
    const excerpt =
      ...
      getLocalizedField(col, "name", ctx.urlPrefix);
    return [
      {
        entityType: "CONTENT_COLLECTION",
        entityId: col.id,
        locale: ctx.urlPrefix,
        title: getLocalizedField(col, "name", ctx.urlPrefix),
