    ...
    return [
      {
        ...
        entityId: faq.id,
        locale: ctx.urlPrefix,
        title: getLocalizedField(faq, "question", ctx.urlPrefix),
        body: getLocalizedField(faq, "answer", ctx.urlPrefix),
    ...
    return [
      {
        ...
        locale: ctx.urlPrefix,
        title: t.name,
        body: getLocalizedField(t, "content", ctx.urlPrefix),
...
export const mediaSearchProvider = defineSearchProvider({
  ...
  defaultBoost: 0.5,
  shouldIndex: () => true,
  buildRecords(asset: { id: string; filename: string; altEn: string; altAr: string }, ctx) {
    return [
      {
        entityType: "MEDIA",
        entityId: asset.id,
        ...
        body: getLocalizedField(asset, "alt", ctx.urlPrefix),
        urlPath: "/admin/media",
        kind: "media",
        visibility: "admin",
        boost: 0.5,
