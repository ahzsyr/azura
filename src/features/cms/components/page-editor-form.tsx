...
type PageWithRevisions = CmsPage & { revisions?: CmsPageRevision[]; seoMeta?: SeoMeta | null; author?: PostAuthor | null };
...
function PageEditorFields({
  ...
  const seoPanelMounted = activeTab === "seo" && Boolean(page?.id && seoFormProps);
...
export function PageEditorForm({
  ...
  const handleCancel = useCallback(() => {
    ...
    const restored = structuredClone(baselineFormStateRef.current);
    formStateRef.current = restored;
    blocksRef.current = restored.composition.regions[selectedRegion] ?? [];
    setFormState(restored);
    router.refresh();
  }, [page, router, selectedRegion]);
  ...
  const shouldSuppressPageActions = isSeoTab && Boolean(page?.id && seoFormProps);

  const handleLegacyPropUpdate = useCallback(
    (blockId: string, field: string, localeCode: string, value: string) => {
      const suffix = getContentFieldSuffix(localeCode);
      // Dual-write locale keys so clears persist on block.props and beat stale
      // EntityTranslation rows on the live site (see isExplicitLocaleFieldClear).
      const patch: Record<string, unknown> =
        suffix === "En"
          ? { [field]: value, [`${field}${suffix}`]: value }
          : suffix === "Ar"
            ? { [`${field}${suffix}`]: value }
            : { [`${field}_${localeCode.toLowerCase()}`]: value };
      setFormState((prev) => {
        const next = {
          ...prev,
          composition: updateCompositionBlock(prev.composition, blockId, (block) =>
            patchBlockSettings(block, patch),
          ),
        };
        formStateRef.current = next;
  ...
  const editorTree = (
    <AdminFormProvider
      suppressPageActions={shouldSuppressPageActions}
      onSave={shouldSuppressPageActions ? undefined : handleSave}
      onCancel={handleCancel}
      onPublish={!shouldSuppressPageActions && page?.id ? handlePublish : undefined}
      onPreview={shouldSuppressPageActions ? undefined : handlePreview}
      canPublish={!shouldSuppressPageActions && Boolean(page?.id)}
      canCancel
      trackFormId="page-editor-form"
    >
      <PageEditorSaveGuard
      ...
      />
      <form ref={formRef} id="page-editor-form" action={upsertCmsPage} data-page-editor className="flex flex-col min-h-0 flex-1 w-full">
