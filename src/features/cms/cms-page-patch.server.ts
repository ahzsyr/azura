...
export async function patchCmsPageRecord(
  ...
  const planComparison = compareExecutionPlans(execution, shadowExecution);
  if (!planComparison.ok) {
    agentLog({
      location: "cms-page-patch.server.ts:shadowPlanComparison",
      message: "cms page patch/full execution plan mismatch",
      hypothesisId: "PATCH_SAVE",
      data: {
        pageId,
        mismatches: planComparison.mismatches,
        patchPaths: execution.changeSet.paths,
        shadowPaths: shadowExecution.changeSet.paths,
      },
    });
  }
