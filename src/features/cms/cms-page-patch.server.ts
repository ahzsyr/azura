...
export async function patchCmsPageRecord(
  ...
  // Allow translation-only saves (empty prop patch + blockTranslation clears).
  if (isEmptyPatch(changes) && !statusOverride && !hasBlockTranslationEdits(blockTranslationsRaw)) {
  ...
  const blocksChanged =
    pathChanged(appliedPaths, "blocks") || pathChanged(appliedPaths, "composition");
