...
async function upsertCmsPageCore(
  ...
  // #region agent log
  agentLog({
    location: "cms/actions.ts:upsertCmsPage:entry",
    message: "upsertCmsPage start",
    hypothesisId: "D",
    data: {
      pageId: id,
      blocksRawLength: blocksRaw?.length ?? 0,
      editorTab: formData.get("editorTab"),
      clientNavigation,
    },
  });
  // #endregion
  ...
  try {
    ...
    try {
      ...
      // #region agent log
      agentLogError("cms/actions.ts:upsertCmsPage:parseBlocks", parseErr, "D", { pageId: id });
      // #endregion
      throw parseErr;
    }
    ...
    try {
      ...
      // #region agent log
      agentLogError("cms/actions.ts:upsertCmsPage:validateBlocks", validateErr, "D", {
        pageId: id,
        parsedTypes: Array.isArray(blocksParsed)
          ? (blocksParsed as { type?: unknown }[]).map((b) => b.type)
          : [],
      });
      // #endregion
      throw validateErr;
    ...
    // #region agent log
    agentLog({
      location: "cms/actions.ts:upsertCmsPage:validated",
      message: "blocks validated",
      hypothesisId: "D",
      data: {
        pageId: id,
    ...
    if (id) {
      ...
      // #region agent log
      agentLog({
        location: "cms/actions.ts:upsertCmsPage:revisionSaved",
    ...
    // #region agent log
    agentLog({
      location: "cms/actions.ts:upsertCmsPage:cacheSynced",
      message: "cache synced",
      hypothesisId: "D",
      data: { pageId: page.id, status: page.status },
    });
    // #endregion
    ...
    // #region agent log
    agentLog({
      location: "cms/actions.ts:upsertCmsPage:entityTranslationsSynced",
      message: "entity translations synced",
      hypothesisId: "D",
      data: { pageId: page.id },
    });
    // #endregion
    ...
    // #region agent log
    agentLog({
      location: "cms/actions.ts:upsertCmsPage:blockTranslationsSynced",
      message: "block translations synced",
      hypothesisId: "D",
      data: { pageId: page.id, blockCount: blocks.length },
    });
    // #endregion
    ...
    const redirectTo = `/admin/pages/${page.id}?${qs.toString()}`;

    // #region agent log
    agentLog({
      location: "cms/actions.ts:upsertCmsPage:complete",
      message: "upsertCmsPage complete",
      hypothesisId: "D",
      data: { pageId: page.id, redirectTo, clientNavigation },
    });
    // #endregion
    ...
    // #region agent log
    agentLogError("cms/actions.ts:upsertCmsPage:failed", error, "D", { pageId: id });
    // #endregion
    throw error;
  }
}
