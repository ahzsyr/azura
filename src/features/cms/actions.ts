...
async function upsertCmsPageCore(
  ...
  try {
    ...
    if (id) {
      ...
      const updateData: Prisma.CmsPageUpdateInput = {
        ...sharedPageData,
        author: pageAuthorId ? { connect: { id: pageAuthorId } } : { disconnect: true },
      };
      page = await runCmsPageStep("updatePage", id, () => cmsRepository.updatePage(id, updateData));
      ...
      const createData: Prisma.CmsPageCreateInput = {
        ...sharedPageData,
        author: pageAuthorId ? { connect: { id: pageAuthorId } } : undefined,
      };
      page = await runCmsPageStep("createPage", null, () =>
        cmsRepository.createPage(createData),
      );
      incrementSavePipelineMetric(metrics, "dbWrites");
...
export async function patchCmsPageFromEditor(
  ...
  try {
    ...
    // Translation-only edits (e.g. clearing a Contact Form field that lives in
    // EntityTranslation while props were already empty) produce an empty patch.
    // Still sync blockTranslations so empty clears delete stale rows.
    if (
      isEmptyPatch(changes) &&
      !input.statusOverride &&
      !hasBlockTranslationEdits(input.blockTranslationsRaw)
    ) {
      const qs = buildEditorRedirectQuery({
        tab: input.editorTab,
...
async function upsertPostCore(formData: FormData, clientNavigation: boolean): Promise<string | void> {
  ...
  let featuredImageId = (formData.get("featuredImageId") as string | null) || null;
  const featuredImageUrl = String(formData.get("featuredImageUrl") ?? "").trim();
  if (!featuredImageId && featuredImageUrl) {
    const { ensureMediaAssetByUrl } = await import("@/features/media/ensure-media-asset-by-url");
    const asset = await ensureMediaAssetByUrl(featuredImageUrl);
