...
export async function patchCmsPageFromEditor(
  ...
  try {
    ...
    if (isEmptyPatch(changes) && !input.statusOverride) {
      const qs = new URLSearchParams({ tab: input.editorTab });
      if (input.selectedBlockId) qs.set("block", input.selectedBlockId);
      if (input.editorInspector) qs.set("inspector", input.editorInspector);
      return { ok: true, redirectTo: `/admin/pages/${input.pageId}?${qs.toString()}` };
    }
...
export async function patchPostFromEditor(input: PatchPostEditorInput): Promise<void> {
  ...
  if (isPostPatchShadowModeEnabled()) {
    agentLog({
      location: "cms/actions.ts:patchPostFromEditor:shadow",
      message: "post patch shadow comparison prepared",
...
export async function upsertPost(formData: FormData) {
  ...
  if (id) {
    ...
    if (existing && isPostPatchShadowModeEnabled()) {
      ...
      if (!comparison.ok) {
        agentLog({
          location: "cms/actions.ts:upsertPost:shadowPlanComparison",
          message: "post patch/full execution plan mismatch",
          hypothesisId: "PATCH_SAVE",
          data: { postId: id, mismatches: comparison.mismatches },
        });
      }
      ...
      agentLog({
        location: "cms/actions.ts:upsertPost:shadowPatch",
        message: "post legacy save shadow execution plan prepared",
        hypothesisId: "PATCH_SAVE",
        data: {
          postId: id,
          appliedPaths,
          effects: execution.effects.map((effect) => effect.id),
          asyncTasks: execution.asyncTasks.map((task) => task.id),
          reasoningTrace: execution.reasoningTrace,
        },
      });
    }
  ...
  if (!id || appliedPaths.some((p) => p === "blocks" || p.startsWith("blocks.")) || formData.has("blockTranslations")) {
  ...
  incrementSavePipelineMetric(metrics, "revalidationRuns");
  finishSavePipelineMetrics(metrics, { mode: "legacy-shadow", appliedPaths });
