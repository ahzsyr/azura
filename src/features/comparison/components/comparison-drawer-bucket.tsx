...
import { compareHubPath, comparePagePath } from "@/features/comparison/comparison-route-resolver";
...
export function ComparisonDrawerBucket({
  ...
  const listHref = publicLocalePath(locale, `/${listPrefix}`);
  const apiSegment = contentTypeSlug;
  const compareHref =
    totalBuckets > 1
      ? compareHubPath(locale, contentTypeSlug)
      : comparePagePath(locale, apiSegment);
