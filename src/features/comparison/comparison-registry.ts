...
async function listComparableContentTypesUncached(): Promise<RegisteredComparableType[]> {
  ...
  for (const row of rows) {
    ...
    out.push({
      id: row.id,
      slug: row.slug,
      labelPluralEn: localizedFieldValue(rowTranslations, "labelPlural"),
      labelPluralAr: resolveTranslation("labelPlural", "ar", ctx),
...
export async function getComparableContentTypeBySlug(
  ...
  const translations = await loadTranslationsMap("ContentType", [row.id]);
  const rowTranslations = translations.get(row.id) ?? [];
  const ctx = { translations: rowTranslations };
