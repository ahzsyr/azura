...
function defaultMenuLink(id: string, label: string, url: string): MenuItem {
  return {
    id,
    type: "link",
    label,
...
function buildDefaultMainMenuItems(): MenuItem[] {
  return [
    defaultMenuLink("nav-home", "Home", "/"),
    defaultMenuLink("nav-about", "About", "/about"),
    defaultMenuLink("nav-packages", "Packages", "/packages"),
    defaultMenuLink("nav-products", "Products", "/products"),
    defaultMenuLink("nav-services", "Services", "/services"),
    defaultMenuLink("nav-contact", "Contact", "/contact"),
  ];
}
...
/** Safe shell workspace for public fallback paths (intentionally no starter menu links). */
export function createEmptyWorkspace(): HeaderWorkspace {
...
export function newMenuItemFromForm(input: {
  ...
  if (input.type === "link") base.url = input.url ?? "#";
    ...
    base.brandSlug = input.brandSlug ?? "";
  } else if (input.type === "tag") {
    base.tagSlug = input.tagSlug ?? "";
