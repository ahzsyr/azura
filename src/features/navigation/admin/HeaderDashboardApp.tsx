        ...
        <HeaderBuilderShell activeSection={section} onSectionChange={changeSection}>
          {(active) => {
            switch (active) {
              ...
              case "menuEditor":
                return (
                  <MenuBuilder onSwitchToManager={() => changeSection("menuManager")} />
                );
              case "actions":
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!workspaceResult.ok) {
        ...
        // #region agent log
        fetch('http://127.0.0.1:7876/ingest/6e6c5bde-6579-4633-b8e0-f055b7efa2da',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'7ac260'},body:JSON.stringify({sessionId:'7ac260',runId:'ui-runtime-debug',hypothesisId:'H8',location:'src/features/navigation/admin/HeaderDashboardApp.tsx:134',message:'header dashboard workspace load failed',data:{unauthorized:Boolean(workspaceResult.unauthorized),notFound:Boolean(workspaceResult.notFound)},timestamp:Date.now()})}).catch(()=>{});
        // #endregion
        }
        setError(
