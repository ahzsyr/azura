...
import { BrandSelect, ProductSelect, TagSelect } from "./CatalogSelects";
import {
...
interface FormState {
  /** UI picker: page opens the content cascade. */
  pickerType: "link" | "page" | "collection" | "brand" | "tag" | "product" | "image";
  /** Persisted menu item type (may differ from pickerType when using content cascade). */
  type: MenuItemType;
  label: string;
  icon: string;
  badgeText: string;
...
function pickerTypeForItem(item: MenuItem | null): FormState["pickerType"] {
  if (!item) return "link";
  switch (item.type) {
    case "image":
      return "image";
    case "link":
      // Content-cascade link (offerings/listings/custom) still edits via Page cascade
...
/** Reset incompatible targets and seed defaults for the newly selected picker type. */
function formPatchForPickerType(
  pickerType: FormState["pickerType"],
...
export function MenuItemModal({
  ...
  useEffect(() => {
    ...
    // Omit catalog from deps so refreshCatalog does not wipe in-progress edits.
  ...
  const pickerType = form.pickerType;
              ...
              <section className="space-y-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Use <strong>Page</strong> to browse content sections and types (pages, blog,
                    catalog items, and more). Use <strong>Collection</strong> to drill parent →
                    child collections.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <HeaderField label="Item type" htmlFor="m-type">
                    <HeaderSelect
                      ...
                      value={pickerType}
                      onChange={(next) => {
                        const nextPicker = next as FormState["pickerType"];
                        setForm((prev) => ({
                          ...prev,
                          ...formPatchForPickerType(nextPicker, catalog, prev),
                        }));
                      }}
                    >
                    ...
                    <>
                      <CollectionSourceCascade
                        ...
                        onChange={(collectionId, label) =>
                          patch({ type: "collection", collectionId, label })
                        }
                      />
                      {mode !== "edit" ? (
                    ...
                    <>
                      <HeaderField label="Tag" htmlFor="m-tag">
                        <TagSelect
                          ...
                          value={form.tagSlug}
                          onChange={(tagSlug) =>
                            patch({ type: "tag", tagSlug, label: labelForTagSlug(catalog, tagSlug) })
                          }
                        />
                      </HeaderField>
                      {mode !== "edit" ? (
