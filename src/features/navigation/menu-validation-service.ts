...
function targetValue(item: MenuItem): string {
  ...
  if (item.type === "brand") return (item.brandSlug ?? "").trim();
  if (item.type === "tag") return (item.tagSlug ?? "").trim();
...
export function validateMenu(items: MenuItem[], catalog: HeaderBuilderCatalog): MenuHealthIssue[] {
  ...
  const tags = new Set(catalog.tags.map((t) => t.slug));
  const products = new Set(catalog.products.map((p) => p.slug));
  const packages = new Set((catalog.contentByType?.["catalog-items"] ?? []).map((p) => p.slug));
  ...
  for (const node of flat) {
    ...
    if (item.type === "brand" && target && !brands.has(target)) {
      issues.push({ key: `missing-brand-${item.id}`, severity: "warning", itemId: item.id, message: `Missing brand target "${target}".` });
    }
    if (item.type === "tag" && target && !tags.has(target)) {
      issues.push({ key: `missing-tag-${item.id}`, severity: "warning", itemId: item.id, message: `Missing tag target "${target}".` });
    }
    if (item.type === "product" && target && !products.has(target)) {
      issues.push({ key: `missing-product-${item.id}`, severity: "warning", itemId: item.id, message: `Missing product target "${target}".` });
    }
    if (item.type === "package" && target && !packages.has(target) && !products.has(target)) {
      issues.push({ key: `missing-package-${item.id}`, severity: "warning", itemId: item.id, message: `Missing package target "${target}".` });
    }
