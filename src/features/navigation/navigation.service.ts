...
export const navigationCatalogService = {
  async getCatalog(locale: string = "en"): Promise<HeaderBuilderCatalog> {
    ...
    const [cmsPages, contentTypes, allContentItems, posts] = await Promise.all([
    ...
    const [cmsTranslations, itemTranslations, postTranslations] = await Promise.all([
      loadTranslationsMap("CmsPage", cmsPages.map((p) => p.id)),
      loadTranslationsMap("ContentItem", contentItemIds),
      loadTranslationsMap("Post", posts.map((p) => p.id)),
    ]);

    const pickTitle = (
      entityType: "CmsPage" | "ContentItem" | "Post",
      id: string,
      slug: string,
      field: string,
    ) => {
    ...
    const contentByType: Record<string, { slug: string; name: string }[]> = {};
    for (const item of allContentItems) {
      const typeSlug = item.contentType.slug;
      if (!item.slug) continue;
      if (!contentByType[typeSlug]) contentByType[typeSlug] = [];
      contentByType[typeSlug].push({
        slug: item.slug,
        name: pickTitle("ContentItem", item.id, item.slug, "title"),
      });
    }

    const productSlugs = new Set(fsProducts.map((p) => p.slug));
    const productTypeItems = contentByType["products"] ?? [];
    const mergedProducts = [
      ...fsProducts,
      ...productTypeItems
        .filter((p) => !productSlugs.has(p.slug))
    ...
    const contentTypeMeta = contentTypes.map((t) => {
      const builtin = getBuiltinContentType(t.slug);
      return {
        slug: t.slug,
        name: builtin?.labelPluralEn ?? builtin?.nameEn ?? t.slug,
        routePrefix: t.routePrefix ?? builtin?.routePrefix ?? t.slug,
      };
    });
    ...
    return {
      ...
      sourceFamilies: buildSourceFamilies(contentTypeMeta),
    };
  },
};
