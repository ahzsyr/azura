...
/** Top floating gap for boxed headers, including mobile sticky-boxed overrides. */
export function resolveBoxedHeaderTopGapPx(opts: {
  headerStyle: string | undefined;
  isMobile?: boolean;
  mobileBoxedSticky?: boolean;
  mobileFlushTop?: boolean;
}): number {
  if (!isBoxedHeaderStyle(opts.headerStyle)) return 0;
  if (opts.isMobile) {
    // Boxed on → always keep the floating gap.
    if (opts.mobileBoxedSticky) return 12;
    // Boxed off → auto-remove (default) flushes; off keeps the gap.
    if (opts.mobileFlushTop !== false) return 0;
    return 12;
  }
  return 12;
}
