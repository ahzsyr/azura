    ...
    while (cur) {
      if (seen.has(cur)) {
        warnings.push({
          ...
          message: `Circular hierarchy detected for category "${col.slug}" via parent "${cur}"`,
      ...
      if (cur === col.slug) {
        warnings.push({
          code: "CIRCULAR_HIERARCHY",
          message: `Category "${col.slug}" is its own ancestor`,
    ...
    for (let j = i + 1; j < slugs.length; j++) {
      ...
      if (ratio >= 0.85 && slugs[i] !== slugs[j]) {
        warnings.push({
          ...
          message: `Categories "${slugs[i]}" and "${slugs[j]}" have very similar slugs (similarity: ${(ratio * 100).toFixed(0)}%)`,
...
function validateCollectionRules(collections: Collection[]): ValidationWarning[] {
  const warnings: ValidationWarning[] = [];
  const validOps = new Set<string>(ALL_MATCHING_OPERATORS);

  for (const col of collections) {
    const root = upgradeLegacyRuleSet(col.conditions);
    const leaves = collectRuleLeaves(root);
    if (isEmptyRuleTree(root) || leaves.length === 0) {
      warnings.push({
        code: "INVALID_RULE",
        message: `Category "${col.slug}" has no matching rules — it will never match any product`,
        context: { collectionSlug: col.slug },
      });
      continue;
    }
    for (const leaf of leaves) {
      if (!isValidProductRuleField(leaf.field)) {
        warnings.push({
          code: "INVALID_RULE",
          message: `Category "${col.slug}" has rule with unknown field "${leaf.field}"`,
          context: { collectionSlug: col.slug, rule: leaf },
        });
      }
      if (!validOps.has(leaf.operator)) {
        warnings.push({
          code: "INVALID_RULE",
          message: `Category "${col.slug}" has rule with unknown operator "${leaf.operator}"`,
          context: { collectionSlug: col.slug, rule: leaf },
        });
      }
      const unary = isUnaryOperator(leaf.operator as MatchingRuleOperator);
      const hasValue =
        (leaf.value != null && String(leaf.value).trim() !== "") ||
        (Array.isArray(leaf.values) && leaf.values.length > 0);
      if (!unary && !hasValue) {
        warnings.push({
          code: "INVALID_RULE",
          message: `Category "${col.slug}" has a rule with empty value`,
          context: { collectionSlug: col.slug, rule: leaf },
        });
      }
    }
...
// Phase 4: kept dead / unreachable — Sync API rejects autoCreate:true.
// Do not wire this into Sync Center; delete in a later cleanup phase.
 ...
 * @deprecated Unreachable from Sync Center / API (autoCreate rejected). Kept until Phase cleanup.
 */
...
export async function syncCollections(options: SyncOptions = {}): Promise<SyncReport> {
  ...
  for (const orphan of orphanInfos) {
    warnings.push({
      code: "ORPHAN_PRODUCT",
      message: `Product "${orphan.slug}" does not match any category`,
  ...
  for (const col of localizedCollections) {
    if ((collectionCounts[col.slug] ?? 0) === 0) {
      warnings.push({
        ...
        message: `Category "${col.slug}" (${col.name}) has no matching products`,
...
export async function syncSingleProduct(
  ...
  if (matches.length === 0) {
    warnings.push({
      code: "ORPHAN_PRODUCT",
      message: `Product "${slug}" does not match any category`,
