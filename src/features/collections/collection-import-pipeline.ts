...
function normalizeImportCollection(raw: Collection, now: string): Collection | null {
  ...
  const rawConditions =
    ...
    (typeof raw === "object" && raw !== null && "rules" in raw
      ? (raw as Collection & { rules?: unknown }).rules
      : undefined);
  ...
  return normalizeCollectionForExport({
    ...
    conditions: upgradeLegacyRuleSet(rawConditions ?? emptyRuleGroup("any")),
...
function mergeCollection(existing: Collection, incoming: Collection): Collection {
  return {
    ...
    conditions: !isEmptyRuleTree(upgradeLegacyRuleSet(incoming.conditions))
      ? upgradeLegacyRuleSet(incoming.conditions)
      : existing.conditions,
