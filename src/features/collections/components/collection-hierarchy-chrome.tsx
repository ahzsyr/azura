...
export function CollectionHierarchyChrome({
  ...
  const selectCount = useMemo(() => {
    ...
    const hasMore = getChildCollections(leaf, all).length > 0;
  ...
  const levels = useMemo(() => {
    ...
    for (let i = 0; i < selectCount; i++) {
      ...
      const options = i === 0 ? getRootCollections(all) : getChildCollections(parentSlug, all);
      const selected = pathSlugs[i] ?? "";
      out.push({ options, selected });
    }
    return out;
  }, [selectCount, pathSlugs, all]);
  ...
  const chromeTabs = useMemo((): ChromeTab[] => {
    ...
    const siblings = leaf ? getChildCollections(leaf, all) : getRootCollections(all);
    ...
    return tabs;
  }, [all, bySlug, labels.allCollections, pathSlugs, value]);
  ...
  if (variant === "chrome") {
    const roots = getRootCollections(all);
    return (
      <nav
