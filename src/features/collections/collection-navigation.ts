...
export function buildCollectionTrail(
  ...
  return getCollectionAncestorChain(slug, bySlug).map((c) => ({
    name: c.name,
    href: publicLocalePath(prefix, `/categories/${c.slug}`),
  }));
}
...
/** True when a stored collection image is a real URL, not an icon-library id. */
export function isCollectionImageSrc(value?: string | null): boolean {
  const v = value?.trim() ?? "";
  return /^(https?:\/\/|\/\/|\/|data:|blob:)/i.test(v);
}

export function pickCollectionCardImage(media: {
  coverImage?: string;
  iconImage?: string;
}): string | undefined {
  if (media.coverImage && isCollectionImageSrc(media.coverImage)) return media.coverImage;
  if (media.iconImage && isCollectionImageSrc(media.iconImage)) return media.iconImage;
  return undefined;
}
