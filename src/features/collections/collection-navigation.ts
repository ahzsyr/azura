...
/** Direct children (visible), sorted by name. */
export function getChildCollections(parentSlug: string, all: Collection[]): Collection[] {
  const p = parentSlug.trim();
  return all
    .filter((c) => c.visible !== false && (c.parentSlug ?? "").trim() === p)
    .sort((a, b) => (a.name || "").localeCompare(b.name || ""));
}

/** Visible root collections (no parent in set), sorted by name. */
export function getRootCollections(all: Collection[]): Collection[] {
