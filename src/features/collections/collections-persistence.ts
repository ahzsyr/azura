...
async function saveJsonStoreCollections(collections: Collection[]): Promise<void> {
  ...
  await jsonStoreService.set(CATALOG_COLLECTIONS_NAMESPACE,
    CATALOG_COLLECTIONS_GLOBAL_KEY,
...
export async function upsertCatalogCollection(
  ...
  if (useDatabaseOnlyCatalog()) {
    ...
    if (originalSlug && originalSlug !== col.slug) {
      ...
      const { dualWriteCategoryFromCollection } = await import(
        "@/features/categories/dual-write-from-collection"
      );
      await dualWriteCategoryFromCollection(col);
      revalidateHeaderFlyoutImages();
...
export async function deleteCatalogCollection(slug: string): Promise<void> {
  ...
  if (useDatabaseOnlyCatalog()) {
    ...
    const { dualWriteDeleteCategoryBySlug } = await import(
      "@/features/categories/dual-write-from-collection"
    );
    await dualWriteDeleteCategoryBySlug(key);
    revalidateHeaderFlyoutImages();
    return;
  }
