...
function indexLeafValue(index: RuleIndex, leaf: RuleLeaf, slug: string): void {
  const val = leaf.value == null ? "" : String(leaf.value).trim();
  if (!val && !(leaf.values && leaf.values.length)) {
  ...
  switch (leaf.field) {
    case "brand":
      if (val) addToMap(index.byBrand, val, slug);
      break;
    case "category":
    case "categories":
      if (val) addToMap(index.byCategory, val, slug);
      for (const v of leaf.values ?? []) {
        if (v != null && String(v).trim()) addToMap(index.byCategory, String(v), slug);
      }
      break;
    case "tags":
      if (val) addToMap(index.byTag, val, slug);
      for (const v of leaf.values ?? []) {
        if (v != null && String(v).trim()) addToMap(index.byTag, String(v), slug);
      }
      break;
    default:
      index.unindexed.add(slug);
...
export function buildCollectionRuleIndex(collections: Collection[]): RuleIndex {
  ...
  for (const col of collections) {
    ...
    // Nested groups: skip index pruning — full evaluate is required.
    if (hasNestedRuleGroups(root)) {
    ...
    for (const leaf of leaves) {
      indexLeafValue(index, leaf, col.slug);
    }
  }
