...
export async function buildLocaleProductIndex(
  ...
  // Full taxonomy (incl. hidden) so facet-index respects Category.visible
  const collections = orderCollectionsHierarchy(await loadCollectionsFromFs(prefix));
  ...
  const invertedFacetIndex = buildInvertedFacetIndex(locale, records, collections);
...
function buildIndexesFromRecords(
  ...
  return {
    ...
    invertedFacetIndex: buildInvertedFacetIndex(locale, records, collections),
    slugPathIndex: {
      version: PRODUCT_INDEX_VERSION,
      semanticVersions: PRODUCT_INDEX_SEMANTIC_VERSIONS,
      locale,
