...
export function ProductCardQuickAction({ ctx }: Props) {
  if (!isActionEnabled(ctx, "buy_now") || !ctx.showBuyNow || !ctx.buyNowNode) return null;
  return (
...
export function ProductCardActionBar({ ctx }: Props) {
  ...
  const showBuy = isActionEnabled(ctx, "buy_now") && ctx.showBuyNow && ctx.buyNowNode;
  const showQuote = isActionEnabled(ctx, "quote") && ctx.showQuoteCta && ctx.quoteCtaNode;
  const customs = ctx.design.actions.customActions.filter((action) => action.enabled);

  if (!showQuickView && !showBuy && !showQuote && !customs.length) return null;
  ...
  return (
    ...
    >
      ...
      {showBuy ? ctx.buyNowNode : null}
      {showQuote ? ctx.quoteCtaNode : null}
    </footer>
  );
}
