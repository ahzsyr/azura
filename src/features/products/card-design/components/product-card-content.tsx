...
export function ProductCardContent({ ctx }: Props) {
  ...
  useEffect(() => {
    cardSessionDebugLog(
      "product-card-content.tsx:mount",
      "ProductCardContent slot layout",
      {
  ...
  return (
    <section className="pl-card__content">
      ...
      {metaNodes.length > 0 ? (
        <footer className="pl-card__meta">{metaNodes}</footer>
      ) : null}
    </section>
  );
}
