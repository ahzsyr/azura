...
export function ProductCardContent({ ctx }: Props) {
  ...
  return (
    <section className="pl-card__content">
      ...
      {metaNodes.length > 0 || (ctx.ctaLayout === "inline_meta" && ctx.showProductCta && ctx.productCtaNode) ? (
        <footer className="pl-card__meta">
          {metaNodes}
          {ctx.ctaLayout === "inline_meta" && ctx.showProductCta && ctx.productCtaNode ? ctx.productCtaNode : null}
        </footer>
      ) : null}
    </section>
  );
