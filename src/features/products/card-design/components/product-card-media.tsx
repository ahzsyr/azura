...
export function ProductCardMedia({ ctx }: Props) {
  const { product, design, navHref, priority } = ctx;
  ...
  const showHoverSwap = Boolean(hoverSrc && !showGallery);
  ...
  const onImageLoad = useCallback(() => {
    setLoaded(true);
    cardSessionDebugLog(
      "product-card-media.tsx:onLoad",
      "Card image loaded (skeleton class toggle only)",
      { slug: product.slug, showSkeleton: design.media.showSkeleton, showHoverSwap },
      "H12",
      "post-fix",
    );
  }, [product.slug, design.media.showSkeleton, showHoverSwap]);
  // #endregion
  ...
  return (
    <LocaleLink
      ...
      className={[
        "pl-card__media-link",
        design.media.showSkeleton && !loaded && displaySrc ? "pl-card__media-link--loading" : "",
