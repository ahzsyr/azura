...
describe("resolveProductCardDesign preset persistence", () => {
  ...
  it("resolves media_flip preset with flip-oriented defaults", () => {
    const merged = applyProductCardPreset("media_flip", {});
    const design = resolveProductCardDesign({
      partial: {
        ...merged,
        presetId: "media_flip",
    ...
    assert.equal(design.presetId, "media_flip");
    assert.equal(design.style, "media_flip");
    assert.equal(design.hoverEffect, "none");
    assert.equal(design.showCategory, true);
    assert.equal(design.quickViewMode, "disabled");
    assert.equal(design.viewDetailsMode, "product_page");
    assert.equal(design.media.hoverSwap, false);
    assert.equal(design.media.effect, "none");
    assert.ok(design.contentOrder.includes("features"));
  });
