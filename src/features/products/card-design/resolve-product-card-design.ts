...
import {
  ...
  type ProductCardContentAlignment,
...
export const DESIGN_DEFAULTS: ResolvedProductCardDesign = {
  ...
  quickViewMode: "hover_overlay",
  viewDetailsMode: "disabled",
  effects: {
    enabled: false,
  ...
  actions: {
    enabledTypes: ["buy_now", "cta", "wishlist", "compare", "quick_view"],
    customActions: [],
...
const CONTENT_ALIGNMENTS: ProductCardContentAlignment[] = ["left", "center", "mixed"];
const CARD_SURFACES: ProductCardSurface[] = ["flat", "elevated", "glass", "premium"];
const QUICK_VIEW_MODES: ProductCardQuickViewMode[] = ["disabled", "hover_overlay", "action_button"];
const VIEW_DETAILS_MODES: ProductCardViewDetailsMode[] = ["disabled", "product_page", "quick_view"];

function isContentAlignment(v: unknown): v is ProductCardContentAlignment {
  return typeof v === "string" && CONTENT_ALIGNMENTS.includes(v as ProductCardContentAlignment);
}

function isCardSurface(v: unknown): v is ProductCardSurface {
  return typeof v === "string" && CARD_SURFACES.includes(v as ProductCardSurface);
}

function isQuickViewMode(v: unknown): v is ProductCardQuickViewMode {
  return typeof v === "string" && QUICK_VIEW_MODES.includes(v as ProductCardQuickViewMode);
}

function isViewDetailsMode(v: unknown): v is ProductCardViewDetailsMode {
  return typeof v === "string" && VIEW_DETAILS_MODES.includes(v as ProductCardViewDetailsMode);
}
...
export function normalizeProductCardDesignPartial(raw: unknown): ProductCardDesignPartial | undefined {
  ...
  if (isContentAlignment(o.contentAlignment)) out.contentAlignment = o.contentAlignment;
  if (isCardSurface(o.cardSurface)) out.cardSurface = o.cardSurface;
  if (isQuickViewMode(o.quickViewMode)) out.quickViewMode = o.quickViewMode;
  if (isViewDetailsMode(o.viewDetailsMode)) out.viewDetailsMode = o.viewDetailsMode;
  ...
  if (o.actions && typeof o.actions === "object") {
    ...
    const rawTypes = Array.isArray(a.enabledTypes) ? a.enabledTypes : [];
    const enabledTypes = rawTypes
      .map((t) => (t === "quote" ? "cta" : t))
      .filter(
        (t): t is ResolvedProductCardActions["enabledTypes"][number] =>
          t === "buy_now" ||
          t === "cta" ||
          t === "wishlist" ||
          t === "compare" ||
    ...
    const primaryRaw = a.primaryAction === "quote" ? "cta" : a.primaryAction;
    out.actions = {
      primaryAction:
        primaryRaw === "cta" ? "cta" : primaryRaw === "buy_now" ? "buy_now" : undefined,
      enabledTypes: enabledTypes.length ? enabledTypes : undefined,
...
function mergePartial(base: ResolvedProductCardDesign, partial: ProductCardDesignPartial): ResolvedProductCardDesign {
  ...
  return {
    ...
    contentAlignment: partial.contentAlignment ?? base.contentAlignment,
    cardSurface: partial.cardSurface ?? base.cardSurface,
    quickViewMode: partial.quickViewMode ?? base.quickViewMode,
    viewDetailsMode: partial.viewDetailsMode ?? base.viewDetailsMode,
...
export function resolveProductCardDesign(input: {
  ...
  const presetId =
    input.partial?.presetId && isStyle(input.partial.presetId)
      ? input.partial.presetId
      : undefined;

  if (presetId) {
    merged = applyProductCardPreset(presetId, {});
  }
  ...
  if (input.partial) {
    const normalized = normalizeProductCardDesignPartial(input.partial) ?? input.partial;
...
export function serializeProductCardDesignForSite(
  ...
  if (design.contentAlignment !== DESIGN_DEFAULTS.contentAlignment) o.contentAlignment = design.contentAlignment;
  if (design.cardSurface !== DESIGN_DEFAULTS.cardSurface) o.cardSurface = design.cardSurface;
  if (design.quickViewMode !== DESIGN_DEFAULTS.quickViewMode) o.quickViewMode = design.quickViewMode;
  if (design.viewDetailsMode !== DESIGN_DEFAULTS.viewDetailsMode) o.viewDetailsMode = design.viewDetailsMode;
