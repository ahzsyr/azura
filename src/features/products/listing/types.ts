...
export type ProductListingRecord = {
  ...
  categories: string[];
  /** Canonical Category ids when present on the product payload. */
  categoryIds?: string[];
  tags: string[];
...
export type ListingFilterLogic = "and" | "or";
...
export type ListingFilterState = {
  ...
  per: ListingPerPage;
  /**
   * How non-empty facet dimensions combine. Default `"and"` (existing behavior).
   * `"or"` ORs across dimensions (used by catalog nav ANY / cross-field).
   * Path scope (`collectionScope`), q, price, and stock still AND.
   */
