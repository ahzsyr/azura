...
export type RecordFromProductOptions = {
...
function normalizeSearchTextPart(part: string): string {
  const trimmed = part.trim();
  if (!trimmed) return "";
  if (!trimmed.includes("<")) return decodeBasicHtmlEntities(trimmed);
  return decodeBasicHtmlEntities(stripHtmlAndHiddenContent(trimmed));
}

function stripHtmlAndHiddenContent(input: string): string {
  return input
    .replace(/<\s*(script|style|noscript|template)\b[^>]*>[\s\S]*?<\s*\/\s*\1\s*>/gi, " ")
    .replace(
      /<([a-z][a-z0-9:-]*)\b[^>]*(?:\shidden\b|\saria-hidden\s*=\s*["']?true["']?|\sstyle\s*=\s*["'][^"']*(?:display\s*:\s*none|visibility\s*:\s*hidden)[^"']*["']|\sclass\s*=\s*["'][^"']*(?:\bhidden\b|\bsr-only\b|\bvisually-hidden\b)[^"']*["'])[^>]*>[\s\S]*?<\s*\/\s*\1\s*>/gi,
      " ",
    )
    .replace(/<[^>]+>/g, " ")
    .replace(/\s+/g, " ")
...
export function recordFromProduct(
  ...
  options?: RecordFromProductOptions,
): IndexedProductListingRecord {
  const name = product.name || product.productTitle || product.title || slug;
  const images = product.media?.images ?? [];
  const primaryImage = resolveProductPrimaryImageUrl(product);
  const secondaryImage = resolveProductSecondaryImageUrl(product);
  ...
  return {
    ...
    ...(product.buy_now_slug?.trim() ? { buy_now_slug: product.buy_now_slug.trim() } : {}),
  };
}
