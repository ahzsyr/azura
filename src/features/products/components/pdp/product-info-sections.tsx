...
const EXPLORE_COLLECTIONS_VISIBLE_LIMIT = 3;
...
export function ProductInfoCore({ product, slug, labels, display, brandHref }: CoreProps) {
  const sharedSlug = slug ?? product.mpn ?? product.id ?? "product";
  ...
  return (
    <>
      <div className="prd-info__brand" data-prd-compact-key="brand">
        {brandHref && product.brand ? (
          <Link href={brandHref} className="prd-info__brand-link">
            {product.brand}
          </Link>
        ) : (
      ...
      <SharedElementMarker type="product" id={sharedSlug} kind="title" as="h1" className="prd-info__title ui-text-product-detail" data-prd-compact-key="title">
        {title}
      </SharedElementMarker>
...
export function ProductInfoSection({
  ...
  if (section === "inlineCta") {
    ...
    const linkContext = productLinkContextFromProduct({ ...product, slug });
    return (
  ...
  if (section === "linkedTags") {
    ...
    return (
      <div className="prd-info__tags" data-prd-compact-key="linkedTags">
        ...
        <ProductCollectionChips tags={linkedTags.slice(0, 24)} />
      </div>
    );
  }
...
function ProductCollectionChips({ tags }: { tags: TagLink[] }) {
  const [expanded, setExpanded] = useState(false);
  const hasOverflow = tags.length > EXPLORE_COLLECTIONS_VISIBLE_LIMIT;
  const visibleTags = hasOverflow && !expanded
    ? tags.slice(0, EXPLORE_COLLECTIONS_VISIBLE_LIMIT)
    : tags;
  ...
  return (
    <>
      ...
      {hasOverflow ? (
        <button
          type="button"
          className="prd-info__tags-toggle"
          onClick={() => setExpanded((current) => !current)}
          aria-expanded={expanded}
        >
          {expanded ? "Show less" : `Show more (${tags.length - EXPLORE_COLLECTIONS_VISIBLE_LIMIT})`}
        </button>
      ) : null}
    </>
  );
