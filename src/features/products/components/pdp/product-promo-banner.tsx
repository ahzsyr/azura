...
import { publicLocalePath, stripAnyLocalePrefix } from "@/i18n/url-helpers";
...
export function ProductPromoBanner({ promo, locale }: Props) {
  ...
  const href = /^https?:\/\//i.test(promo.ctaHref)
    ? promo.ctaHref
    : publicLocalePath(
        locale,
        stripAnyLocalePrefix(
          promo.ctaHref.startsWith("/") ? promo.ctaHref : `/${promo.ctaHref}`,
          [locale],
        ),
      );
