...
export function ProductGallery({
  ...
  useEffect(() => {
    ...
    const syncHeight = () => {
      ...
      if (h > 0) {
        gallery.style.setProperty("--prd-gallery-main-height", `${Math.round(h)}px`);
      }
    };
  ...
  }, [thumbPlacement, active, allMedia.length]);

  useEffect(() => {
    if (thumbPlacement === "below") return;
    const rail = thumbsRef.current;
    if (!rail) return;
    const activeThumb = rail.querySelector<HTMLElement>(".prd-gallery__thumb.is-active");
    activeThumb?.scrollIntoView({ block: "nearest", behavior: "smooth" });
  }, [active, thumbPlacement]);
  ...
  }, [allMedia.length, isMobileGallery]);
  ...
  const mainWrapper = (
    <div ref={mainWrapperRef} className="prd-gallery__main-wrapper">
      <div
        ref={mainRef}
            ...
            {media.kind === "image" ? (
              <>
                <div
                  ...
                  {...(index === active ? sharedElementAttrs("product", slug, "image") : {})}
                >
                  <Image
                    ...
                    unoptimized={!shouldOptimizeNextImage(media.url)}
                    data-zoom-image={index === active ? "" : undefined}
