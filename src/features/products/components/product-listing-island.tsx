...
import {
  ...
  productListingListColumnsCssVars,
} from "@/features/catalog/lib/catalog-layout";
...
import { DEFAULT_PRODUCT_LISTING_FILTER_SETTINGS } from "@/features/products/listing/product-listing-filters.schema";
...
const VIEW_MODE_STORAGE_PREFIX = "pl-catalog-view-mode";

function resolveInitialViewMode(viewModes: ViewMode[], preferred: ViewMode): ViewMode {
  if (viewModes.includes(preferred)) return preferred;
  return viewModes[0] ?? "grid";
}
...
function readStoredViewMode(
  ...
  viewModes: ViewMode[],
  fallback: ViewMode,
  mobileFallback?: ViewMode,
): ViewMode {
  if (typeof window === "undefined") return fallback;
  const isMobile = window.matchMedia(MOBILE_DOCK_MQ).matches;
  const resolvedFallback =
    isMobile && mobileFallback && viewModes.includes(mobileFallback)
      ? mobileFallback
      : fallback;
  try {
    const stored = sessionStorage.getItem(`${VIEW_MODE_STORAGE_PREFIX}:${storageKey}`);
    if (stored && viewModes.includes(stored as ViewMode)) {
...
type Props = {
  ...
  /** Full taxonomy incl. hidden — used for Category.visible facet suppression. */
  ...
  /** Admin visibility + order for sidebar filter sections. */
...
export function ProductListingIsland({
  ...
  catalogToolbarDock,
  pageDir = "ltr",
  listingFilters = DEFAULT_PRODUCT_LISTING_FILTER_SETTINGS,
  ...
  const listColumnsCssVars = productListingListColumnsCssVars({
    listColumnsDesktop,
    listColumnsTablet,
  });
  const rootStyle = {
    ...(dock ? catalogToolbarDockCssVars(dock) : {}),
    ...listColumnsCssVars,
  } as CSSProperties;
  ...
  /** True while the user is editing search ahead of (or pending) URL sync. */
  const searchDirtyRef = useRef(false);
  const [fuzzySearching, setFuzzySearching] = useState(false);
  const listingViewStorageKey = `${listingMode}-${layoutVariant}-${collectionScope ?? "all"}`;
  const resolvedDefaultViewMode = resolveInitialViewMode(viewModes, defaultViewMode);
  const mobileDefaultViewMode = viewModes.includes("list") ? "list" : resolvedDefaultViewMode;
  const [viewMode, setViewMode] = useState<ViewMode>(resolvedDefaultViewMode);

  useLayoutEffect(() => {
    setViewMode(
      readStoredViewMode(
        listingViewStorageKey,
        viewModes,
        resolvedDefaultViewMode,
        mobileDefaultViewMode,
      ),
    );
  }, [listingViewStorageKey, viewModes, resolvedDefaultViewMode, mobileDefaultViewMode]);

  const handleViewModeSelect = useCallback(
    (nextMode: ViewMode) => {
      if (!viewModes.includes(nextMode) || nextMode === viewMode) return;
      setViewMode(nextMode);
      try {
        sessionStorage.setItem(`${VIEW_MODE_STORAGE_PREFIX}:${listingViewStorageKey}`, nextMode);
      } catch {
        /* ignore storage errors */
      }
    },
    [listingViewStorageKey, viewMode, viewModes],
  );
  ...
  const facetTaxonomyNodes = facetTaxonomy ?? collections;

  const hierarchyAsLocalized = useMemo(
    () =>
      facetTaxonomyNodes.map((c) => ({
        slug: c.slug,
        name: c.name,
        parentSlug: c.parentSlug,
        id: c.id || c.slug,
        ...
        visible: c.visible,
        conditions: { kind: "group" as const, match: "any" as const, children: [] },
      })),
    [facetTaxonomyNodes],
  );
  ...
  const state = optimisticState ?? urlState;

  useEffect(() => {
    if (!optimisticState) return;
    if (
      serializeCanonicalFilterState(urlState) === serializeCanonicalFilterState(optimisticState)
    ) {
      setOptimisticState(null);
    }
  }, [urlState, optimisticState]);

  // Sync search input from URL only when the user is not mid-edit (dirty).
  // Filter-only URL updates must never wipe in-progress typing.
  useEffect(() => {
    const urlQ = urlState.q.trim();
    if (searchDirtyRef.current) {
      // Clear dirty once our debounced write-back (or matching URL) lands.
      if (urlQ === qInput.trim() || urlQ === debouncedQ.trim()) {
        searchDirtyRef.current = false;
      }
      return;
    }
    if (urlQ === qInput.trim()) {
      flushDebouncedQ(urlQ);
      return;
    }
    setQInput(urlState.q);
    flushDebouncedQ(urlQ);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- intentionally keyed on urlState.q only; qInput/debouncedQ read for dirty comparison
  }, [urlState.q, flushDebouncedQ]);
  ...
  const replaceStateNow = useCallback(
    (next: ListingFilterState, _source = "unknown") => {
      const nextKey = serializeCanonicalFilterState(next);
      const currentKey = serializeCanonicalFilterState(urlState);
  ...
  const replaceState = useCallback(
    (next: ListingFilterState, source = "unknown") => {
      // Optimistic: update controls immediately; URL follows (debounced except search settle).
  ...
  // Single debounce path: qInput → debouncedQ → URL + pendingFilterState (same value).
  // No secondary 1200ms settle — Enter is optional flush only.
  useEffect(() => {
    const trimmed = debouncedQ.trim();
    if (trimmed === state.q.trim()) return;
    replaceState({ ...state, q: trimmed, page: 1 }, "debounced-search-sync");
  ...
  const filterOptions = useMemo(
    () => ({
      ...
      listingMode: mode,
      categoryTaxonomy: facetTaxonomyNodes,
    }),
    [scopeBySlug, mode, facetTaxonomyNodes],
  );
  ...
  const showFiltersSidebar = listingFilters.showSidebar !== false;

  const sidebar = showFiltersSidebar ? (
    <aside
  ...
  const filtersToggleBtn =
    showFiltersSidebar && !pinnedSidebar ? (
      <button
        type="button"
  ...
  const rootClass = [
    ...
    showFiltersSidebar && drawerOpen && !pinnedSidebar ? "pl-root--filters-drawer-open" : "",
    showFiltersSidebar ? "pl-root--sidebar-sticky" : "pl-root--no-filters",
    showFiltersSidebar && pinnedSidebar ? "pl-root--sidebar-pinned" : "",
    listingFetch.loading ? "pl-root--pending" : "",
    `pl-root--view-${viewMode}`,
  ]
    .filter(Boolean)
    .join(" ");
  ...
  const mobileFiltersPortal =
    showFiltersSidebar && drawerPortalReady && !pinnedSidebar
      ? createPortal(
          <div
            className="pl-filters-drawer-portal pl-root--catalog pl-root--collections-catalog"
            dir={pageDir}
          >
            {filtersBackdrop}
            {sidebar}
  ...
  const catalogToolbar = (
    ...
    >
      ...
      <div className="pl-catalog-toolbar__row pl-catalog-toolbar__row--search">
        ...
        >
          {showFiltersSidebar && !pinnedSidebar ? filtersToggleBtn : null}
  ...
  return (
    <ProductCardThemeProvider theme={cardTheme} listingViewMode={viewMode}>
      <div className={rootClass} style={rootStyle} dir={pageDir}>
        <CatalogContentLayout
          dir={pageDir}
          sidebar={showFiltersSidebar && pinnedSidebar ? sidebar : null}
          backdrop={null}
          dock={
            dockEnabled ? (
              <div
              ...
              >
                <div className="pl-catalog-dock-shell">
                  <div id="pl-catalog-dock-content" className="pl-catalog-dock-content">
                    {catalogToolbar}
                  </div>
                </div>
              </div>
