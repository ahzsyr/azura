...
type ProductCardThemeContextValue = ProductCardTheme & {
  ...
  /** Listing toolbar view mode; list disables media-flip interaction. */
  listingViewMode: "grid" | "list" | "table";
};
...
export type ProductCardThemeProviderProps = {
  ...
  /** Active listing view — list disables media-flip cards. */
  listingViewMode?: "grid" | "list" | "table";
  children: ReactNode;
};
...
export function ProductCardThemeProvider({
  ...
  const value = useMemo<ProductCardThemeContextValue>(
    ...
    [theme, viewport, displayOverrides, cardVariant, listingViewMode],
  );
