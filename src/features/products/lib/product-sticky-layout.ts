...
function detectLayoutViewport(): LayoutViewport {
  if (typeof window === "undefined") return "desktop";
  if (window.innerWidth >= BUILDER_DESKTOP_MIN_PX) return "desktop";
  if (window.innerWidth > BUILDER_MOBILE_MAX_PX) return "tablet";
  return "mobile";
}

function syncViewportLayoutAttrs(page: HTMLElement): void {
  const rawMap = page.getAttribute("data-prd-layout-map");
  if (!rawMap) return;
  let map: Partial<Record<LayoutViewport, LayoutAttrSet>> | null = null;
  try {
    map = JSON.parse(rawMap) as Partial<Record<LayoutViewport, LayoutAttrSet>>;
  } catch {
    return;
  }
  const viewport = detectLayoutViewport();
  const attrs = map?.[viewport];
  if (!attrs) return;
  page.setAttribute("data-prd-sticky-buy", attrs.stickyBuy);
  page.setAttribute("data-prd-sticky-crumb", attrs.stickyCrumb);
  page.setAttribute("data-prd-fixed-buy", attrs.fixedBuy);
  page.setAttribute("data-prd-mobile-gallery", attrs.mobileGallery);
  page.setAttribute("data-prd-side-compact-enabled", attrs.compactEnabled);
  page.setAttribute("data-prd-side-compact-offset", attrs.compactOffset);
  page.setAttribute("data-prd-side-compact-visible", attrs.compactVisible);
}
