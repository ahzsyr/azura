...
function readLayoutHint(raw: Record<string, unknown>, csvText?: string): string | null {
  const fromJson = [raw.page_layout_template, raw.output_format, raw._output_format]
 ...
 */
export function applyMikrotikImportLayoutHint(
  ...
  if (!hint) return raw;
  if (validateTemplateId(hint) !== "mikrotik" && hint.toLowerCase() !== "mikrotik") return raw;
  return { ...raw, page_layout_template: "mikrotik" };
}

export function applyMikrotikImportLayout(product: Product, csvText?: string): Product {
  return applyMikrotikImportLayoutHint(
