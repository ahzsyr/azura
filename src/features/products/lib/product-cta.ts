...
} from "./product-whatsapp-link";

export type ProductCtaVariant = "solid" | "outline" | "ghost" | "link" | "soft" | "gradient";

export type ProductCtaLinkType = "internal" | "external" | "whatsapp";
...
export const DEFAULT_RESOLVED_PRODUCT_CTA: ResolvedProductCtaConfig = {
  ...
  externalUrl: "",
  whatsappPhone: "",
  whatsappMessage: DEFAULT_WHATSAPP_MESSAGE_TEMPLATE,
  openInNewTab: false,
  placements: {
    inline: true,
    floating: false,
...
function isLinkType(v: unknown): v is ProductCtaLinkType {
  return v === "internal" || v === "external" || v === "whatsapp";
}
...
function resolveLinkTypeFromRaw(
  o: Record<string, unknown>,
  base: ResolvedProductCtaConfig,
): ProductCtaLinkType {
  ...
  if (linkType === "external" && parseWhatsAppExternalUrl(externalUrlRaw)) {
    return "whatsapp";
  }
  return linkType;
}

function whatsappFieldsFromRaw(
  o: Record<string, unknown>,
  linkType: ProductCtaLinkType,
  base: ResolvedProductCtaConfig,
): { whatsappPhone: string; whatsappMessage: string } {
  const phone = str(o.whatsappPhone, base.whatsappPhone);
  const message = str(o.whatsappMessage, base.whatsappMessage) || DEFAULT_WHATSAPP_MESSAGE_TEMPLATE;
  if (linkType !== "whatsapp") {
    return { whatsappPhone: phone, whatsappMessage: message };
  }
  if (phone) return { whatsappPhone: phone, whatsappMessage: message };
  const parsed = parseWhatsAppExternalUrl(str(o.externalUrl, ""));
  if (parsed) {
    return {
      whatsappPhone: parsed.phone,
      whatsappMessage: parsed.message.includes("{productTitle}")
        ? parsed.message
        : parsed.message
          ? `${parsed.message.trim()} {productTitle}`
          : DEFAULT_WHATSAPP_MESSAGE_TEMPLATE,
    };
  }
  return { whatsappPhone: phone, whatsappMessage: message };
...
export function normalizeProductCtaGlobal(raw: unknown): ResolvedProductCtaConfig {
  ...
  let linkType = resolveLinkTypeFromRaw(o, base);
  const { whatsappPhone, whatsappMessage } = whatsappFieldsFromRaw(o, linkType, base);
  const externalUrlRaw = str(o.externalUrl, "");
  const internalPathRaw = str(o.internalPath, base.internalPath);
...
export function mergeProductCta(base: ResolvedProductCtaConfig, partial: unknown): ResolvedProductCtaConfig {
  ...
  const linkType =
    p.linkType !== undefined ? (isLinkType(p.linkType) ? p.linkType : base.linkType) : base.linkType;

  return {
    enabled: typeof p.enabled === "boolean" ? p.enabled : base.enabled,
    label: p.label !== undefined ? str(p.label, base.label) || base.label : base.label,
    ...
    whatsappPhone: p.whatsappPhone !== undefined ? str(p.whatsappPhone, "") : base.whatsappPhone,
    whatsappMessage:
      p.whatsappMessage !== undefined
        ? str(p.whatsappMessage, base.whatsappMessage) || DEFAULT_WHATSAPP_MESSAGE_TEMPLATE
...
export function buildProductCtaHref(
  ...
  if (cfg.linkType === "whatsapp") {
    return buildWhatsAppProductUrl(cfg.whatsappPhone, cfg.whatsappMessage, ctx ?? { productTitle: "" });
  }

  if (cfg.linkType === "external") {
    const ext = cfg.externalUrl.trim();
    if (!ext) return null;
    const parsed = parseWhatsAppExternalUrl(ext);
    if (parsed) {
      const template = parsed.message.includes("{productTitle}")
        ? parsed.message
        : parsed.message.trim()
          ? `${parsed.message.trim()} {productTitle}`
          : DEFAULT_WHATSAPP_MESSAGE_TEMPLATE;
      return buildWhatsAppProductUrl(parsed.phone, template, ctx ?? { productTitle: "" });
    }
    return ext;
  }
...
export function validateProductCtaExternalUrl(raw: string): { valid: boolean; message?: string } {
  ...
  if (parseWhatsAppExternalUrl(t)) return { valid: true };
  try {
    const u = new URL(t);
