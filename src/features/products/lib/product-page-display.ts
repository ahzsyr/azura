...
/** Keep legacy addToCart alias aligned with buyNow visibility. */
export function syncBuyNowAddToCartDisplay(
  display: ResolvedProductPageDisplay,
): ResolvedProductPageDisplay {
  const buyNowEnabled = display.buyNow.enabled;
  return {
    ...display,
    buyNow: { enabled: buyNowEnabled },
    addToCart: { enabled: buyNowEnabled },
  };
}
...
> = {
  ...
  quantity: "Quantity",
  addToCart: "Add to cart (legacy alias)",
