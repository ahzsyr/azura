...
export type ProductBuyNowDestinationType = "shop" | "whatsapp";

export interface ProductBuyNowPartial {
  enabled?: boolean;
  label?: string;
  destinationType?: ProductBuyNowDestinationType;
  shopBaseUrl?: string;
...
function isDestinationType(v: unknown): v is ProductBuyNowDestinationType {
  return v === "shop" || v === "whatsapp";
}
...
export function normalizeProductBuyNowPartial(raw: unknown): ProductBuyNowPartial | undefined {
  ...
  if (isDestinationType(o.destinationType)) out.destinationType = o.destinationType;
  ...
  if (typeof o.whatsappPhone === "string") out.whatsappPhone = o.whatsappPhone;
  if (typeof o.whatsappMessage === "string") out.whatsappMessage = o.whatsappMessage;
...
export function resolveProductBuyNow(
  ...
  return {
    ...
    destinationType: isDestinationType(g.destinationType) ? g.destinationType : base.destinationType,
    ...
    whatsappPhone: str(g.whatsappPhone, base.whatsappPhone),
    whatsappMessage: str(g.whatsappMessage, base.whatsappMessage) || DEFAULT_WHATSAPP_MESSAGE_TEMPLATE,
...
/** Build buy-now URL (shop link or WhatsApp). Returns null when destination is not configured. */
export function buildBuyNowHref(
  ...
  if (cfg.destinationType === "whatsapp") {
    const slug = slugOverride?.trim() || productSlug.trim();
    return buildWhatsAppProductUrl(cfg.whatsappPhone, cfg.whatsappMessage, {
      productTitle: ctx?.productTitle || slug,
      productSlug: ctx?.productSlug ?? slug,
...
export function previewBuyNowHref(
  cfg: ResolvedProductBuyNow,
  sampleSlug = "sample-product",
  sampleTitle = "Sample Product",
): string | null {
  return buildBuyNowHref(cfg, sampleSlug, null, {
    productTitle: sampleTitle,
    productSlug: sampleSlug,
  });
}
