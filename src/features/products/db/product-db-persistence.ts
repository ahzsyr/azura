...
export async function saveProductToDb(
  ...
  try {
    ...
    const synced = await syncProductManualCategoryMemberships(row.id, product);
    if (
      JSON.stringify(synced.categoryIds ?? []) !== JSON.stringify(product.categoryIds ?? []) ||
      JSON.stringify(synced.categories ?? []) !== JSON.stringify(product.categories ?? [])
    ) {
