...
describe("applyProductOrderingToSearchHits", () => {
  it("keeps non-product hits in ranked position while reordering products", async () => {
    const { applyProductOrderingToSearchHits } = await import(
      "../apply-product-ordering-to-search-hits"
    );
    const settings = parseProductOrderingSettings({
      profiles: [
        baseProfile({
      ...
      activeProfileIdBySurface: { SEARCH: "search" },
    });
    ...
    const hits = [
      ...
      },
      {
        ...
        title: "Z Product",
        body: "coverage",
        urlPath: "/en/products/z-product",
        metadata: { slug: "z-product", card: { slug: "z-product" } },
        score: 50,
      },
      {
        id: "3",
        ...
        title: "A Product",
        body: "coverage",
        urlPath: "/en/products/a-product",
        metadata: { slug: "a-product", card: { slug: "a-product" } },
        score: 40,
      },
    ];

    const ordered = applyProductOrderingToSearchHits(hits, settings);
    assert.equal(ordered[0]?.entityType, "CONTENT_ITEM");
    assert.deepEqual(
      ordered.filter((h) => h.entityType === "CATALOG_PRODUCT").map((h) => {
        const meta = h.metadata as { slug?: string };
        return meta.slug;
      }),
      ["a-product", "z-product"],
    );
