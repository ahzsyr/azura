/**
 * @deprecated Phase 3 — prefer `entityService` from `@/features/entities`.
 * Direct use remains for legacy catalog paths during storage migration.
 */
import "server-only";
...
} from "./lib/product-primary-image";
import {
...
export type GetProductOptions = ProductVisibilityOptions;
...
async function readProductFromDb(
  ...
  if (!isProductVisibleOnStorefront(resolved.row.status, options)) return null;
...
function toSummary(slug: string, product: Product): ProductSummary {
  ...
  const primary = resolveProductPrimaryImageUrl(product);
  const secondary = resolveProductSecondaryImageUrl(product);
  const out =
...
async function loadSourceProductForFallback(
  ...
  const read = await readProductFromDb("en", slug, options);
...
async function applyLocaleFallback(
  ...
  const sourceProduct = await loadSourceProductForFallback(slug, options);
...
export const productsDataService = {
  ...
  async getProduct(
    ...
    if (useCatalogProductsDb()) {
      const read = await readProductFromDb(urlPrefix, s, options);
      if (!read) return null;
      if ("issue" in read) return null;
      return applyLocaleFallback(urlPrefix, read.localizedSlug, read.product, [], options);
    }
    ...
    if (!isProductVisibleOnStorefront(publishStatusFromProductPayload(read.product), options)) {
      return null;
    }

    return applyLocaleFallback(urlPrefix, entry.slug, read.product, [], options);
  },
  ...
  async getAllProducts(
    ...
    for (const entry of entries) {
      const loaded = await this.getProduct(urlPrefix, entry.slug, { includeUnpublished: true });
      if (!loaded) continue;
  ...
  async getProductSlugAlternates(
    ...
    if (useCatalogProductsDb()) {
      ...
      if (!isProductVisibleOnStorefront(resolved.row.status)) return out;
