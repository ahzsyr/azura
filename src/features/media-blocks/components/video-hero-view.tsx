...
import type { VideoHeroSlide } from "@/features/media-blocks/schemas/media-blocks";
...
export function VideoHeroView({
  ...
  const caption =
    locale.startsWith("ar") ? current?.captionAr || current?.captionEn : current?.captionEn || current?.captionAr;
  ...
  const mediaBg = (
    <div className="absolute inset-0 -z-20">
      {reduceMotion && currentPoster ? (
        ...
        >
          {captionTrackUrl ? <track kind="captions" src={captionTrackUrl} srcLang={locale.startsWith("ar") ? "ar" : "en"} /> : null}
        </video>
      ) : currentPoster ? (
        <Image src={currentPoster} alt="" fill className="object-cover" sizes="100vw" priority />
      ) : null}
  ...
  if (layout === "split") {
    return (
      <section data-block-type="videoHero" className={cn("relative overflow-hidden", overlayClass)} style={{ minHeight }}>
