...
import { MediaLightbox, type LightboxItem } from "@/features/media-blocks/components/media-lightbox";
import type { GalleryMediaPublic } from "@/features/gallery/types";
import type { MasonryGalleryItem } from "@/features/media-blocks/schemas/media-blocks";
...
function inlineToPublic(items: MasonryGalleryItem[]): GalleryMediaPublic[] {
  return items.map((item) => ({
    id: item.id,
    titleEn: item.altEn || item.captionEn,
    titleAr: item.altAr || item.captionAr,
    excerptEn: null,
    excerptAr: null,
    descriptionEn: item.captionEn,
    descriptionAr: item.captionAr,
    infoEn: item.categoryEn,
    infoAr: item.categoryAr,
