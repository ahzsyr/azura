...
export function ContentListPage({ contentType, items, initialSearch = "" }: Props) {
  ...
  return (
    <div className="space-y-6">
      <ContentAdminTabs />
      <AdminPageHeader
        title={contentType.labelPluralEn}
        description={contentType.nameEn}
        actions={
          <Button asChild>
            <Link href={`/admin/content/${typeSlug}/new`}>
              <Plus className="h-4 w-4 me-1" />
              Add {contentType.labelSingularEn.toLowerCase()}
            </Link>
          </Button>
        }
      />
      ...
      <Card>
        ...
        <CardContent>
          <EntityListManager
            ...
            emptyMessage={`No ${contentType.labelPluralEn.toLowerCase()} yet.`}
