...
export function ContentEditPage({
  ...
  const [sources, setSources] = useState<CitationSource[]>(() =>
    parseCitationSources(
      Array.isArray((item as (ItemWithType & { sources?: unknown }) | undefined)?.sources)
        ? (item as ItemWithType & { sources?: unknown }).sources as unknown[]
        : []
    )
  );
        ...
        <>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Reference links shown at the bottom of this page.</CardDescription>
            </CardHeader>
            <CardContent>
              <CitationSourcesField value={sources} onChange={setSources} />
            </CardContent>
          </Card>
        </>
      );
  ...
  const shouldSuppressPageActions = isSeoTab && Boolean(item);
  ...
  const shell = (
    <EntityAdminShell
      ...
      onSave={shouldSuppressPageActions ? undefined : () => void submitContentForm()}
      onCancel={!shouldSuppressPageActions && item ? handleCancel : undefined}
      onPreview={
        !shouldSuppressPageActions && previewHref
