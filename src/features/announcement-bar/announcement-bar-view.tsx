...
import {
  ...
  type NormalizeAnnouncementItemsOptions,
} from "@/features/announcement-bar/normalize-announcement-items";
...
import {
  ...
  resolveBarTone,
  resolveAnnouncementTextDirection,
...
export function AnnouncementBarView({
  ...
  const normalizeOptions: NormalizeAnnouncementItemsOptions = {
    locale,
    enabledLocales,
  };
  const lines = normalizeAnnouncementItems(items, normalizeOptions);
  const baseRows = lines.length > 0 ? lines : [FALLBACK_LINE];
  const sep = typeof separator === "string" && separator.length > 0 ? separator : " · ";
  const speedPercent = animations.scrollSpeedPercent ?? 100;
  const durationSec = scrollDurationSec(scrollSpeed, animations.scrollSpeedCustom, speedPercent);
  const mobileSpeed = responsive.mobileSpeed || scrollSpeed;
  const mobileDurationSec = scrollDurationSec(mobileSpeed, animations.scrollSpeedCustom, speedPercent);
  const scrollDirection = resolveMarqueeScrollDirection(direction, locale, enabledLocales);
  const textDirection = resolveAnnouncementTextDirection(locale, enabledLocales);
      ...
      >
        ...
        <div ref={viewportRef} className="az-ab__viewport" style={viewportStyle}>
          ...
          <div
            ...
            className={cn(
              "az-ab__track",
              scrollDirection === "right" && "az-ab__track--scroll-right",
            )}
            style={trackStyle}
          >
            <AnnouncementBarChunk
