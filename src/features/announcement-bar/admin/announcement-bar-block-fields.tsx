...
import { ItemCard } from "@/features/content-blocks/admin/shared/repeatable-section";
      ...
      >
        <div className="space-y-2">
          ...
          {items.map((item, index) => (
            <ItemCard
              key={item.id}
              title={item.messageEn?.trim() || item.message?.trim() || `Announcement ${index + 1}`}
              defaultOpen={false}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                ...
                <Input
                  className="mt-1 h-8 text-sm"
                  value={item.badgeEn || item.badge}
                  onChange={(e) =>
                    updateItems(
                      items.map((i) =>
                        i.id === item.id ? { ...i, badgeEn: e.target.value, badge: e.target.value } : i,
                      ),
                    )
                  }
                />
