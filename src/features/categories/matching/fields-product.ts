...
export function flattenProductSpecifications(
  ...
  canonicalSpecs?: Record<string, { value?: string; display?: string; name?: string }>,
): Record<string, string> {
  const out: Record<string, string> = {};
  if (canonicalSpecs && Object.keys(canonicalSpecs).length > 0) {
    for (const [id, record] of Object.entries(canonicalSpecs)) {
      const value = String(record.display || record.value || "").trim();
      if (value) out[`spec:${id}`] = value;
    }
    return out;
  }
...
export function collectProductSpecKeys(
  ...
  return Object.keys(flattenProductSpecifications(groups, canonicalSpecs)).map((k) =>
    k.replace(/^spec:/, ""),
  );
}
...
export function flattenSpecificationTokens(
  ...
  if (canonicalSpecs && Object.keys(canonicalSpecs).length > 0) {
    for (const [id, record] of Object.entries(canonicalSpecs)) {
      const value = String(record.display || record.value || "").trim();
      const name = String(record.name || id.split(".").pop() || id).trim();
      if (name) tokens.add(name);
      if (value) tokens.add(value);
      if (name && value) tokens.add(`${name}: ${value}`);
      tokens.add(id);
    }
    return [...tokens];
  }
...
export function productToRuleFields(slug: string, p: CatalogProduct): RuleEntityFields {
  ...
  const canonical = getCanonicalSpecsMap(p);
  const specs = flattenProductSpecifications(p.specifications, canonical);
  const specification = flattenSpecificationTokens(p.specifications, canonical);
  const matchingRules = readProductMatchingRules(p);
  const aliases = extractSpecAliasFields(p.source_specifications?.length ? p.source_specifications : p.specifications);
