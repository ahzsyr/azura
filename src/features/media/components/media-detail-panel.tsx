...
export function MediaDetailPanel({ assetId, folders, onClose, onUpdated }: Props) {
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      ...
      if (detail) {
        ...
        setAltEn(detail.altEn);
        setAltAr(detail.altAr);
  ...
  const saveMeta = () => {
    ...
    startTransition(async () => {
      ...
      onUpdated();
      const detail = await getMediaAssetDetail(assetId);
      if (detail) setAsset(detail as AssetDetail);
    });
  };
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          <div className="relative aspect-video rounded-lg bg-muted overflow-hidden">
            {asset.mediaType === "IMAGE" || asset.mediaType === "SVG" ? (
              <Image src={asset.url} alt={altEn} fill className="object-contain" sizes="400px" />
            ) : (
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Input value={altEn} onChange={(e) => setAltEn(e.target.value)} className="mt-1" />
            </div>
            <div>
              <Label>Alt text (AR)</Label>
              <Input value={altAr} onChange={(e) => setAltAr(e.target.value)} dir="rtl" className="mt-1" />
            </div>
            <div>
              <Label>Folder</Label>
              <select
            ...
            <Button type="button" size="sm" disabled={pending} onClick={saveMeta}>
              Save metadata
            </Button>
          </div>
