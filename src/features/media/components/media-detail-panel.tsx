...
export function MediaDetailPanel({ assetId, folders, onClose, onUpdated }: Props) {
  ...
  const [baseline, setBaseline] = useState<{ filename: string; folderId: string } | null>(null);
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      ...
      if (detail) {
        ...
        setBaseline({ filename: detail.filename, folderId: detail.folderId ?? "" });
        markSaved();
      }
    });
  }, [assetId, markSaved]);
  ...
  }, [assetId, filename, folderId, markSaved, onUpdated, reloadAlt, saveAlt, setSaveStatus]);

  const cancelMeta = useCallback(() => {
    if (!baseline) return;
    setFilename(baseline.filename);
    setFolderId(baseline.folderId);
    void reloadAlt();
    markSaved();
  }, [baseline, markSaved, reloadAlt]);

  useAdminPageActions({
    ownerKey: "media-detail",
    scope: "editor",
    priority: 0,
    onSave: saveMeta,
    onCancel: cancelMeta,
    ...
    canSave: !pending && !altLoading,
    canCancel: true,
    enabled: Boolean(assetId && asset),
  });

  useEffect(() => {
    if (!baseline) return;
    if (filename !== baseline.filename || folderId !== baseline.folderId) {
      markUnsaved();
    }
  }, [filename, folderId, baseline, markUnsaved]);
