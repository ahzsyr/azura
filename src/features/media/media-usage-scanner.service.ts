...
export const mediaUsageScanner = {
  ...
  async scanAll(): Promise<{ tracked: number }> {
    ...
    const [contentMedia, galleryAlbums, galleryMedia, faqSets, testimonials, themes, cmsPages, posts, headerRefs] =
      await Promise.all([
        ...
        prisma.faqSet.findMany({ select: { id: true, coverUrl: true } }),
    ...
    for (const row of faqSets) {
      if (!row.coverUrl) continue;
      const mediaId = urlToId.get(row.coverUrl);
      if (mediaId) refs.push({ mediaId, entityType: "FAQ_SET", entityId: row.id, field: "coverUrl" });
    }
