    ...
    for (const step of workflow.steps) {
      ...
      if (step.type === "workflow") {
        if (step.workflowId === workflow.id) {
          errors.push(`Workflow ${workflow.id} step cannot reference itself`);
        } else if (!registry.workflowsById.has(step.workflowId)) {
          errors.push(`Workflow ${workflow.id} step workflow missing "${step.workflowId}"`);
        }
      }
    }
