          ...
          {workflow.steps.map((step, index) => (
            <li key={step.id} className="flex flex-col items-start">
              <div className="flex flex-wrap items-center gap-2 rounded-md border bg-muted/30 px-3 py-2 text-sm">
                ...
                {step.type === "workflow" && (
                  <a
                    href={`#${step.workflowId}`}
                    className="text-primary underline-offset-2 hover:underline"
                    onClick={(e) => {
                      const nested = helpRegistry.workflowsById.get(step.workflowId);
                      const topicStep = nested?.steps.find((s) => s.type === "topic");
                      if (topicStep?.type === "topic" && onOpenTopic) {
                        e.preventDefault();
                        onOpenTopic(topicStep.topicId);
                      }
                    }}
                  >
                    Open
