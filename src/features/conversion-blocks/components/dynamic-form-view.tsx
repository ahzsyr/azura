...
function fieldLabel(field: FormFieldDefinition, locale: string) {
  return locale.startsWith("ar") ? field.labelAr || field.labelEn : field.labelEn || field.labelAr;
}
        ...
        <select className="w-full border rounded-md h-10 px-2 text-sm" {...register(field.id, { required: field.required })}>
          ...
          {(field.options ?? []).map((o) => (
            <option key={o.value} value={o.value}>
              {locale.startsWith("ar") ? o.labelAr || o.labelEn : o.labelEn || o.labelAr}
            </option>
          ))}
        </select>
        {err && <p className="text-xs text-destructive">{err}</p>}
...
export function DynamicFormView({
  ...
  const steps: FormStepDefinition[] = useMemo(() => {
    ...
    if (!multiStep) return [{ id: "all", titleEn: "", titleAr: "", fieldIds: definition.fields.map((f) => f.id) }];
    const chunk = 2;
    ...
    for (let i = 0; i < definition.fields.length; i += chunk) {
      ...
      out.push({
        id: `step-${i}`,
        titleEn: `Step ${out.length + 1}`,
        titleAr: `الخطوة ${out.length + 1}`,
        fieldIds: slice.map((f) => f.id),
      });
    }
    return out.length ? out : [{ id: "all", titleEn: "", titleAr: "", fieldIds: definition.fields.map((f) => f.id) }];
        ...
        <div className="space-y-2">
          ...
          {progressStyle === "steps" && (
            <p className="text-xs text-muted-foreground">
              {locale.startsWith("ar") ? steps[step]?.titleAr : steps[step]?.titleEn} ({step + 1}/{steps.length})
            </p>
          )}
          {progressStyle === "dots" && (
