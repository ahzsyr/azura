...
export type ProductsAdminInitialProps = {
  ...
  /** PRODUCT Category options for multi-select (unified Categories). */
...
export async function loadProductsAdminInitialProps(): Promise<ProductsAdminInitialProps> {
  ...
  let initialProductCategories: Array<{ id: string; slug: string; name: string }> = [];
  try {
    const { categoriesDataService } = await import(
      "@/features/categories/categories-data.service"
    );
    initialProductCategories = await categoriesDataService.listProductCategoryOptions();
  } catch {
