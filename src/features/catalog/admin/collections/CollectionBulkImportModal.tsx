...
function extractCollections(raw: unknown): unknown[] {
  ...
  throw new Error("Expected { collections: [...] } or a categories array (legacy key: collections)");
}
...
export function CollectionBulkImportModal({ open, onClose, onDone }: Props) {
  ...
  const runImport = async (preview: boolean) => {
    ...
    try {
      ...
      if (totalItems === 0) throw new Error("No categories found in file");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      ...
      >
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Upload a JSON file with <code className="text-xs">{`{ collections: [...] }`}</code> or a categories array (legacy key: collections).
        </p>
        ...
        <div className="mt-4 space-y-4">
          ...
          <label className="flex items-center gap-2 text-sm">
            ...
            Replace all existing categories (clear before import)
          </label>
