...
export default function ProductManagerApp({
  ...
  const [layoutRules, setLayoutRules] = useState<ProductPageLayoutRules>(() =>
    initialProductPageLayoutRules ??
    (initialProductPageLayout
      ? buildProductPageSettingsFromSite({ productPageLayout: initialProductPageLayout }).layoutRules
      : defaultPageSettings.layoutRules),
  );
  const [elementsRules, setElementsRules] = useState<ProductPageElementsRules>(() =>
    initialProductPageElementsRules ??
    buildProductPageSettingsFromSite({
  ...
  const [pageOverflow, setPageOverflow] = useState<ResolvedProductPageOverflow>(
    () => defaultPageSettings.overflow,
  );
  ...
  const saveCurrentSettingsTab = useCallback(async (): Promise<boolean> => {
    ...
    const ok = "Settings saved and storefront cache refreshed.";
    try {
      switch (adminTab) {
        case "buy-now":
        ...
        case "quote-cta":
          ...
          break;
        case "page-elements":
          const savedElementsRules = await saveProductPageElementsOnlySettings(
            adminLocaleCode,
            elementsRules,
            catalogLocaleCodes,
          );
          setElementsRules(savedElementsRules);
          savedSettingsRef.current.elementsRules = savedElementsRules;
          setLayoutFeedback({ kind: "ok", text: "Page element settings saved and cache refreshed." });
          break;
        case "page-layout":
          await saveProductPageLayoutOnlySettings(adminLocaleCode, layoutRules, pageOverflow);
          savedSettingsRef.current.layoutRules = layoutRules;
          savedSettingsRef.current.pageOverflow = pageOverflow;
          setLayoutFeedback({ kind: "ok", text: ok });
          break;
  ...
  const cancelCurrentSettingsTab = useCallback(() => {
    ...
    switch (adminTab) {
      ...
      case "quote-cta":
        ...
        break;
      case "page-elements":
        setElementsRules(saved.elementsRules);
        break;
      case "page-layout":
        setLayoutRules(saved.layoutRules);
        break;
      case "card-appearance":
        ...
        >
          ...
          {tab === "page-elements" && (
            <ProductPageElementsPanel
              ...
              setElementsRules={setElementsRules}
              onDirty={() => markUnsaved()}
            />
          )}
          ...
          {editPanel("page-display", active ? (
            <div className="pm-stack">
              <p className="pm-hint">
                ...
                ordering live in the <a href="#page-elements">Page elements</a> settings tab.
              </p>
              <ProductPageDisplayFields
                showInherit
                globalDisplay={elementsRules.desktop.display}
