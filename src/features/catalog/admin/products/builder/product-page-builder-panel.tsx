  ...
  return (
    <section className="ppb-root" aria-labelledby="ppb-title">
      <header className="ppb-header">
        <div className="ppb-header__left">
          ...
          <p className="ppb-header__desc">
            Layout, structure, and visibility for product detail pages. Commerce actions (Buy Now,
            CTA) are configured under Storefront Actions.
          </p>
        </div>
        <div className="ppb-header__actions">
          ...
          {onCancel ? (
            <Button
              ...
              disabled={!canCancel || saving || publishing}
            >
              Cancel
            </Button>
          ) : null}
          {onSave ? (
            <Button type="button" size="sm" onClick={() => void onSave()} disabled={saving || publishing}>
              {saving ? "Saving…" : "Save Changes"}
            </Button>
          ) : null}
          {onPublish ? (
            <Button type="button" size="sm" onClick={() => void onPublish()} disabled={saving || publishing}>
      ...
      {subTab === "components" ? (
        ...
        <div className="ppb-workspace ppb-workspace--visibility" role="tabpanel">
          <ProductPageVisibilityPanel studio={studio} />
        </div>
      )}
    </section>
  );
