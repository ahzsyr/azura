...
export function ProductPageLayoutFields({
  ...
  const id = `ppl-${viewport}`;
  const showDesktopChrome = viewport === "desktop";
  const showLegacyMobileGallery = viewport === "desktop";
  ...
  return (
    <fieldset className="apm-fieldset">
      ...
      <p className="apm-fieldset__hint">
        Controls the product detail template (gallery, buy box, tabs) for{" "}
        {viewport === "desktop" ? "desktop" : viewport === "tablet" ? "tablet" : "mobile"} viewports.
        Button styling is on the Buy Now and Get Quote CTA tabs.
      </p>
      <div className="pm-cta-grid">
        ...
        {viewport === "tablet" ? (
          <label className="pm-cta-field" htmlFor={`${id}-tablet-cols`}>
            <span>Tablet columns</span>
            <select
              id={`${id}-tablet-cols`}
              value={value.tabletColumnMode}
              onChange={(e) =>
                onChange({
                  ...value,
            ...
            >
              <option value="single">Single column stack</option>
              <option value="split">Split (gallery + buy box)</option>
            </select>
          </label>
        ) : null}
        {viewport === "mobile" || viewport === "tablet" ? (
          <label className="pm-cta-field" htmlFor={`${id}-gal-mobile`}>
            <span>Gallery mobile style</span>
            <select
              id={`${id}-gal-mobile`}
              value={value.galleryMobileLayout}
              onChange={(e) =>
                onChange({
                  ...value,
            ...
            >
              <option value="immersive">Immersive (contain, swipe, dots)</option>
              <option value="classic">Classic</option>
            </select>
          </label>
        ) : null}
        <label className="pm-cta-field" htmlFor={`${id}-gal-thumbs`}>
          <span>Thumbnail placement</span>
          <select
            id={`${id}-gal-thumbs`}
            value={value.galleryThumbPlacement}
            onChange={(e) =>
              onChange({
                ...value,
                galleryThumbPlacement: e.target.value as ResolvedProductPageLayout["galleryThumbPlacement"],
              })
            }
          >
            <option value="below">Below main image</option>
            <option value="left">Left of main image</option>
            <option value="right">Right of main image</option>
          </select>
        </label>
        ...
        {showLegacyMobileGallery ? (
          <label className="pm-cta-field pm-inline-check">
            ...
            />
            Mobile: gallery before buy box (legacy fallback)
          </label>
        ) : null}
        <label className="pm-cta-field pm-inline-check">
          <input
