...
type CtaSubTab = (typeof CTA_SUB_TABS)[number]["id"];

export function ProductCtaSettingsPanel({
  locale,
  cta,
  setCta,
  visibilityContext,
  ...
  const [subTab, setSubTab] = useState<CtaSubTab>("general");
  ...
  return (
    <section className="apm-dashboard-card apm-products-settings pm-storefront-cta" aria-labelledby="apm-product-cta-h">
      <header className="apm-dashboard-card__head">
        <h2 id="apm-product-cta-h" className="apm-dashboard-card__title">
          CTA Button
        </h2>
        <p className="apm-dashboard-card__lede">
          Customizable action button for lead generation — contact, WhatsApp, phone, email, booking, or any destination.
          Appears on the product page and optionally on catalog cards. Save from the top bar.
        </p>
      </header>

      <div className="apm-product-cta__subtabs">
        <AdminSettingsRibbon
          tabs={[...CTA_SUB_TABS]}
          activeTab={subTab}
      ...
      <div className="pm-cta-layout pm-storefront-cta__layout">
        <div className="pm-cta-layout__main apm-cta-form-stack">
          {subTab === "general" && (
            <div className="pm-cta-band pm-cta-band--cta">
                ...
                <section className="apm-cta-form__section" aria-labelledby="qc-sec-button">
                  ...
                  <label className="apm-cta-form__status">
                    ...
                    <span className="apm-cta-form__status-text">
                      <strong>Enable CTA Button</strong>
                      <span>Turn the CTA on or off site-wide (per placement on the Placements tab).</span>
                    </span>
                  </label>
                  ...
                  <div className="apm-cta-form__row apm-cta-form__row--1">
                    <label className="pm-cta-field">
                      ...
                      <select
                        value={cta.linkType}
                        onChange={(e) => {
                          const next = e.target.value as ResolvedProductCtaConfig["linkType"];
                          patch((c) => ({
                            ...c,
                            linkType: next,
                            ...(next === "whatsapp" && !c.icon.includes("whatsapp")
                    ...
                    ) : cta.linkType === "whatsapp" ? (
                      <>
                        <label className="pm-cta-field">
                          <span>WhatsApp phone number</span>
                          <input
                            type="tel"
                            value={cta.whatsappPhone}
                            onChange={(e) => patch((c) => ({ ...c, whatsappPhone: e.target.value }))}
                        ...
                        <label className="pm-cta-field">
                          ...
                          <textarea
                            rows={3}
                            value={cta.whatsappMessage}
                            onChange={(e) => patch((c) => ({ ...c, whatsappMessage: e.target.value }))}
                            placeholder="Hi, I'm interested in {productTitle}"
                          />
                        </label>
                        <p className="apm-cta-form__hint">
                          Use {"{productTitle}"}, {"{productSlug}"}, or {"{productSku}"} — replaced per product on the storefront.
                        </p>
                      </>
                    ) : (
                      <label className="pm-cta-field">
        ...
        <aside className="pm-cta-layout__aside pm-storefront-cta__preview" aria-label="CTA Button preview">
          <ProductActionVisibilityPanel
            action="cta"
            context={{ ...visibilityContext, productCta: cta }}
            className="pm-action-visibility--aside"
          />
          <CtaLivePreview cfg={cta} visibilityContext={{ ...visibilityContext, productCta: cta }} />
        </aside>
      </div>
    </section>
  );
...
/** @deprecated Use ProductCtaSettingsPanel */
export const ProductQuoteCtaSettingsPanel = ProductCtaSettingsPanel;
