...
export interface DataTableProps<T> {
  ...
  /** Render bulk actions as buttons or as a select + apply button. */
  bulkActionMode?: "buttons" | "select";
  /** Optional placeholder when `bulkActionMode` is `select`. */
  ...
  /**
   * `immediate` (default): persist on cell commit, clear dirty, toast "Saved".
   * `deferred`: apply local draft only; keep dirty until parent Save; toast "Updated".
   */
  inlineEditMode?: "immediate" | "deferred";
...
/** Imperative API for flushing / discarding an open inline cell edit. */
export type DataTableHandle = {
  /** Commit the open cell (if any). Resolves true when a commit ran successfully. */
  commitActiveEdit: () => Promise<boolean>;
  /** Discard the open cell without applying. */
  cancelActiveEdit: () => void;
  /** Clear all dirty-row indicators (after parent Save or Cancel). */
  clearDirtyKeys: () => void;
};
