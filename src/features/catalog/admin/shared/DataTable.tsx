...
export type { DataTableHandle } from "./types";
...
function DataTableInner<T>(
  {
    ...
    bulkActionPlaceholder = "Select action",
    getRowKey,
    onInlineEdit,
    inlineEditMode = "immediate",
    onBulkDelete,
    toolbarLeft,
    toolbarRight,
    loading = false,
  ...
  const deferred = inlineEditMode === "deferred";
  ...
  const handleCommit = useCallback(async (): Promise<boolean> => {
    ...
    if (!save || !onInlineEdit) return false;
  ...
  }, [ts, onInlineEdit, getRowKey, pushToast, deferred]);

  useImperativeHandle(
    ref,
    () => ({
      commitActiveEdit: async () => {
        if (!ts.editState) return false;
        return handleCommit();
      },
      cancelActiveEdit: () => {
        ts.cancelEdit();
  ...
  const selectedBulkAction = useMemo(
    () => bulkActions.find((action) => action.key === selectedBulkActionKey) ?? null,
    [bulkActions, selectedBulkActionKey],
  );

  useEffect(() => {
    if (selectedBulkActionKey && !selectedBulkAction) {
      setSelectedBulkActionKey("");
    }
  }, [selectedBulkActionKey, selectedBulkAction]);

  const applySelectedBulkAction = useCallback(async () => {
    if (!selectedBulkAction) return;
    const ok = await runBulkAction(selectedBulkAction);
    if (ok) setSelectedBulkActionKey("");
  }, [runBulkAction, selectedBulkAction]);
      ...
      {ts.selectedKeys.size > 0 && (
        <div className="dt-bulk-bar">
          ...
          {bulkActionMode === "select" ? (
            <>
              <select
                className="dt-density-select dt-bulk-select"
