...
export async function loadCatalogItems(config: CatalogBlockConfig): Promise<CatalogCardData[]> {
  ...
  async function loadForSlug(contentTypeSlug: string): Promise<CatalogCardData[]> {
    ...
    const attributeFilters = catalogAttributeFiltersForSource(
      contentTypeSlug === "offerings" ? "services" : config.source,
      config,
    );
    let filtered = cards;
  ...
  // Empty solutions catalog → fall back to offerings/services (existing service cards)
  if (
    result.length === 0 &&
    (typeSlug === "solutions" || config.source === "solutions")
  ) {
    result = await loadForSlug("offerings");
  }
