...
function localeProductsPath(locale: string): string {
  return publicLocalePath(locale, "/products");
}
...
function pageLinkHref(locale: string, item: CatalogNavigationItem): string {
  ...
  switch (item.targetType) {
    case "CATEGORY":
      return id
        ? publicLocalePath(locale, `/categories/${id}`)
        : publicLocalePath(locale, "/categories");
    case "BRAND":
      return id
        ? publicLocalePath(locale, `/brands/${id}`)
        : publicLocalePath(locale, "/brands");
    case "PRODUCT":
      return id
        ? publicLocalePath(locale, `/products/${id}`)
        : publicLocalePath(locale, "/products");
    case "PAGE":
      return id ? publicLocalePath(locale, `/${id.replace(/^\//, "")}`) : publicLocalePath(locale, "/");
    case "URL":
      return item.url || publicLocalePath(locale, "/");
    default:
      return item.url || publicLocalePath(locale, "/");
  }
}
