...
async function persistLogEntry(entry: CatalogSyncLogEntry): Promise<void> {
  try {
    ...
    await jsonStoreService.set(CATALOG_SYNC_LOG_NAMESPACE,
      "entries",
      next as unknown as Prisma.InputJsonValue, { revalidate: true });
  } catch {
    /* JsonStore optional */
  }
}
...
export async function saveCatalogSyncStatus(status: CatalogSyncStatus): Promise<void> {
  try {
    const { jsonStoreService } = await import("@/features/storage/json-store.service");
    await jsonStoreService.set(CATALOG_SYNC_STATUS_NAMESPACE,
      CATALOG_SYNC_STATUS_KEY,
      status as unknown as Prisma.InputJsonValue, { revalidate: true });
  } catch {
    /* optional */
  }
}
