...
} from "@/capabilities/search/engine/indexer/catalog-index-sync";
...
} from "@/lib/patch/revalidation-map";

export type CatalogSyncResult = {
  ok: boolean;
  collectionSync: ProductSyncResult | null;
  indexSync: { count: number; mode: "patch" | "full" | "db-memory" } | null;
...
async function localePrefixFromCatalogLocale(locale: string): Promise<string> {
...
export const catalogSyncOrchestrator = {
  async onProductSaved(
    ...
    const prefix = await localePrefixFromCatalogLocale(catalogLocale);
  ...
  /** Scoped sync after a partial product patch (only runs side effects for changed path groups). */
  async onProductPatched(
    localePrefix: string,
    slug: string,
    product: Product,
    appliedPaths: string[],
    ...
    const affectsCollections = productPatchAffectsCollections(appliedPaths);
    const affectsListing = productPatchAffectsListing(appliedPaths);
    const affectsSearch = productPatchAffectsSearch(appliedPaths);

    if (!affectsCollections && !affectsListing && !affectsSearch) {
      return {
        ok: true,
        collectionSync: null,
        indexSync: null,
    ...
    if (affectsSearch) {
      try {
        ...
        searchSync = await syncSearchAfterProductSave(localePrefix, catalogLocale, slug);
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        errors.push(`search sync: ${msg}`);
      }
    ...
    if (affectsListing || affectsSearch) {
      invalidateCatalogCaches(localePrefix, slug);
      if (opts?.oldSlug && opts.oldSlug !== slug) {
        revalidateProductSlug(localePrefix, opts.oldSlug);
      }
    ...
    await logCatalogSync({
      level: ok ? "info" : "warn",
      operation: "onProductPatched",
      locale: localePrefix,
      slug,
      durationMs: Date.now() - started,
      message: ok ? `paths: ${appliedPaths.join(",")}` : errors.join("; "),
    });
  ...
  async onCollectionChanged(allowDefer = true): Promise<CatalogSyncResult> {
    ...
    if (allowDefer && shouldDeferCatalogSync(productCount)) {
      const job = await createCatalogSyncJob("full_catalog_sync", {
        locales: [...(await getCatalogLocaleCodes())],
      });
      jobId = job.id;
      // Drop stale facets immediately so storefront rebuilds from current Category rules
      // while the deferred full sync job runs.
      invalidateProductCatalogIndex();
      invalidateProductIndexLoaderCache();
      return {
        ok: true,
    ...
    for (const locale of await getCatalogLocaleCodes()) {
      const prefix = await localePrefixFromCatalogLocale(locale);
      try {
  ...
  async onBulkImportComplete(localePrefixes: string[]): Promise<CatalogSyncResult> {
    ...
    if (useCatalogProductsDb()) {
      for (const localePrefix of unique) {
        ...
        try {
          ...
          if (await canWriteProductIndexes()) {
            await this.writeLocaleIndexesIfWritable(catalogLocale, localePrefix);
          }
        } catch (e) {
      ...
      return {
        ok: errors.length === 0,
        collectionSync: null,
        indexSync: { count: unique.length, mode: "db-memory" },
        searchSync: { upserted: true },
        errors,
        warnings: [],
      };
  ...
  async writeLocaleIndexesIfWritable(
    catalogLocale: string,
    localePrefix: string,
  ): Promise<void> {
    if (!(await canWriteProductIndexes())) return;
    const prefix = await localePrefixFromCatalogLocale(catalogLocale);
    const result = await writeLocaleProductIndexes(catalogLocale, prefix, { gzip: true });
    await updateManifest(catalogLocale, result.count, result.signature);
    invalidateCatalogCaches(localePrefix);
