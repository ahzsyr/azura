...
import { loadAllProductsFromDb } from "@/features/collections/collection-sync.service";
...
export async function countOrphanIndexEntries(
  ...
  const listingPath = join(localeIndexDir(locale), "product-listing-index.json");
...
export async function validateCatalogConsistency(
  ...
  if (useDatabaseOnlyCatalog()) {
    return validateCatalogConsistencyDb(locales);
  }
...
async function validateCatalogConsistencyDb(
  locales: string[] = ["en-us"],
): Promise<CatalogValidationReport> {
  const errors: CatalogValidationIssue[] = [];
  const warnings: CatalogValidationIssue[] = [];
  ...
  try {
    ...
    const localizedCollections = collections.filter((c) => c.visible !== false);

    for (const locale of locales) {
      const products = await loadAllProductsFromDb(locale);
      const engineProducts = products.map(({ slug, product }) =>
      ...
      for (const [k, n] of slugCounts) {
        if (n > 1) {
          warnings.push({
            level: "warn",
            code: "DUPLICATE_SLUG",
            message: `${locale}: duplicate slug "${k}" (${n} rows)`,
          });
        }
      }

      const orphans = detectOrphanProducts(engineProducts, localizedCollections);
      if (orphans.length > 0) {
        warnings.push({
          level: "warn",
          code: "ORPHAN_PRODUCTS",
          message: `${locale}: ${orphans.length} product(s) match no collection`,
        });
      }
      ...
      const { getCollectionsMatchingProduct } = await import(
        "@/features/products/product-collections"
      );
      for (const ep of engineProducts) {
        for (const m of getCollectionsMatchingProduct(ep, localizedCollections)) {
          counts.set(m.slug, (counts.get(m.slug) ?? 0) + 1);
        }
      }
      for (const col of localizedCollections) {
        if ((counts.get(col.slug) ?? 0) === 0 && col.visible !== false) {
          warnings.push({
            level: "warn",
            code: "EMPTY_COLLECTION",
            message: `Collection "${col.slug}" has no products in ${locale}`,
          });
        }
      }
      ...
      if (products.length === 0) {
        warnings.push({
          level: "warn",
          code: "NO_PRODUCTS",
          message: `${locale}: no products in database`,
        });
      }
    }
  } catch (e) {
