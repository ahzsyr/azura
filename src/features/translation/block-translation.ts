...
export const BLOCK_TRANSLATABLE_FIELDS: Partial<Record<BlockType, string[]>> = {
  ...
  contactMap: ["title", "description", "directionsText", "markerLabel", "overlayHours"],
  contactLocation: ["title", "description", "addressLine1", "addressLine2", "city", "postalCode", "country", "hours", "mapsButtonText"],
  contactPhone: ["title", "description"],
  contactSocial: ["title", "description", "responseNote"],
...
export function applyResolvedBlockCopyToProps(
  ...
  for (const field of fields) {
    ...
    // Always write, including empty string, so cleared copy does not leave stale props.
...
export function overrideMapToInputs(map: Map<string, string>): EntityTranslationInput[] {
  ...
  for (const [key, value] of map) {
    ...
    if (!parsed) continue;
    // Include empty values so sync can treat them as explicit clears.
    inputs.push({
      entityType: BUILDER_BLOCK_ENTITY_TYPE,
