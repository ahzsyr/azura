...
} from "@/features/translation/hooks/use-entity-translations";
...
type Props = {
  ...
  entityType?: string;
  entityId?: string;
  /** Server-fetched rows — seeds fields immediately and skips the client refetch */
...
function buildLocaleValues(
  ...
  indexed: ReturnType<typeof indexTranslations>,
  legacyEntity: Record<string, unknown> | undefined,
  defaultCode: string
): Record<string, string> {
  const next: Record<string, string> = {};
  for (const locale of locales) {
    const fromDb = entityId ? (indexed[fieldKey]?.[locale.code]?.value ?? "") : "";
    next[locale.code] =
      fromDb ||
      resolveAdminFieldValue(undefined, legacyEntity, fieldKey, locale.code, defaultCode);
  }
  return next;
}
...
export function AdminLocalizedFormField({
  ...
  const { translations, loading, getFieldValue } = useEntityTranslations(
    entityType ?? "",
    entityId,
    { initialRows: initialTranslations }
  );
  ...
  const [localeValues, setLocaleValues] = useState<Record<string, string>>(() =>
    buildLocaleValues(
      ...
      locales,
      entityId,
      initialTranslations ? indexTranslations(initialTranslations) : {},
      legacyEntity,
      defaultCode
    )
  );
