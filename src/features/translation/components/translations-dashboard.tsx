...
import { AdminCardGrid, AdminSettingsRibbon } from "@/components/admin/layout/admin-shell";
...
import { TranslationAiConfigPanel } from "./translation-ai-config-panel";
...
type Props = {
  ...
  aiConfig: TranslationAiConfigPublic;
};

type StatusFilter = "all" | "missing" | "draft" | "published";
type TranslationsTab = "editor" | "configuration";

const PAGE_SIZE = 25;

const TRANSLATIONS_TABS = [
  { id: "editor", label: "Editor" },
  { id: "configuration", label: "Configuration" },
] as const;

export function TranslationsDashboard({ locales, aiConfig: initialAiConfig }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<TranslationsTab>("editor");
  ...
  const handleRevertDirty = useCallback(() => {
    setDirtyCells({});
    setCellFeedback({});
    setSaveNotice(null);
    markSaved();
  ...
  const handleSaveAll = useCallback(() => {
    const cells = Object.entries(dirtyCells)
      .map(([key, dirty]) => {
        const parsed = parseTranslationCellKey(key);
  ...
  }, [dirtyCells, loadGrid, markSaved, setSaveStatus]);

  useEffect(() => {
    if (dirtyCount > 0) markUnsaved();
    else markSaved();
  }, [dirtyCount, markUnsaved, markSaved]);

  useAdminPageActions({
    ownerKey: "translations-dashboard",
    scope: "settings",
    priority: 0,
    onSave: handleSaveAll,
    onCancel: handleRevertDirty,
    saveLabel: dirtyCount > 0 ? `Save changes (${dirtyCount})` : "Save changes",
    saveStatusMode: "self-managed",
    canSave: dirtyCount > 0 && !isPending,
    canCancel: dirtyCount > 0 && !isPending,
  });
  ...
  const handleQueueAiTranslation = () => {
    ...
    startTransition(async () => {
      setAiNotice(null);
      try {
        const queued = await queueAiTranslationAction(
          effectiveAiLocale,
          aiEntityType === "all" ? undefined : aiEntityType
        );
        if (!queued.success || !queued.jobId) {
          setAiNotice(
            "error" in queued && queued.error
              ? queued.error
              : "Failed to queue AI translation."
          );
          return;
        }
        const result = await processTranslationJobsAction(queued.jobId);
        if (!result.success || result.error) {
          setAiNotice(result.error ?? "AI translation failed.");
          return;
        }
        const more =
          result.remainingFields > 0
            ? ` More fields remain — click Queue AI translation again to continue.`
            : "";
        setAiNotice(
          `Processed ${result.processedJobs} job(s), translated ${result.translatedFields} field(s).${more}`
        );
        await loadGrid();
      } catch (error) {
        setAiNotice(
          error instanceof Error ? error.message : "AI translation failed. Check server configuration."
        );
      }
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <AdminSettingsRibbon
        tabs={[...TRANSLATIONS_TABS]}
        activeTab={activeTab}
        onTabChange={(id) => setActiveTab(id as TranslationsTab)}
        layoutId="translations-settings-ribbon"
      />

      {activeTab === "configuration" ? (
        <TranslationAiConfigPanel config={aiConfig} onConfigChange={setAiConfig} />
      ) : (
      <>
      {/* Toolbar */}
      <Card>
          ...
          <Card className="border-primary/20">
            <CardHeader>
              ...
              <CardDescription>
                ...
                {!isAiConfigured
                  ? " Configure a provider under the Configuration tab first."
                  : ` Using ${aiConfig.providerLabel}.`}
              </CardDescription>
            </CardHeader>
            <CardContent className="flex flex-wrap items-end gap-4">
              ...
              <Button
                type="button"
                disabled={isPending || !effectiveAiLocale || !isAiConfigured}
