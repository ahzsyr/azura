...
test("resolveContentField treats explicit dual-write clear as empty over EntityTranslation", () => {
  const value = resolveContentField(
    { formSectionTitle: "", formSectionTitleEn: "" },
    "formSectionTitle",
    "en",
    {
      enabledLocales: LOCALES,
      defaultCode: "en",
      translations: [row("formSectionTitle", "en", "BC")],
      includeLegacySuffixFields: true,
    },
  );
  assert.equal(value, "");
...
test("resolveContentField keeps EntityTranslation when dual-write clear key is absent", () => {
  const value = resolveContentField(
    { formSectionTitle: "" },
    "formSectionTitle",
    "en",
...
test("resolveContentField treats heroTitleEn clear as empty for all contact experience fields", () => {
  for (const field of ["heroTitle", "heroDescription", "formSectionTitle", "title", "successMessage"]) {
    const value = resolveContentField(
      { [field]: "", [`${field}En`]: "" },
      field,
      "en",
      {
        enabledLocales: LOCALES,
        defaultCode: "en",
        translations: [row(field, "en", "Stale")],
        includeLegacySuffixFields: true,
      },
    );
