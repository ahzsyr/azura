...
export async function getCompletionMatrixAction() {
  ...
  const matrix = await translationService.getCompletionMatrix(nonDefault.map((l) => l.code));
  console.error(
    `[debug-5e2427] ${JSON.stringify({
      step: "getCompletionMatrixAction",
...
export async function findPriorityMissingAction(limit = 30) {
  await requireAdmin();
  const startedAt = Date.now();
  const missing = await translationService.findPriorityMissing(limit);
  console.error(
    `[debug-5e2427] ${JSON.stringify({
      step: "findPriorityMissingAction",
      ms: Date.now() - startedAt,
...
export async function scaffoldLocaleTranslationsAction(targetLocaleCode: string) {
  await requireAdmin();
  const job = await translationJobService.createJob(targetLocaleCode);
  try {
...
export async function queueAiTranslationAction(localeCode: string, entityType?: string) {
  await requireAdmin();
  const job = await translationJobService.createJob(localeCode, entityType);
  revalidatePath("/admin/translations");
