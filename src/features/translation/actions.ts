...
export async function queueAiTranslationAction(localeCode: string, entityType?: string) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to queue AI translation";
    return { success: false as const, jobId: null as string | null, error: message };
  }
}

export async function processTranslationJobsAction(jobId?: string) {
  await requireAdmin();
  try {
    const result = await aiCapability.processJobs(jobId ? { jobId } : undefined);
    if (!result.error) {
      revalidatePath("/admin/translations");
      revalidatePath("/admin/languages");
    }
...
export async function getTranslationAiConfigAction() {
  await requireAdmin();
  return translationAiConfigService.getPublic();
}

export async function upsertTranslationAiConfigAction(input: {
  provider?: string;
  apiKey?: string;
  clearApiKey?: boolean;
  model?: string;
  ...
  await requireAdmin();
  try {
    const result = await translationAiConfigService.upsert(input);
    revalidatePath("/admin/translations");
