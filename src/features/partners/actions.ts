...
export async function upsertPartnerProgram(formData: FormData) {
  ...
  const slug = slugInput
    ...
    : await uniqueSlug(prisma.partnerProgram, titleEn, id, "partners");
  ...
  for (let i = 0; i < categories.length; i++) {
    ...
    if (categoryId) {
      ...
      const created = await prisma.partnerCategory.create({ data: categoryData });
  ...
  for (let i = 0; i < partners.length; i++) {
    ...
    if (partnerId) {
      ...
      keptPartnerIds.add(partnerId);
    } else {
      const created = await prisma.partner.create({ data: partnerData });
      keptPartnerIds.add(created.id);
    }
  }
