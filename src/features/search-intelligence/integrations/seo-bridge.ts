...
import { loadMarketingGoogleAdsContext } from "@/features/seo/google-platform/marketing-ads-bridge";
...
export async function loadSeoConnectorSnapshots(): Promise<ConnectorConfigSnapshot[]> {
  const [tracking, integrations, platform, marketingGoogleAds] = await Promise.all([
    ...
    loadMarketingGoogleAdsContext().catch(() => undefined),
  ]);
