...
export function createOperationsEngine(options?: {
  ...
  initialRecords?: ExecutionRecord[];
  onChange?: (records: ExecutionRecord[]) => void | Promise<void>;
}) {
  ...
  function notifyChange() {
    ...
    persistQueue = persistQueue
      .then(() => onChange?.(snapshot))
      .catch(() => {
        /* persistence failures should not break ops */
      });
  }
  ...
  function hydrate(incoming: ExecutionRecord[]) {
  ...
  function setOnChange(handler: ((records: ExecutionRecord[]) => void | Promise<void>) | undefined) {
    onChange = handler;
  }
