...
import { revalidateLocales, revalidateAllLocaleLayoutPaths } from "@/services/cache";
...
async function invalidateLocaleLayoutsBestEffort() {
  try {
    const prefixes = await getEnabledUrlPrefixes();
    revalidateAllLocaleLayoutPaths(prefixes.length > 0 ? prefixes : undefined);
  } catch {
...
export async function upsertLocaleAction(formData: FormData): Promise<UpsertLocaleResult> {
  ...
  try {
    ...
    revalidateLocales();
    await invalidateLocaleLayoutsBestEffort();
