...
import type { FooterColumn, FooterWorkspace } from "@/features/footer/types";
import { createEmptyWorkspace, mergeHeaderWorkspaceWithTheme } from "@/features/navigation/defaults";
import { navigationService, navigationCatalogService } from "@/features/navigation/navigation.service";
...
import {
  ...
  resolveWhatsAppTemplateOverride,
...
function getLastKnownHeaderWorkspace(locale: string): HeaderWorkspace | null {
  return lastKnownHeaderWorkspaceByLocale.get(locale) ?? null;
}
...
async function loadPublicShellContextUncached(
  ...
  const needsCatalog = (footerWorkspace.columns ?? []).some(
    (c: FooterColumn) => c.menuSource === "category" || c.menuSource === "collection",
  );
  const catalog = needsCatalog
    ? await navigationCatalogService.getCatalog(locale).catch(() => null)
    : null;
  ...
  const localeCode = localeEntry?.code ?? locale;
  whatsappMessage = resolveWhatsAppTemplateOverride(
    whatsappSettings,
    "message.default",
    localeCode,
    whatsappMessage,
  ...
  whatsappAriaLabel = resolveWhatsAppTemplateOverride(
    whatsappSettings,
    "fab.ariaLabel",
    localeCode,
    whatsappAriaLabel,
...
const publicShellCacheByLocale = new Map<string, () => Promise<PublicShellContext>>();

function getCachedPublicShellContext(locale: string): Promise<PublicShellContext> {
  let loader = publicShellCacheByLocale.get(locale);
  if (!loader) {
    loader = createCached(
    ...
    publicShellCacheByLocale.set(locale, loader);
  }
  return loader();
}
...
export function createFallbackPublicShellContext(
  ...
  return {
    ...
    resolvedFooter: resolveFooter(footerWorkspace, { locale, brand: brandConfig }),
    brandConfig,
    siteIdentity,
    whatsappPhone: "",
