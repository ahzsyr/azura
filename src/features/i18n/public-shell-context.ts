...
async function safeLoad<T>(label: string, fn: () => Promise<T>, fallback: T): Promise<T> {
  try {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7876/ingest/6e6c5bde-6579-4633-b8e0-f055b7efa2da',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'7ac260'},body:JSON.stringify({sessionId:'7ac260',runId:'ui-runtime-debug',hypothesisId:'H7',location:'src/features/i18n/public-shell-context.ts:64',message:'public shell safeLoad fallback triggered',data:{label,errorName:error instanceof Error ? error.name : typeof error,errorMessage:error instanceof Error ? error.message : String(error)},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
