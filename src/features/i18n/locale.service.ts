...
export const localeService = {
  ...
  async getDefaultUrlPrefix(): Promise<string> {
    ...
    return def?.urlPrefix ?? FALLBACK_LOCALES[0]!.urlPrefix;
  },
  ...
  async getRoutingConfig(): Promise<{ locales: string[]; defaultLocale: string }> {
    ...
    const defaultLocale =
      ...
      prefixes[0] ??
      FALLBACK_LOCALES[0]!.urlPrefix;
    return {
      locales: prefixes.length > 0 ? prefixes : FALLBACK_LOCALES.map((locale) => locale.urlPrefix),
      defaultLocale,
    };
  },

  getDirection(locale: string): "ltr" | "rtl" {
    return getDirectionSync(locale, FALLBACK_LOCALES);
  },
};
