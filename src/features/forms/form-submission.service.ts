...
export async function persistFormSubmission(
  ...
  // Resolve internal campaign from first-party attribution when present
  if (
    ctx.utm?.campaign ||
    ctx.utm?.a ||
    ctx.utm?.utm_campaign ||
    ctx.utm?.visitor_token
  ) {
    try {
      const { attachLeadAttributionFromUtm, resolveCampaignFromAttributionUtm } = await import(
        "@/modules/marketing/leads/attribution-bridge"
      );
      const utmRecord = ctx.utm as Record<string, string>;
      let visitorId: string | null = null;
      if (utmRecord.visitor_token) {
        const visitor = await prisma.marketingVisitor.findUnique({
          where: { visitorToken: utmRecord.visitor_token },
          select: { id: true },
        });
        visitorId = visitor?.id ?? null;
      }
      const { campaignId } = await resolveCampaignFromAttributionUtm(utmRecord, visitorId);
      if (campaignId) {
