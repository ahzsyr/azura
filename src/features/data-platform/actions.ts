...
/** Server Actions can only return JSON-serializable values (no fns, Dates as ISO). */
...
export async function listDataSourceRecordsAction(sourceId: string, page = 1) {
  ...
  const { items, total } = await platformService.listRecords(sourceId, page);
  // Omit source (it contains query fns) and flatten Prisma Dates / nested objects.
  return toPlainJson({ items, total });
}
...
export async function inspectDataSourceRecordAction(sourceId: string, id: string) {
  ...
  const allowed = BROWSABLE_SOURCES.some((s) => s.id === sourceId);
  if (!allowed) throw new Error("Source not allowed");
  const record = await platformService.inspectRecord(sourceId, id);
  return record == null ? null : toPlainJson(record);
}

export async function searchDataSourcesAction(query: string) {
  await requireAdmin();
  if (!query || query.trim().length < 2) return [];
  return toPlainJson(await platformService.searchSources(query));
}
