...
const emptyCollections: DiagnosticCheck = {
  id: "empty-categories",
  title: "Empty content categories",
  description: "Published content categories (legacy ContentCollection) with no items assigned.",
  category: "integrity",
  severity: "info",
  deploymentNavItemId: "categories",
  run: () =>
    safeRun("empty-categories", async (start) => {
      const empty = await (prisma as unknown as {
        contentCollection: {
          findMany: (args: unknown) => Promise<Array<{ id: string; slug: string }>>;
        };
      ...
      return makeResult(
        ...
        empty.length > 0
          ? {
              status: "warn",
              message: `${empty.length} published categor${empty.length !== 1 ? "ies" : "y"} have no items`,
              count: empty.length,
          ...
          : { status: "pass", message: "All published content categories have items" },
        start
      );
    }),
};
