...
import {
  migrateLegacyInquiryFormBlocks,
  inquiryFormBlocksWereMigrated,
} from "@/features/builder/migration/migrate-legacy-inquiry-form";
...
export function migrateBlocksToBlockSystem(blocks: unknown): MigrationResult {
  ...
  const inquiryMigrated = migrateLegacyInquiryFormBlocks(stripMigrated);
  if (inquiryFormBlocksWereMigrated(stripMigrated, inquiryMigrated)) {
    warnings.push("Migrated legacy inquiryForm blocks to contactFormBuilder.");
  }

  const v2Upgraded = upgradePageBlocksToV2(inquiryMigrated);
  ...
  return {
    ...
    migrated:
      hadV1Only ||
      catalogMigrated !== raw ||
      stripMigrated !== catalogMigrated ||
      inquiryFormBlocksWereMigrated(stripMigrated, inquiryMigrated),
    version: "2.0",
    warnings,
  };
}
