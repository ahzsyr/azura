...
export function resolveBaseContentOverflow(block: BlockNode): ResolvedContentOverflow {
  ...
  // Showcase panels render via ProductRecordsOverflowLayout; layout props must
  // resolve to slider so "Carousel" in block settings is not ignored as grid.
  if (
    ...
    const layout =
      type === "taxonomyProductTabs"
        ? ((p.productLayout as string) ?? "grid")
        : type === "megaCollectionShowcase"
          ? ((p.centerLayout as string) ?? "grid")
          : ((p.layout as string) ?? "grid");
