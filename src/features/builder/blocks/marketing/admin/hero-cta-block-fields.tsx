...
import { BlockBackgroundSettings } from "@/features/builder/blocks/marketing/admin/block-background-settings";
    ...
    <div className="space-y-3">
      ...
      {hasActiveBlockVisualBackground(block) ? (
        <p className="text-xs text-amber-700 dark:text-amber-400 rounded-md border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/30 px-3 py-2">
          {(p.backgroundType as string) === "transparent"
            ? "The Look & Feel section background is still visible because the hero background is set to Transparent. To fully remove it, clear the section background in Look & Feel settings."
        ...
        <label className="flex items-start gap-2 text-sm">
          ...
          <span>
            ...
            <span className="block text-xs text-muted-foreground">
              Softly blends the image, video, solid color, or gradient into the page background at the bottom.
            </span>
          </span>
        </label>
      )}
