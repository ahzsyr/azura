...
import { clampOverlayOpacity } from "@/features/builder/blocks/marketing/lib/background-scrim";
import { BlockCtaButtons } from "@/features/builder/blocks/marketing/components/block-cta-buttons";
import { cn } from "@/lib/utils";
import { shouldShowHeroOverlay } from "@/features/builder/blocks/marketing/lib/hero-pro-overlay";
import {
  heroEntranceClass,
  isHeroEntranceEffect,
  type HeroAnimationsConfig,
} from "@/features/builder/blocks/marketing/lib/hero-animations";
import { HeroMotionClient } from "@/features/builder/blocks/marketing/components/hero-motion-client";
...
export function HeroProView({
  ...
  const isTransparentBg =
    backgroundType === "transparent" || backgroundType === "none" || Boolean(useTransparentHero);
  const effectiveSolidColor =
    backgroundType === "solid"
      ? backgroundColor?.trim() || "var(--color-primary, hsl(var(--primary)))"
      : backgroundColor;
  const hasFilledBackground =
    ...
    (hasHeroImage ||
      ...
      backgroundType === "image" ||
      backgroundType === "solid");
  const isLightHero = isTransparentBg || (!hasFilledBackground && backgroundType !== "solid");
    ...
    <>
      ...
      {subtitle && (
        <>
          ...
          <p
            className={cn(
              "max-w-xl text-lg",
              hasFilledBackground && !isTransparentBg
      ...
      {(primaryCta?.label || secondaryCta?.label) && (
        <BlockCtaButtons
          ...
          dark={hasFilledBackground && !isTransparentBg}
        />
      )}
    </>
  );
  ...
  return (
    ...
    >
      ...
      <BlockBackgroundLayer
        backgroundType={isTransparentBg ? "transparent" : backgroundType}
        imageUrl={imageUrl}
        videoUrl={videoUrl}
        backgroundColor={effectiveSolidColor}
        overlayOpacity={overlayOpacity}
        fadeIntoSiteBackground={fadeBg}
        imagePosition={imagePosition}
      ...
      />
      {hasFilledBackground && (
        <HeroAtmosphere showGlow={!useBlockVisualBg && (hasHeroImage || showHeroOverlay)} />
      )}
