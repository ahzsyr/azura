      ...
      {block && overflow ? (
        (() => {
          const renderCard = (item: GridItem) => {
            ...
            return (
              <div className={cn(cardClasses[cardVariant], "flex h-full min-h-[220px] flex-col")}>
                {item.imageUrl ? (
                ...
                <h3 className="font-heading text-lg font-semibold text-card-foreground" dir="auto">
                  {itemTitle}
                </h3>
                <div className="gold-divider my-3" />
                <p className="line-clamp-3 text-sm leading-relaxed text-card-foreground/75" dir="auto">
                  {desc}
                </p>
                {item.href && linkLabel && (
                  <Link href={item.href} className="mt-auto pt-4 inline-block text-sm font-medium text-primary hover:underline">
                    {linkLabel}
                  </Link>
                )}
              </div>
        ...
        <div className={cn("grid gap-6", columnClasses[columns])}>
          {filtered.map((item) => {
            ...
            const linkLabel = resolveItemField(item, "linkLabel", locale, itemLocaleOptions);
            return (
              <div key={item.id} className={cn(cardClasses[cardVariant], "flex h-full min-h-[220px] flex-col")}>
                {item.imageUrl ? (
                  <div className="relative mb-4 h-12 w-12 overflow-hidden rounded-lg">
                    <Image src={item.imageUrl} alt="" fill className="object-cover" sizes="48px" />
                  </div>
