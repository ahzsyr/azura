...
export function AdvancedFiltersIsland({
  locale,
  blockProps: raw,
  searchConfig,
    ...
    fetch(`/api/catalog/listing?locale=${encodeURIComponent(locale)}`)
      ...
      .then((data) => {
        if (data.facets) setFacets(data.facets as ListingFacets);
          ...
          // Empty taxonomy would emit orphan product.category strings — omit categories.
          const empty = aggregateFacets(data.records, []);
          setFacets({ ...empty, categories: [] });
        }
      })
      ...
      <div className={cn("space-y-3", p.layout === "chips" && "space-y-2")}>
        ...
        {search.showContentTypeChips && search.discoveryContentTypes?.length ? (
          <SearchFilterChips
            chips={search.discoveryContentTypes.map((ct) => ({
              id: ct.slug,
              label: getLocalizedField(ct as Record<string, unknown>, "label", locale),
        ...
        {search.enabledFilters
          ...
          .map((filter) => {
            ...
            const label = getLocalizedField(filter as Record<string, unknown>, "label", locale);
            return (
