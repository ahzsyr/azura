...
function inferFromPathname(
  ...
  if (segment === "products") {
    return {
      ...
      entityId: slug,
      title: slug,
      urlPath: publicLocalePath(locale, `/products/${slug}`),
    };
  }
  if (segment === "blog") {
    return {
      ...
      entityId: slug,
      title: slug,
      urlPath: publicLocalePath(locale, `/blog/${slug}`),
    };
  }
  if (segment === "content") {
    return {
      ...
      entityId: slug,
      title: slug,
      urlPath: publicLocalePath(locale, `/content/${slug}`),
    };
  }
  return null;
}
  ...
  useEffect(() => {
    ...
    pushRecentlyViewed(locale, {
      ...
      urlPath: payload.urlPath.startsWith("/")
        ? publicLocalePath(locale, payload.urlPath)
        : publicLocalePath(locale, `/${payload.urlPath}`),
