  ...
  if (scope === "PRODUCT") {
    ...
    return list
      ...
      .map((c) => ({
        slug: c.slug,
        name: c.name,
        href: publicLocalePath(localePrefix, `/categories/${c.slug}`),
    ...
    try {
      ...
      if (cats.length > 0) {
        return cats.map((cat) => ({
          ...
          href: `${publicLocalePath(localePrefix, "/blog")}?category=${encodeURIComponent(cat.slug)}`,
        }));
      }
    } catch {
  ...
  if (scope === "CONTENT") {
      ...
      if (ownerId) {
        ...
        if (cats.length > 0) {
          return cats.map((cat) => ({
            slug: cat.slug,
            name: cat.metadata?.name || cat.slug,
            href: `${publicLocalePath(localePrefix, "/content")}?collection=${encodeURIComponent(cat.slug)}`,
          }));
        }
      }
    } catch {
    ...
    return rows.map((col) => {
      ...
      return {
        slug: col.slug,
        name: name || col.slug,
        href: `${publicLocalePath(localePrefix, "/content")}?collection=${encodeURIComponent(col.slug)}`,
      };
    });
  }
