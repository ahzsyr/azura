import { z } from "zod";
import { dynamicFormAppearanceSchema } from "@/features/forms/fxs/appearance/dynamic-form-appearance";

const formLayoutEnum = z.enum([
  "stacked",
  "inline",
  "twoColumn",
  "responsiveGrid",
...
const columnRatioEnum = z.enum(["50/50", "40/60", "60/40", "30/70", "70/30"]);

const sectionStyleEnum = z.enum(["card", "flat", "bordered", "filled", "collapsible", "accordion"]);

/** Keep labelEn / labelAr / label_xx via passthrough for nested locale copy. */
...
export const leadFormPropsSchema = z.object({
  ...
  heroEstimatedMinutes: z.coerce.number().optional(),
  trustItems: z.array(trustItemSchema).default(defaultTrustItems),
  formSectionTitle: z.string().default(""),
  successMessage: z.string().default("Thank you! We will be in touch."),
  layout: formLayoutEnum.default("split"),
  columnRatio: columnRatioEnum.optional(),
  sectionStyle: sectionStyleEnum.optional(),
});

export const contactFormBuilderPropsSchema = z
  .object({
    title: z.string().default(""),
    templateId: z.string().default(""),
    heroTitle: z.string().default(""),
