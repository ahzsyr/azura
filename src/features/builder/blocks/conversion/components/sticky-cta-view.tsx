...
import type { StickyCtaProps } from "@/features/builder/blocks/conversion/schemas/conversion-blocks";
...
export function StickyCtaView(props: Props) {
  ...
  const message = resolveTopLevelField(propsRecord, "message", locale);
  const primaryButton = resolveTopLevelField(propsRecord, "primaryButton", locale);
  const secondaryButton = resolveTopLevelField(propsRecord, "secondaryButton", locale);
  ...
  return (
    ...
    >
      <div className={cn("mx-auto flex items-center gap-4", effectiveVariant === "fab" ? "flex-col text-center" : "max-w-5xl justify-between flex-wrap")}>
        ...
        <BlockCtaButtons
          primary={{ label: primaryButton, href: primaryHref }}
          secondary={
            secondaryButton
              ? { label: secondaryButton, href: secondaryHref }
              : undefined
          }
        />
        {dismissible && (
