...
import type { DownloadGateProps } from "@/features/builder/blocks/conversion/schemas/conversion-blocks";
...
export function DownloadGateView(props: Props) {
  const {
    locale,
    blockId,
    mediaAssetId,
  ...
  const description = resolveTopLevelField(propsRecord, "description", locale);
  const fileLabel = resolveTopLevelField(propsRecord, "fileLabel", locale);
  const unlockSuccessMessage = resolveTopLevelField(propsRecord, "successMessage", locale);
