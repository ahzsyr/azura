...
describe("VideoHeroView", () => {
  ...
  it("omits poster and marks decorative video as aria-hidden", () => {
    const html = renderToStaticMarkup(
      <VideoHeroView
        title="Hero title"
        videoUrl="/hero.mp4"
    ...
    assert.match(html, /<video[^>]*aria-hidden(?:=\{?true\}?|=\"true\")?/);
    assert.doesNotMatch(html, /<video[^>]*\sposter=/);
  });

  it("keeps poster when controls are enabled", () => {
    const html = renderToStaticMarkup(
      <VideoHeroView
        title="Hero title"
        videoUrl="/hero.mp4"
    ...
    assert.match(html, /<video[^>]*poster="\/poster\.jpg"/);
    assert.doesNotMatch(html, /<video[^>]*aria-hidden/);
  });
