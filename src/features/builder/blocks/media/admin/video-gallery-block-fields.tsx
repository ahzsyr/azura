      ...
      {source === "inline" && (
        <ModalRepeatableListEditor
          ...
          strings={{
            ...
            emptyLabel: "No videos yet. Click Add video to create one.",
            dialogTitleCreate: "Add video",
            dialogTitleEdit: "Edit video",
            saveButtonLabelCreate: "Save video",
            saveButtonLabelEdit: "Save video",
          }}
          renderSummary={(item, index) => ({
            title: ((item.title as string | undefined) ?? "").trim() || `Video ${index + 1}`,
          ...
          renderForm={(draft, onUpdate) => (
            <div className="space-y-3">
              ...
              <UrlPrimaryMediaPickerField
                ...
                onPick={({ url, mediaId }) =>
                  onUpdate({ thumbnailUrl: url, thumbnailMediaAssetId: mediaId ?? "" })
                }
              />
              <LocalizedItemFields
                fields={[
                ...
                onChange={(patch) => onUpdate(patch as Partial<VideoGalleryItem>)}
              />
            </div>
          )}
        />
