...
/** @deprecated Use elements[] via custom-html module. Kept for backward compat reads. */
export const customHtmlItemSchema = z
  .object({
    id: z.string(),
...
export const customHtmlPropsSchema = z.object({
  elements: z.array(z.object({ id: z.string(), tag: z.string() }).passthrough()).default([]),
}).passthrough();

export const richTextSectionSchema = z
  .object({
    id: z.string(),
  })
  .passthrough();
...
export const advancedRichTextPropsSchema = z.object({
  sections: z.array(richTextSectionSchema).default([]),
