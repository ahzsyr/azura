...
import type { categoryShowcasePropsSchema } from "@/features/builder/blocks/commerce/commerce-showcase/schemas/showcase-blocks";
...
function manualToShowcaseNode(
  ...
  const href = n.href.startsWith("/") ? publicLocalePath(localePrefix, n.href) : n.href;
  return {
    slug: n.id,
    name: label,
    href,
...
function enrichFromCollections(
  ...
  return nodes.map((n) => {
    ...
    return {
      ...
      iconUrl: isCollectionImageSrc(col.iconImage) ? col.iconImage : undefined,
...
export async function resolveCategoryShowcaseNodes(
  ...
  if (config.source === "categories" || (config.source as string) === "collections") {
