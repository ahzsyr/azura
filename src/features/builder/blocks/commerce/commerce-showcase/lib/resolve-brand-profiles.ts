...
import type { brandShowcasePropsSchema } from "@/features/builder/blocks/commerce/commerce-showcase/schemas/showcase-blocks";
import {
...
function resolveBrandHref(href: string, localePrefix: string, brandName: string): string {
  const fallback = publicLocalePath(localePrefix, `/brands/${brandNameToSlug(brandName)}`);
  const trimmed = href.trim();
  if (!trimmed) return fallback;
  if (trimmed.startsWith("http")) return trimmed;
  if (trimmed.startsWith(`/${localePrefix}/`) || trimmed === `/${localePrefix}`) {
    return publicLocalePath(localePrefix, trimmed.slice(localePrefix.length + 1) || "/");
  }
  if (trimmed.startsWith("/")) return publicLocalePath(localePrefix, trimmed);
  return trimmed;
}
...
export async function resolveBrandShowcaseNodes(
  ...
  if (selection === "manual") {
    nodes = (config.manualBrands ?? []).map((b) => {
      ...
      const href =
        b.href.trim() || publicLocalePath(localePrefix, `/brands/${brandNameToSlug(name)}`);
      return {
