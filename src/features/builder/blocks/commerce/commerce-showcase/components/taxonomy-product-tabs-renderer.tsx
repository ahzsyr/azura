...
import { resolveProductsForShowcaseTab } from "@/features/builder/blocks/commerce/commerce-showcase/lib/resolve-product-source";
import { resolveAutoTaxonomyTabs } from "@/features/builder/blocks/commerce/commerce-showcase/lib/resolve-category-showcase-nodes";
import { parseTaxonomyProductTabsProps } from "@/features/builder/blocks/commerce/commerce-showcase/lib/parse-block-props";
import { TaxonomyProductTabsIsland } from "@/features/builder/blocks/commerce/commerce-showcase/components/taxonomy-product-tabs-island";
...
export async function TaxonomyProductTabsRenderer({
  ...
  if (p.tabSource === "manual" && p.tabs.length > 0) {
    ...
    const { resolveBrandShowcaseNodes } = await import(
      "@/features/builder/blocks/commerce/commerce-showcase/lib/resolve-brand-profiles"
    );
    ...
    tabs = nodes.map((n) => ({
      slug: n.slug,
      label: getLocalizedField(n as Record<string, unknown>, "name", locale) || n.name,
      count: n.count,
      iconUrl: n.logoUrl,
    }));
  } else {
