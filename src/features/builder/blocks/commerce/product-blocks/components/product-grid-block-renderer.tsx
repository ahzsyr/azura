...
import { resolveProductsForBlock } from "@/features/builder/blocks/commerce/product-blocks/lib/resolve-products-for-block";
...
import { parseProductGridProps } from "@/features/builder/blocks/commerce/product-blocks/lib/parse-block-props";
import { ProductGridBlockIsland } from "@/features/builder/blocks/commerce/product-blocks/components/product-grid-block-island";
...
export async function ProductGridBlockRenderer({
  ...
  const records = p.showToolbar
    ? await resolveProductsForBlock(locale, selection)
    : [];
  const viewModels = p.showToolbar
    ? []
    : await resolveProductCardViewModelsForBlock(locale, selection, {
        templateId,
        displayOverrides,
      });
  const emptyMessage =
  ...
  if (records.length === 0 && viewModels.length === 0) {
    if (previewMode) {
      return (
  ...
  const dir = await getDirectionByPrefix(locale);
  ...
  const gridContent = p.showToolbar ? (
    ...
    <ProductRecordsOverflowLayout
      viewModels={paginateListing(viewModels, 1, p.limit as 12).items}
      localePrefix={locale}
    ...
    <ProductListingGrid
      products={paginateListing(
        viewModels.map((vm) => vm.product),
        1,
        p.limit as 12,
      ).items}
      localePrefix={locale}
    ...
    <ProductBlockCardShell locale={locale} displayOverrides={displayOverrides}>
      <div>
        ...
        <CatalogContentLayout
          dir={dir}
          className={title || subtitle || badge ? "catalog-content-layout catalog-content-layout--product-block mt-8" : "catalog-content-layout catalog-content-layout--product-block"}
