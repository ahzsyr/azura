...
export function BlockFieldEditor({
  ...
  switch (block.type) {
            ...
            >
              ...
              {galleryOptions.map((g) => (
                <option key={g.slug} value={g.slug}>
                  {resolveBuilderOptionTitle(g, activeLocaleCode)} ({g.slug}) — {g.mediaCount} item
                  {g.mediaCount === 1 ? "" : "s"}
            ...
            >
              ...
              {faqSetOptions.map((s) => (
                <option key={s.slug} value={s.slug}>
                  {resolveBuilderOptionTitle(s, activeLocaleCode)} ({s.slug}) — {s.itemCount} item
                  {s.itemCount === 1 ? "" : "s"}
      ...
      return (
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            Shows all published FAQ sets as cards. Edit covers and titles in{" "}
            <Link href="/admin/faqs" className="text-primary underline">
              Admin → FAQs
            </Link>
            .
          </p>
              ...
              >
                ...
                {testimonialCollectionOptions.map((c) => (
                  <option key={c.slug} value={c.slug}>
                    {resolveBuilderOptionTitle(c, activeLocaleCode)} ({c.slug}) — {c.itemCount} item
                    {c.itemCount === 1 ? "" : "s"}
    ...
    case "customHtml":
      return <CustomHtmlBlockFields block={block} onChange={onChange} />;

    case "catalog":
      return (
        <CatalogBlockFields block={block} onChange={onChange} contentTypeOptions={contentTypeOptions} />
      );

    case "contentList":
      return (
        <ContentBlockFields block={block} onChange={onChange} contentTypeOptions={contentTypeOptions} />
      );
    ...
    case "section":
      return <SectionBlockFields block={block} setProp={setProp} />;
    ...
    default: {
      ...
      if (isContactBlock(block.type)) {
        const entry = getContactBlock(block.type);
        if (entry?.editor) {
