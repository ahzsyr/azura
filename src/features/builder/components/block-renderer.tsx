...
import { isContactBlock } from "@/features/builder/blocks/contact/contact-block-ids";
import { ContactSectionView } from "@/features/builder/blocks/contact/components/contact-section-view";
...
} from "@/features/builder/blocks/contact/renderers";
import {
...
async function renderBlockContent(
  ...
  // Contact module — leaf renderers + composition for contactSection
  if (isContactBlock(block.type)) {
    if (block.type === "contactSection") {
      const sectionProps = lp as unknown as ContactSectionProps;
      const children = block.children ?? [];
      const leftIds = sectionProps.leftSlotIds ?? [];
      const rightIds = sectionProps.rightSlotIds ?? [];
      const leftChildren = leftIds
        .map((id) => children.find((c) => c.id === id))
        .filter((c): c is BlockNode => Boolean(c));
      const rightChildren = rightIds
        .map((id) => children.find((c) => c.id === id))
        .filter((c): c is BlockNode => Boolean(c));
      const assigned = new Set([...leftIds, ...rightIds]);
      const unassigned = children.filter((c) => !assigned.has(c.id));
      const rightAll = [...rightChildren, ...unassigned];
      ...
      return (
        <Section>
          <ContactSectionView
            props={{
              ...sectionProps,
              title: loc("title") || sectionProps.title,
              subtitle: loc("subtitle") || sectionProps.subtitle,
            }}
            left={leftChildren.map((child) => (
              <RenderBlock key={child.id} block={child} ctx={ctx} />
            ))}
            right={rightAll.map((child) => (
              <RenderBlock key={child.id} block={child} ctx={ctx} />
            ))}
          />
        </Section>
    ...
    const leaf =
      block.type === "contactMap" ? (
        <ContactMapBlockRenderer props={lp} />
      ) : block.type === "contactLocation" ? (
        <ContactLocationBlockRenderer props={lp} />
      ) : block.type === "contactPhone" ? (
        <ContactPhoneBlockRenderer props={lp} />
      ) : block.type === "contactSocial" ? (
        <ContactSocialBlockRenderer props={lp} />
      ) : null;

    return leaf ? <Section>{leaf}</Section> : null;
  }
  ...
  switch (block.type) {
    case "hero": {
      ...
      return (
        <HeroProView
          ...
          fadeIntoSiteBackground={Boolean(p.fadeIntoSiteBackground)}
          primaryCta={
    ...
    case "image": {
      const url = (p.url as string) || (p.imageUrl as string) || "";
      const imageSrc = url ? normalizeLocalMediaUrl(normalizeRemoteImageUrl(url) ?? url) : "";
      ...
      const descriptionSizeClass =
        (p.descriptionSize as string) === "sm"
          ...
          : (p.descriptionSize as string) === "lg"
            ? "text-lg"
            : "text-base";
      const imageRadius = block.styles?.borderRadius;
      const imageFrameStyle: CSSProperties = {};
      if (imageRadius !== undefined) {
        imageFrameStyle.borderRadius =
          typeof imageRadius === "number" ? `${imageRadius}px` : imageRadius;
      }
      const layoutLayers = [
        block.styles,
      ...
      const hasAuthoredImageWidth = layoutLayers.some(
        (layer) =>
      ...
      const hasCopy =
        Boolean(imageTitle) ||
        Boolean(loc("badge")) ||
        Boolean(loc("subtitle")) ||
        Boolean(imageDescription);
      // Banner-style image blocks should hug the photo; default section padding
      // otherwise leaves a large empty gap under the scaled image.
      const hugImage = !hasCopy;
      if (!url && !imageTitle) return null;
      return (
        <Section suppressPadding={hugImage} suppressAtmosphere={hugImage}>
          {imageTitle && (
            <SectionHeader
              badge={loc("badge") || undefined}
              title={imageTitle}
          ...
          {url && (
            <div
              className={cn(
                "relative mx-auto w-full min-w-0 overflow-hidden leading-none",
                hasAuthoredImageWidth || hugImage ? "max-w-full" : "max-w-4xl",
                imageRadius === undefined && "rounded-xl",
              )}
    ...
    case "faqSetGrid": {
      ...
      const columnsRaw = Number(p.columns ?? 3);
      const columns = (columnsRaw === 2 || columnsRaw === 4 ? columnsRaw : 3) as 2 | 3 | 4;
      return (
        <Section>
          {Boolean(loc("title")) && (
            <SectionHeader title={loc("title")} subtitle={loc("subtitle") || undefined} />
          )}
          <FaqSetGrid faqSets={faqSets} locale={locale} columns={columns} />
        </Section>
      );
    }
      ...
      return (
        ...
        >
          <SectionLayoutView
            ...
            autoplay={sectionProps.autoplay}
