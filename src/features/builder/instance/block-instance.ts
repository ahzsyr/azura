...
function isLocaleDualWriteKey(key: string): boolean {
  return /(?:En|Ar)$/.test(key) || /_[a-z]{2}(?:-[a-z]+)?$/i.test(key);
}
...
export function getBlockSettings(block: BlockNode): Record<string, unknown> {
  ...
  for (const key of Object.keys(settings)) {
    ...
    if (isUnsetSettingValue(settingVal) && !isUnsetSettingValue(propVal)) {
      // Keep intentional empty clears on locale dual-write keys (fieldEn / field_ar).
      if (settingVal === "" && isLocaleDualWriteKey(key)) continue;
      merged[key] = propVal;
    }
  }
