...
describe("block instance", () => {
  ...
  it("preserves image block media settings through upgrade pipeline", () => {
    const block: BlockNode = {
      id: "img1",
      type: "image",
      version: "2.0",
    ...
    const norm = normalizeBlockInstance(block);
    assert.equal(norm.settings.url, "https://cdn.example/image.jpg");
    assert.equal(norm.settings.mediaAssetId, "cmq56fhnv0000150467cng4pm");
    const upgraded = upgradePageBlocksToV2([block]);
    assert.equal(upgraded[0]?.settings?.url, "https://cdn.example/image.jpg");
    assert.equal(upgraded[0]?.props?.url, "https://cdn.example/image.jpg");
  });
});
