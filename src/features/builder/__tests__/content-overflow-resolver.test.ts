...
describe("content-overflow-resolver", () => {
  ...
  it("taxonomyProductTabs productLayout carousel resolves to slider", () => {
    const b = block({
      type: "taxonomyProductTabs",
      props: { productLayout: "carousel" },
    });
    const base = resolveBaseContentOverflow(b);
    assert.equal(base.effectiveMode, "slider");
    assert.equal(base.sliderEnabled, true);
  ...
  it("taxonomyProductTabs productLayout grid stays grid", () => {
    const b = block({
      type: "taxonomyProductTabs",
      props: { productLayout: "grid" },
    });
    const base = resolveBaseContentOverflow(b);
    assert.equal(base.effectiveMode, "grid");
    assert.equal(base.sliderEnabled, false);
  });

  it("productShowcase layout carousel resolves to slider", () => {
    const b = block({
  ...
  it("megaCollectionShowcase centerLayout carousel resolves to slider", () => {
    const b = block({
      type: "megaCollectionShowcase",
      props: { centerLayout: "carousel" },
    });
    const base = resolveBaseContentOverflow(b);
    assert.equal(base.effectiveMode, "slider");
    assert.equal(base.sliderEnabled, true);
