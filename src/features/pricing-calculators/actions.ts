...
export async function upsertPricingCalculator(formData: FormData) {
  ...
  const slug = slugInput
    ...
    : await uniqueSlug(prisma.pricingCalculator, titleEn, id, "calculator");
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    const fieldType = Object.values(PricingCalculatorFieldType).includes(typeRaw as PricingCalculatorFieldType)
    ...
    if (fieldId) {
      ...
      const created = await prisma.pricingCalculatorField.create({ data: fieldData });
