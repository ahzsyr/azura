...
import type { CategoryShowcaseNode } from "@/features/commerce-showcase/lib/resolve-category-showcase-nodes";
...
function nodeToCard(node: CategoryShowcaseNode, locale: Locale) {
  return {
    ...
    description: node.descriptionEn || node.descriptionAr,
    count: node.count,
    featured: false,
  };
}
    ...
    <ul className={cn(depth > 0 && "ml-4 border-l pl-3")}>
      {nodes.map((node) => (
        <li key={node.slug} className="py-1">
          <Link href={node.href} className="text-sm font-medium hover:text-primary">
            {node.name}
            {node.count != null ? (
