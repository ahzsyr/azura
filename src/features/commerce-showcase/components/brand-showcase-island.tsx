...
import type { BrandShowcaseNode } from "@/features/commerce-showcase/lib/resolve-brand-profiles";
...
import { ShowcaseCardGrid } from "@/features/commerce-showcase/components/showcase-card-grid";
import { ShowcaseSliderShell } from "@/features/commerce-showcase/components/showcase-slider-shell";
import { BrandLogoTrack } from "@/features/commerce-showcase/components/brand-logo-track";
...
export function BrandShowcaseIsland({ locale, nodes, blockProps: raw }: Props) {
  ...
  const cards = filtered.map((n) => ({
    slug: n.slug,
    name: locale.startsWith("ar") ? n.nameAr || n.name : n.nameEn || n.name,
    href: n.href,
    imageUrl: n.bannerUrl || n.logoUrl,
    iconUrl: n.logoUrl,
    description: locale.startsWith("ar") ? n.descriptionAr : n.descriptionEn,
    count: n.count,
    featured: n.featured,
  }));
              ...
              {featured.logoUrl ? (
                <div className="relative h-16 w-32">
                  <Image
                    ...
                    alt=""
                    fill
                    className={cn("object-contain", p.grayscale && "grayscale")}
                  />
                </div>
              ) : null}
              <div>
                ...
                {featured.descriptionEn ? (
                  <p className="text-sm text-muted-foreground mt-1 max-w-xl">{featured.descriptionEn}</p>
                ) : null}
                  ...
                  {c.iconUrl ? (
                    <div className="relative h-12 w-full">
                      <Image
                        ...
                        alt={c.name}
                        fill
                        className={cn("object-contain", p.grayscale && "grayscale opacity-80")}
                      />
                    </div>
                  ) : (
