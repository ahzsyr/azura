...
import type { BrandOverride, BrandSelection } from "@/features/commerce-showcase/schemas/showcase-blocks";
...
export function mergeBrandWithOverrides(
  ...
  const nameEn = override.nameEn?.trim() || node.nameEn;
  const nameAr = override.nameAr?.trim() || node.nameAr;
  const name = nameEn || nameAr || node.name;
  return {
    ...node,
    name,
    nameEn,
    ...
    descriptionEn: override.descriptionEn?.trim() || node.descriptionEn,
    descriptionAr: override.descriptionAr?.trim() || node.descriptionAr,
    href: override.href?.trim() || node.href,
  };
}

export function slugFromBrandName(nameEn: string, nameAr: string, currentSlug: string): string {
  const name = nameEn.trim() || nameAr.trim();
  if (!name) return currentSlug;
  return brandNameToSlug(name) || currentSlug;
}
