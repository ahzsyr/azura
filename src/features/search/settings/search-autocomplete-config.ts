import type { SearchAutocompleteSettings } from "@/features/search/settings/admin-search-settings.schema";
  ...
  return {
    ...
    suggestDebounceMs:
      typeof raw.suggestDebounceMs === "number"
        ? Math.min(800, Math.max(80, raw.suggestDebounceMs))
        : typeof raw.debounceMs === "number"
          ? Math.min(800, Math.max(80, raw.debounceMs))
