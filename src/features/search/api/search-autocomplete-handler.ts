...
import { getTrendingSearchQueries } from "@/features/search/analytics/search-query-stats.service";
...
export async function handleSearchAutocomplete(
  ...
  const types = searchQueryBuilder.parseTypesParam(params.types ?? null);
  const contentTypeSlugs = searchQueryBuilder.parseContentTypeSlugsParam(
    params.contentTypeSlugs ?? null
  );
  const kinds = searchQueryBuilder.parseKindsParam(params.kinds ?? null);
  const facetFilters = searchQueryBuilder.parseFacetsParam(params.facets ?? null);
...
function mapResult(
  ...
  return {
    id: r.id,
    entityId: "entityId" in r && r.entityId ? String(r.entityId) : r.id,
    title: r.title,
    snippet: r.snippet,
    urlPath: r.urlPath,
