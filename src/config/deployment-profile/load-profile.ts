...
export function isAdminNavItemEnabled(navItemId: string): boolean {
  if (!profile.generated) return true;
  if (profile.enabledNavItemIds.includes(navItemId)) return true;
  // Categories unification: accept either id during profile cutover
  if (navItemId === "categories" && profile.enabledNavItemIds.includes("collections")) return true;
  if (navItemId === "collections" && profile.enabledNavItemIds.includes("categories")) return true;
  return false;
}
