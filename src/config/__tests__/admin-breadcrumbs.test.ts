...
describe("admin chrome breadcrumbs", () => {
  ...
  it("uses SEO operator labels for the dashboard", () => {
    const crumbs = getBreadcrumbs("/admin/seo");
    assert.ok(crumbs.some((crumb) => crumb.label === "SEO"));
    assert.ok(crumbs.some((crumb) => crumb.label === "Dashboard"));
  });

  it("keeps Search Operations children under one breadcrumb parent", () => {
    const crumbs = getBreadcrumbs("/admin/seo/search-operations/pages");
    assert.ok(crumbs.some((crumb) => crumb.label === "Search Operations"));
    assert.ok(crumbs.some((crumb) => crumb.label === "Pages"));
  });
});
