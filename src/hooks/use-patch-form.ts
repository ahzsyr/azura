...
function hasActionablePageActions(actions?: PageActions): boolean {
...
export function usePatchFormState<T extends Record<string, unknown>>(
  ...
  const pageActions = pickPageActions(options);
  const hasSave = Boolean(options?.onSave || options?.onSavePatch);
  const adminForm = useAdminFormState(
    options && (hasSave || hasActionablePageActions(pageActions))
      ? {
          ...pageActions,
          onSave: hasSave ? dispatchSave : pageActions?.onSave,
          ownerKey: options.ownerKey ?? "patch-form",
          scope: options.scope,
