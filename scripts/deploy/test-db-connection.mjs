...
try {
  ...
  if (message.includes("Can't reach database server")) {
    console.error("- Hostinger MySQL: use localhost in DATABASE_URL, or set HOSTINGER_MYSQL_LOCALHOST=1 in hPanel");
    console.error("- hPanel → Databases → MySQL → use host localhost (not srv*.hstgr.io) for apps on the same server");
  }
