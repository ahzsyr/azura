...
function refreshMiddlewareManifest() {
  const result = spawnSync("npm", ["run", "middleware:manifest"], {
    stdio: "inherit",
    shell: process.platform === "win32",
    env: process.env,
  });
  if ((result.status ?? 1) !== 0) {
    console.warn("[prebuild] middleware manifest generation failed; continuing with bundled fallback.");
  }
}

refreshMiddlewareManifest();
