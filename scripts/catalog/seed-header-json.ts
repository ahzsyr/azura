/**
 * Seeds header workspace from src/data/header.json into JsonStore.
...
async function main() {
  ...
  const path = join(process.cwd(), "src", "data", "header.json");
  ...
  console.log(
    force
      ? "Overwrote header workspace from src/data/header.json"
      : "Seeded header workspace from src/data/header.json",
  );
}
