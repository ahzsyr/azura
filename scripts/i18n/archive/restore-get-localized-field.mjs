/**
 * Restore getLocalizedField(field, ...) calls stripped by fix-orphan-strings.mjs
 */
...
for (const file of walk(ROOT)) {
  ...
  const headCalls = [...head.matchAll(/getLocalizedField\([^)]+\)/g)].map((m) => m[0]);
  const currentCalls = [...current.matchAll(/getLocalizedField\([^)]+\)/g)].map((m) => m[0]);
...
console.log(`Restored getLocalizedField calls in ${fixed} files.`);
