...
| Layer | Concepts | Role |
|-------|----------|------|
| **Data** | Entity, Category | Structured knowledge |
...
| | |
|---|---|
| **Definition** | Canonical taxonomy/grouping object for Entities within a scope (PRODUCT, CONTENT, POST, KNOWLEDGE, PARTNER, TESTIMONIAL). Supports hierarchy, manual membership, Matching Rules, visibility, navigation, presentation, sorting, SEO, and localization. |
| **Owner** | `features/categories/` |
| **Responsibilities** | Membership (manual + rule-derived index), sort order, visibility, hierarchy (parent/child), Matching Rules |
| **Not** | A Page section; not a second concept alongside “Collection” |
| **Storage** | Target: unified `Category` + `CategoryMembership` (materialized index). **Today (migration):** `ContentCollection`, `CatalogCollection`, `PostCategory`, portal category tables, `TestimonialCollection` |

### Collection (legacy)

| | |
|---|---|
| **Definition** | Former name for Category. User-facing and new code must use **Category**. |
| **Status** | Transitional until Categories unification Stage 7 |
| **Storage** | Same as Category during dual-write |
