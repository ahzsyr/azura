...
English is the **default locale** with `localePrefix: "as-needed"`: public URLs omit `/en` (e.g. homepage `https://brt-me.com/`, about `/about`). Non-default locales keep a prefix (`/ar/about`). Legacy `/en` and `/en/...` URLs **308** to unprefixed paths; middleware **rewrites** unprefixed requests internally to `[locale]` routes without changing the browser URL.

| Input | Source |
|---|---|
| `NEXT_PUBLIC_SITE_URL` | Base origin |
| Locale prefix | `LocaleConfig.urlPrefix` (omitted for default locale in public URLs) |
| Path | Route-relative path |
| `LocalizedSlug` | Per-locale slug overrides |
| `canonicalUrl` | Optional override on `SeoMeta` (legacy `/en` overrides normalized at read time) |

**`publicLocalePath()`** — single helper for public hrefs, sitemap locs, and IndexNow URLs.  
**`buildCanonicalUrl()`** — default canonical when no override is set (English home → `{origin}/`).  
**`buildHreflangAlternates()`** — `languages` map + `x-default` → unprefixed default locale home (`/`).
