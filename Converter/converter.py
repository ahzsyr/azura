...
store pages, MikroTik product pages, Inrico jext CMS pages, converter/product
JSON (object or array), WooCommerce CSV, JSON-LD Product schema, or generic
product HTML into WooCommerce CSV + companion JSON (one pair per product, same
as HTML inputs).

Usage:
    python converter.py input/ -o output/
    python converter.py input/IRC380 -o output/
    python converter.py input/unifi-U7-Pro-XG -o output/ --format unifi
    python converter.py input/mikrotik-product-page-sample -o output/ --format mikrotik
    python converter.py input/catalog.csv -o output/
    python converter.py input/products.json -o output/
...
try:
    ...
    from specifications.normalizer import apply_spec_normalization  # type: ignore
...
def resolve_page_base_url(html_content: str) -> str:
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            if origin:
                return origin
    if "inricosolutions.com" in html_content.lower():
        return "https://www.inricosolutions.com"
    return BASE_URL
...
def _looks_like_csv(text: str) -> bool:
    """True when the first line looks like WooCommerce / converter CSV headers."""
    first = _first_nonempty_line(text)
    if not first or ("," not in first and "\t" not in first):
        return False
    lower = first.lower()
    hits = sum(1 for marker in WOO_CSV_HEADER_MARKERS if marker in lower)
    return hits >= 2


def detect_input_format(content: str, path: Optional[Path] = None) -> Literal["json", "csv", "html"]:
    if path is not None and path.name.lower().endswith(".csv"):
        return "csv"
    ...
    if stripped.startswith(("{", "[")):
        try:
            ...
            pass
    if _looks_like_csv(content[:8192]):
        return "csv"
    return "html"
...
def _json_product_records(data: Any) -> List[Dict[str, Any]]:
    """Unwrap a product object, a list of products, or `{products: [...]}`."""
    if isinstance(data, list):
        return [item for item in data if isinstance(item, dict)]
    if isinstance(data, dict):
        nested = data.get("products")
        if isinstance(nested, list) and not data.get("productTitle") and not data.get("title"):
            return [item for item in nested if isinstance(item, dict)]
        return [data]
    return []


def _map_json_image(img: Any, base_url: Optional[str] = None) -> Optional[Dict[str, Any]]:
    if isinstance(img, str):
        url = make_absolute_url(img, base_url)
        return {"url": url, "alt": ""} if url else None
    if not isinstance(img, dict):
        return None
    url = make_absolute_url(str(img.get("url") or ""), base_url)
    if not url:
        return None
    entry: Dict[str, Any] = {"url": url, "alt": img.get("alt", "")}
    for key in ("color", "type", "width", "height", "source"):
        if img.get(key) not in (None, ""):
            entry[key] = img[key]
    return entry


def _map_json_images(images: Any, base_url: Optional[str] = None) -> List[Dict[str, Any]]:
    mapped: List[Dict[str, Any]] = []
    seen: Set[str] = set()
    for img in _as_list(images):
        entry = _map_json_image(img, base_url)
...
def _map_json_videos(videos: Any) -> List[Any]:
    mapped: List[Any] = []
    for item in _as_list(videos):
        if isinstance(item, str) and item.strip():
            mapped.append(item.strip())
        elif isinstance(item, dict) and (item.get("url") or item.get("src")):
            mapped.append(item)
    return mapped
...
def _infer_output_format(data: Dict[str, Any]) -> str:
    explicit = str(data.get("output_format") or "").strip().lower()
    if explicit in ("unifi", "mikrotik"):
        return explicit
    if data.get("included_parts") or data.get("wireless_tables") or data.get("ethernet_test"):
        return "mikrotik"
    page = _as_dict(data.get("page_display"))
    if page.get("modelViewer") or page.get("tabInstallation") or page.get("tabInBox"):
        return "unifi"
    media = _as_dict(data.get("media"))
    model = media.get("3d_model")
    if isinstance(model, dict) and model.get("url"):
        return "unifi"
    for block in _as_list(data.get("detailed_description")):
        if isinstance(block, dict) and block.get("tab"):
            return "unifi"
    return ""


def _variations_from_json(
    data: Dict[str, Any],
) -> Tuple[List[str], str, List[str], str, List[Dict[str, Any]]]:
    """Return (plugs, default_plug, colors, default_color, color_variants)."""
    plug_options = _as_list(data.get("plug_options") or data.get("available_plugs"))
    available_plugs = [str(p).upper() for p in plug_options if str(p).upper() in ("EU", "UK", "US")]
    if not available_plugs and plug_options:
        available_plugs = [str(p) for p in plug_options if p]
    default_plug = str(data.get("default_plug") or "")
    available_colors = [str(c) for c in _as_list(data.get("available_colors")) if c]
    default_color = str(data.get("default_color") or "")
    color_variants: List[Dict[str, Any]] = [
        v for v in _as_list(data.get("color_variants")) if isinstance(v, dict)
    ]

    for var in _as_list(data.get("variations")):
        if not isinstance(var, dict):
            continue
        type_name = str(var.get("type") or "").strip().lower()
        options = [str(o) for o in _as_list(var.get("options")) if o]
        default = str(var.get("default") or "")
        if type_name == "plug" and options:
            available_plugs = available_plugs or options
            default_plug = default_plug or default
        elif type_name == "color" and options:
            available_colors = available_colors or options
            default_color = default_color or default

    skip_attr = {"sku", "condition", "price", "old_price", "images", "files", "3d_model", "plug"}
    if not color_variants:
        for combo in _as_list(data.get("variation_combinations")):
            if not isinstance(combo, dict):
                continue
            color = str(combo.get("Color") or combo.get("color") or "").strip()
            plug = str(combo.get("Plug") or combo.get("plug") or "").strip()
            if plug and plug not in available_plugs:
                available_plugs.append(plug)
            if not color:
                continue
            ...
            attrs = {
                ...
                if str(k).lower() not in skip_attr and v not in (None, "", [], {})
            }
            color_variants.append({
                "color": color,
                "sku": _strip_sku_prefix(str(combo.get("sku") or "")),
                "price": combo.get("price"),
                "images": _map_json_images(combo.get("images")),
                "attributes": attrs,
            })
    else:
        for combo in _as_list(data.get("variation_combinations")):
    ...
    if not default_color and available_colors:
        default_color = available_colors[0]
    if not default_plug and available_plugs:
        default_plug = available_plugs[0]
    return available_plugs, default_plug, available_colors, default_color, color_variants


def normalize_from_json(data: Dict[str, Any]) -> Dict[str, Any]:
    """Convert product JSON (template, UniFi, MikroTik, or sample.json) into the unified schema."""
    media = _as_dict(data.get("media"))
    images = _map_json_images(media.get("images") or data.get("images"))
    files: List[Dict[str, str]] = []
    for f in _as_list(media.get("files")) or _as_list(data.get("documents")):
        if not isinstance(f, dict):
            continue
        url = make_absolute_url(str(f.get("url") or ""))
        if url:
            files.append({"title": str(f.get("title") or ""), "url": url})
    ...
    for block in _as_list(data.get("detailed_description") or data.get("desc_blocks")):
        if not isinstance(block, dict):
            continue
        heading = block.get("heading")
        raw_text = (block.get("text") or "").strip()
        text = None if _is_json_blob(raw_text) else (raw_text or None)
        block_media: List[Dict[str, Any]] = []
        for key in ("media", "images"):
            for m in _as_list(block.get(key)):
                entry = _map_json_image(m)
                if entry:
                    if "width" not in entry:
                        entry["width"] = 0
                    if "height" not in entry:
                        entry["height"] = 0
                    block_media.append(entry)
        img = block.get("image")
        mapped_img = _map_json_image(img) if img else None
        if mapped_img:
            mapped_img.setdefault("width", 0)
            mapped_img.setdefault("height", 0)
            block_media.append(mapped_img)
        videos = _map_json_videos(block.get("videos"))
        features = [f for f in _as_list(block.get("features")) if isinstance(f, dict) or f]
        model_3d = block.get("model_3d")
        if heading or text or block_media or videos or features or model_3d:
            entry = {
                "heading": heading,
                "text": text,
                "media": block_media,
            ...
            if model_3d:
                entry["model_3d"] = model_3d
            desc_blocks.append(entry)

    price_obj = _as_dict(data.get("price"))
    regular_price = str(price_obj.get("value", "") if price_obj else data.get("regular_price") or "")
    stock = 0
    try:
        stock = int(data.get("stock") or 0)
    except (TypeError, ValueError):
        stock = 0
    if stock <= 0 and str(data.get("stock_status") or "").lower() in ("in_stock", "instock"):
        stock = 3

    available_plugs, default_plug, available_colors, default_color, color_variants = (
        _variations_from_json(data)
    )

    brand_paths = [str(p) for p in _as_list(data.get("brandPaths") or data.get("brand_paths")) if p]
    category_paths = [
        str(p) for p in _as_list(data.get("categoryPaths") or data.get("category_paths")) if p
    ]
    cats = _as_list(data.get("categories"))
    brand_name = data.get("brand", "")
    brand_path_titles = list(_as_list(data.get("brandCategories") or data.get("brand_path_titles")))
    if not brand_path_titles and cats:
        brand_path_titles = cats[:-1] if len(cats) > 1 else []
    category_root = cats[-1] if cats else ""
    cat_path_titles = list(_as_list(data.get("storeCategories") or data.get("cat_path_titles")))
    cat_leaf = data.get("category") or data.get("cat_leaf") or ""

    reviews_data = _as_dict(data.get("reviews"))
    model_3d = media.get("3d_model")
    if not isinstance(model_3d, dict):
        model_3d = data.get("model_3d") if isinstance(data.get("model_3d"), dict) else {}
    has_3d_model = bool(isinstance(model_3d, dict) and model_3d.get("url")) or bool(media.get("3d_model"))

    product: Dict[str, Any] = {
        "id": str(data.get("id", "")),
        "uid": data.get("getic_uid") or data.get("uid") or data.get("slug") or data.get("id", ""),
        "title": data.get("productTitle") or data.get("title") or data.get("name") or "",
        "title_extended": data.get("title_extended") or "",
        "short_description": data.get("short_description") or data.get("description") or "",
        "brand": brand_name,
        "mpn": data.get("mpn") or data.get("manufacturer_part_number") or "",
        "ean": data.get("ean", ""),
        "warranty": str(data.get("warranty") or "").replace(" months", "").strip(),
        ...
        "reviews_details": _as_dict(reviews_data.get("breakdown") or reviews_data.get("details")),
        ...
        "has_3d_model": has_3d_model,
        "currency": price_obj.get("currency") or data.get("currency") or "USD",
        "old_price": data.get("old_price"),
        "condition_options": _as_list(data.get("condition_options")) or ["new"],
        "default_plug": default_plug,
        "image_gallery": _as_list(data.get("image_gallery")),
        "thumbnails": _as_list(media.get("thumbnails") or data.get("thumbnails")),
        "videos": _map_json_videos(media.get("videos") or data.get("videos")),
        "model_3d": model_3d if isinstance(model_3d, dict) else {},
        "color_variants": color_variants,
        "available_colors": available_colors,
        "default_color": default_color,
        "output_format": _infer_output_format(data),
        "slug": data.get("slug") or "",
        "mainCategory": data.get("mainCategory") or "",
        "matchingRules": _as_list(data.get("matchingRules")),
    }
    if brand_paths:
    ...
    if data.get("source_specifications"):
        product["source_specifications"] = data["source_specifications"]
    if data.get("canonical_specs"):
        product["canonical_specs"] = data["canonical_specs"]
    if not product.get("canonical_specs"):
...
# ---------------------------------------------------------------------------
# MikroTik product page parser (Livewire / mikrotik.com)
# ---------------------------------------------------------------------------

MIKROTIK_HTML_MARKERS = (
    "mikrotik.com",
    "cdn.mikrotik.com",
    'wire:name="pages.product-page"',
    'wire:name="widgets.product-specification"',
    "widgets.product-details",
)


def _is_mikrotik_html(content: str) -> bool:
    lower = content.lower()
    has_host = "mikrotik.com" in lower or "cdn.mikrotik.com" in lower
    has_widget = (
        "pages.product-page" in lower
        or "widgets.product-specification" in lower
        or "widgets.product-details" in lower
    )
    return has_host and has_widget


def _mtk_widget(soup: BeautifulSoup, name: str):
    return soup.find(attrs={"wire:name": name})


def _mtk_text(el) -> str:
    if el is None:
        return ""
    return re.sub(r"\s+", " ", el.get_text(" ", strip=True)).strip()


def _mtk_parse_price(text: str) -> str:
    if not text:
        return ""
...
def _mtk_abs(url: str, base_url: str) -> str:
    return make_absolute_url(url, base_url or "https://mikrotik.com")


def _mtk_decode_html_entities(text: str) -> str:
    return html.unescape(text or "")


def _mtk_strip_tags(raw: str) -> str:
    if not raw:
        return ""
    return BeautifulSoup(_mtk_decode_html_entities(raw), "html.parser").get_text(" ", strip=True)


def _mtk_extract_hero_background(soup: BeautifulSoup, content: str) -> str:
    for section in soup.select("div.section"):
        style = section.get("style") or ""
        m = re.search(r"url\(['\"]?(https?://[^'\")\s]+)['\"]?\)", style)
        if m and "backgrounds" in m.group(1):
            return m.group(1)
    m = re.search(
        r"url\(['\"]?(https://cdn\.mikrotik\.com/[^'\"]+SXTsq[^'\"]+_bg\.webp)['\"]?\)",
        content,
        re.I,
    )
...
def _mtk_parse_product_details(soup: BeautifulSoup, base_url: str) -> Dict[str, Any]:
    root = _mtk_widget(soup, "widgets.product-details")
    out: Dict[str, Any] = {
        "title": "",
        "short_description": "",
        "images": [],
    ...
    if not root:
        h1 = soup.find("h1")
        out["title"] = _mtk_text(h1)
        return out

    h1 = root.find("h1")
    out["title"] = _mtk_text(h1)
    para = root.find("p")
    out["short_description"] = _mtk_text(para)
    img = root.find("img")
    if img and img.get("src"):
        out["images"].append({
            "url": _mtk_abs(img["src"], base_url),
            "alt": img.get("alt") or out["title"],
        })
    compare = root.find(attrs={"x-data": re.compile(r"compareProductsTrigger")})
    if compare:
        xd = compare.get("x-data") or ""
        m = re.search(r"compareProductsTrigger\('([^']+)'\)", xd)
        if m:
            out["mpn"] = m.group(1)
    return out


def _mtk_parse_feature_list(soup: BeautifulSoup) -> List[Dict[str, str]]:
    root = _mtk_widget(soup, "widgets.feature-list")
    if not root:
        return []
    features: List[Dict[str, str]] = []
    for row in root.select(":scope > div"):
        label = _mtk_text(row)
        if label:
            features.append({"label": label, "title": label})
    return features


def _mtk_parse_feature_grid(soup: BeautifulSoup) -> List[Dict[str, str]]:
    root = _mtk_widget(soup, "widgets.feature-grid")
    if not root:
        return []
    cards: List[Dict[str, str]] = []
    for col in root.select(":scope > div"):
        title_el = col.find(class_=re.compile(r"uppercase"))
        desc_el = col.find(class_=re.compile(r"font-thin|mtk-text-sm"))
        title = _mtk_text(title_el)
        desc = _mtk_text(desc_el)
        if title or desc:
            cards.append({"title": title, "description": desc, "body": desc, "label": title})
    return cards


def _mtk_parse_videos(soup: BeautifulSoup) -> List[Dict[str, str]]:
    root = _mtk_widget(soup, "widgets.product-video")
    if not root:
        return []
    videos: List[Dict[str, str]] = []
    for a in root.find_all("a", href=True):
        ...
        if img and img.get("src"):
            poster = img["src"]
        videos.append({"url": href, "type": "youtube", "poster": poster})
    return videos


def _mtk_parse_gallery(soup: BeautifulSoup, base_url: str, title: str) -> List[Dict[str, str]]:
    root = _mtk_widget(soup, "widgets.product-gallery")
    images: List[Dict[str, str]] = []
    seen: Set[str] = set()
    if not root:
        return images
    main = root.find(attrs={"x-ref": "emblaMain"}) or root
    for img in main.find_all("img"):
        src = img.get("src") or ""
        if not src or "_ts." in src or "_tm." in src:
            continue
        # Prefer lg/xl product renders
        if "rb_images" not in src and "cdn.mikrotik.com" not in src:
            continue
        url = _mtk_abs(src, base_url)
        if url in seen or _is_skip_image(url):
            continue
        seen.add(url)
        images.append({"url": url, "alt": img.get("alt") or title})
    return images


def _mtk_parse_spec_groups(spec_root) -> Tuple[List[Dict[str, Any]], str, List[Dict[str, str]]]:
    """Return (specifications, note, included_parts)."""
    specifications: List[Dict[str, Any]] = []
    note = ""
    included_parts: List[Dict[str, str]] = []
    if not spec_root:
        return specifications, note, included_parts

    for block in spec_root.select("div.flex.gap-8.flex-wrap"):
        label_el = block.find("div", class_=re.compile(r"uppercase"))
        label = _mtk_text(label_el)
        label_norm = label.lower()

        if label_norm == "note":
            note_el = block.find("div", class_=re.compile(r"basis-full|font-light"))
            # Prefer the second content column
            cols = block.find_all("div", recursive=False)
            if len(cols) >= 2:
                note = _mtk_text(cols[1])
            elif note_el:
                note = _mtk_text(note_el)
            continue

        if label_norm == "included parts":
            for btn in block.select("button"):
                img = btn.find("img")
                span = btn.find("span")
                part_label = _mtk_text(span)
                src = (img.get("src") if img else "") or ""
                if not part_label and img:
                    part_label = (img.get("alt") or "").replace("Image of ", "").strip()
                if part_label or src:
                    included_parts.append({"label": part_label, "image": src})
            continue

        ul = block.find("ul")
        if not ul or not label:
            continue
        items: List[Dict[str, Any]] = []
        for li in ul.find_all("li", recursive=False):
            ...
            if len(spans) < 2:
                continue
            name = _mtk_text(spans[0])
            value = _mtk_text(spans[1])
            if name:
                items.append({"name": name, "value": value, "canonicalUid": None})
        if items:
            specifications.append({"technology": label, "items": items})

    return specifications, note, included_parts


def _mtk_parse_wireless_tables(spec_root) -> List[Dict[str, Any]]:
    tables: List[Dict[str, Any]] = []
    if not spec_root:
        return tables
    for table in spec_root.select("div.wireless-table"):
        header = table.select_one(".wireless-header")
        headers = [_mtk_text(d) for d in header.find_all("div", recursive=False)] if header else []
        rows: List[List[str]] = []
        for row in table.select(":scope > div.grid.grid-cols-3"):
            if "wireless-header" in (row.get("class") or []):
                continue
            cells = [_mtk_text(d) for d in row.find_all("div", recursive=False)]
            if cells:
                rows.append(cells)
        band = headers[0] if headers else "Wireless"
        tables.append({"band": band, "headers": headers, "rows": rows})
    return tables


def _mtk_parse_ethernet_test(spec_root) -> Dict[str, Any]:
    if not spec_root:
        return {}
    table = spec_root.select_one("table.performance-table")
    if not table:
        return {}
    html_table = str(table)
    notes: List[str] = []
    for li in spec_root.select("ul.list-decimal li"):
        t = _mtk_text(li)
        if t:
            notes.append(t)
    return {"html": html_table, "notes": notes}


def _mtk_parse_documents(soup: BeautifulSoup, base_url: str) -> Tuple[List[Dict[str, str]], Optional[Dict[str, str]]]:
    root = _mtk_widget(soup, "widgets.product-documents")
    files: List[Dict[str, str]] = []
    brochure: Optional[Dict[str, str]] = None
    if not root:
    ...
    for a in root.find_all("a", href=True):
        ...
        if href.startswith("#") or "javascript:" in href:
            continue
        url = _mtk_abs(href, base_url)
        if url in seen:
            continue
        title = (a.get("title") or "").replace("Open ", "").strip() or _mtk_text(a)
        title = re.sub(r"\s+", " ", title).strip()
        if not title:
            continue
        # Skip empty / download-image buttons without real URLs to product files
        if "downloadFromURL" in (a.get("onclick") or ""):
            continue
        seen.add(url)
        entry = {"title": title, "url": url}
        if "brochure" in title.lower() or url.lower().endswith(".pdf") and brochure is None and "brochure" in (a.get("title") or "").lower():
            thumb = root.find("img", alt=re.compile(r"Brochure", re.I))
            brochure = {
                "title": "Brochure",
                "url": url,
                "thumbnail": _mtk_abs(thumb["src"], base_url) if thumb and thumb.get("src") else "",
            }
            files.append({"title": "Brochure", "url": url})
        elif title.lower() not in ("",):
            files.append(entry)

    # Explicit brochure card
    brochure_link = root.select_one('a[title*="brochure" i], a[href*="thumb" i]')
    for a in root.find_all("a", href=True):
        title_attr = (a.get("title") or "").lower()
        if "brochure" in title_attr:
            url = _mtk_abs(a["href"], base_url)
            thumb = a.find_parent("div")
            img = None
            if thumb:
                img = thumb.find("img")
            brochure = {
                "title": "Brochure",
                "url": url,
                "thumbnail": _mtk_abs(img["src"], base_url) if img and img.get("src") else "",
            }
            if url not in seen:
                files.insert(0, {"title": "Brochure", "url": url})
                seen.add(url)
            break
...
def _mtk_parse_downloads(soup: BeautifulSoup, base_url: str) -> Dict[str, Any]:
    root = _mtk_widget(soup, "widgets.product-download")
    if not root:
        return {"banner": "", "groups": []}
    banner_el = root.select_one(".bg-gray-light, .bg-gray-light.py-2")
    banner = _mtk_text(banner_el)
    groups: List[Dict[str, Any]] = []
    grid = root.select_one(".grid.lg\\:grid-cols-3, div.grid")
    columns = []
    if grid:
        columns = [c for c in grid.find_all("div", recursive=False) if c.find(class_=re.compile(r"uppercase"))]
    if not columns:
        # Fallback: each column headed by uppercase label
        for label_el in root.find_all(class_=re.compile(r"uppercase")):
            parent = label_el.find_parent("div")
            if parent and parent not in columns:
    ...
    for col in columns:
        ...
        label_el = col.find(class_=re.compile(r"uppercase"))
        group_name = _mtk_text(label_el)
        if not group_name or group_name.lower() in ("download",):
            continue
        items: List[Dict[str, str]] = []
        for a in col.find_all("a", href=True):
            href = a["href"]
            if "download.mikrotik.com" not in href and not href.endswith((".npk", ".zip", ".tar.gz")):
                if "sha256" in href:
                    continue
            ...
            if "sha256" in href:
                continue
            label = _mtk_text(a.find("span")) or _mtk_text(a)
            if not label or label.lower() in ("download checksum",):
                continue
            items.append({
                "title": label,
                "url": _mtk_abs(href, base_url),
        ...
        for it in items:
            ...
            uniq.append(it)
        if uniq:
            groups.append({"name": group_name, "items": uniq})

    return {"banner": banner, "groups": groups}


def _mtk_build_desc_blocks(
    soup: BeautifulSoup,
    base_url: str,
    ...
    if highlights:
        blocks.append({
            "heading": "Highlights",
            "text": " · ".join(h.get("label") or h.get("title") or "" for h in highlights),
            "media": [],
            "videos": [],
            "features": highlights,
            "align": "left",
    ...
    # Marketing headings / paragraphs / html images (first occurrence of each widget type sequence)
    marketing_bits: List[Dict[str, Any]] = []
    for el in soup.find_all(attrs={"wire:name": re.compile(r"^widgets\.(heading|paragraph|html)$")}):
        name = el.get("wire:name") or ""
        if name == "widgets.heading":
            text = _mtk_text(el)
            if text:
                marketing_bits.append({"type": "heading", "text": text, "color": el.get("style") or ""})
        elif name == "widgets.paragraph":
            text = _mtk_text(el)
            if text:
                marketing_bits.append({"type": "paragraph", "text": text})
        elif name == "widgets.html":
            img = el.find("img")
            if img and img.get("src"):
                marketing_bits.append({
                    "type": "image",
                    "url": _mtk_abs(img["src"], base_url),
                    "alt": img.get("alt") or "",
                })

    # Collapse adjacent heading+paragraph into sections; attach following image
    i = 0
    while i < len(marketing_bits):
        bit = marketing_bits[i]
        if bit["type"] == "heading":
            heading = bit["text"]
            text_parts: List[str] = []
            media: List[Dict[str, Any]] = []
            j = i + 1
            while j < len(marketing_bits) and marketing_bits[j]["type"] != "heading":
                nxt = marketing_bits[j]
                if nxt["type"] == "paragraph":
                    text_parts.append(nxt["text"])
                elif nxt["type"] == "image":
                    media.append({"url": nxt["url"], "alt": nxt.get("alt") or heading, "width": 0, "height": 0})
                j += 1
            blocks.append({
                "heading": heading,
                "text": "\n\n".join(text_parts),
        ...
        if bit["type"] == "paragraph":
            ...
            blocks.append({
                "heading": "",
                "text": "",
                "media": [{"url": bit["url"], "alt": bit.get("alt") or "", "width": 0, "height": 0}],
                "videos": [],
                "features": [],
                "align": "center",
                "tab": "overview",
...
def try_parse_mikrotik_html(content: str, base_url: str) -> Optional[Dict[str, Any]]:
    """Parse mikrotik.com Livewire product pages into the unified product dict."""
    if not _is_mikrotik_html(content):
        return None

    soup = BeautifulSoup(content, "html.parser")
    details = _mtk_parse_product_details(soup, base_url)
    title = details.get("title") or ""
    if not title:
        og = soup.find("meta", property="og:title")
        if og and og.get("content"):
            title = _clean_page_title(og["content"])
        else:
            title_tag = soup.find("title")
            title = _clean_page_title(_mtk_text(title_tag)) if title_tag else ""

    if not title and not _mtk_widget(soup, "widgets.product-specification"):
        return None

    highlights = _mtk_parse_feature_list(soup)
    feature_cards = _mtk_parse_feature_grid(soup)
    videos = _mtk_parse_videos(soup)
    gallery = _mtk_parse_gallery(soup, base_url, title)

    images: List[Dict[str, str]] = list(details.get("images") or [])
    seen_img: Set[str] = {img["url"] for img in images}
    for img in gallery:
        if img["url"] not in seen_img:
            seen_img.add(img["url"])
            images.append(img)

    spec_root = _mtk_widget(soup, "widgets.product-specification")
    specifications, note, included_parts = _mtk_parse_spec_groups(spec_root)
    wireless_tables = _mtk_parse_wireless_tables(spec_root)
    ethernet_test = _mtk_parse_ethernet_test(spec_root)

    mpn = details.get("mpn") or ""
    price = ""
    for group in specifications:
        if str(group.get("technology") or "").lower() != "specifications":
            continue
        for item in group.get("items") or []:
            name = str(item.get("name") or "").lower()
            if name == "product code" and not mpn:
                mpn = str(item.get("value") or "").strip()
            if name == "suggested price" and not price:
                price = _mtk_parse_price(str(item.get("value") or ""))

    files, brochure = _mtk_parse_documents(soup, base_url)
    downloads = _mtk_parse_downloads(soup, base_url)
    for group in downloads.get("groups") or []:
        for item in group.get("items") or []:
            if item.get("url") and not any(f.get("url") == item["url"] for f in files):
                files.append({"title": f"{group.get('name')}: {item.get('title')}", "url": item["url"]})

    hero_background = _mtk_extract_hero_background(soup, content)
    desc_blocks = _mtk_build_desc_blocks(
        soup, base_url, highlights, feature_cards, videos, note
    )

    # Breadcrumbs
    breadcrumbs: List[Dict[str, str]] = []
    crumb_nav = soup.select_one("nav") or soup.find(string=re.compile(r"Home\s*/\s*Hardware"))
    for a in soup.select('a[href*="/hardware"], a[href*="/product"]'):
        pass  # keep simple
    crumb_text = soup.find(string=re.compile(r"Home\s*/"))
    if crumb_text:
        parts = [p.strip() for p in re.split(r"/", str(crumb_text)) if p.strip()]
        breadcrumbs = [{"title": p, "slug": ""} for p in parts if p.lower() != "home"]

    cat_leaf = ""
    if breadcrumbs:
        cat_leaf = breadcrumbs[-1]["title"] if breadcrumbs else "Wireless systems"
    else:
        cat_leaf = "Wireless systems"

    extraction: Dict[str, Any] = {
        "id": mpn or title,
        "uid": mpn or re.sub(r"[^a-z0-9]+", "-", title.lower()).strip("-"),
        "title": title,
        "title_extended": title,
        ...
        "cat_leaf": cat_leaf,
        "breadcrumbs": breadcrumbs,
        "output_format": "mikrotik",
        "included_parts": [
            {"label": p.get("label", ""), "image": _mtk_abs(p.get("image", ""), base_url)}
            for p in included_parts
            if p.get("label") or p.get("image")
        ],
        "note": note,
        "wireless_tables": wireless_tables,
    ...
    product = normalize_from_generic(extraction, base_url)
    # Preserve MikroTik-only fields that normalize_from_generic does not map
    for key in (
        "included_parts", "note", "wireless_tables", "ethernet_test",
        "downloads", "brochure", "hero_background", "highlights",
    ):
        if key in extraction:
            product[key] = extraction[key]
    product["output_format"] = "mikrotik"
    return product


# ---------------------------------------------------------------------------
# Inrico product page parser (jext CMS / inricosolutions.com)
# ---------------------------------------------------------------------------

INRICO_BASE_URL = "https://www.inricosolutions.com"

NON_BRAND_FOLDER_NAMES = frozenset({
    "input", "output", "converter", "menu", "src", "dist", "tmp", "temp",
})


def _is_inrico_html(content: str) -> bool:
    lower = content.lower()
    if "inricosolutions.com" in lower:
        return True
    if 'id="products_top"' in lower and 'id="products_spe"' in lower:
        return True
    return (
        "website-language" in lower
        and "/file/upload/" in lower
        and 'id="products_spe"' in lower
    )


def _inrico_origin(content: str, base_url: str) -> str:
    origin = _url_origin(base_url)
    if origin and "inrico" in origin.lower():
        return origin
    match = re.search(
        r"https?://(?:www\.)?inricosolutions\.com",
        content,
        re.IGNORECASE,
    )
    if match:
        return INRICO_BASE_URL
    return INRICO_BASE_URL


def _inrico_text(el: Any, sep: str = " ") -> str:
    if el is None:
        return ""
    return re.sub(r"\s+", " ", el.get_text(sep, strip=True)).strip()


def _inrico_spec_value(el: Any) -> str:
    if el is None:
        return ""
    text = el.get_text("\n", strip=True)
...
def _inrico_img_src(img: Any) -> str:
    if img is None:
        return ""
    return str(
        img.get("src")
...
def _inrico_first_section(soup: BeautifulSoup, section_id: str):
    return soup.find(id=section_id)


def _inrico_parse_hero(section, origin: str) -> Tuple[str, str, List[str], str]:
    title = _inrico_text(section.select_one(".title") if section else None)
    subtitle = _inrico_text(section.select_one(".brief.upper") if section else None)
    chips: List[str] = []
    if section:
        for item in section.select(".cont .item"):
            label = _inrico_text(item.select_one(".t"))
    ...
    if section:
        ...
        if match:
            background = make_absolute_url(match.group(1), origin)
    return title, subtitle, chips, background


def _inrico_parse_gallery(section, origin: str, alt: str) -> List[Dict[str, str]]:
    images: List[Dict[str, str]] = []
    seen: Set[str] = set()
    if section is None:
        return images
    for slide in section.select(".swiper-slide"):
        ...
        if "swiper-slide-duplicate" in classes:
            continue
        src = _inrico_img_src(slide.find("img"))
        url = make_absolute_url(src, origin)
        if not url or url in seen or _is_skip_image(src):
            continue
        seen.add(url)
...
def _inrico_parse_highlights(section) -> List[Dict[str, str]]:
    features: List[Dict[str, str]] = []
    if section is None:
        return features
    for item in section.select(".cont .item"):
        heading = _inrico_text(item.select_one(".tit"))
        brief = _inrico_text(item.select_one(".brief"))
        if heading or brief:
            features.append({"title": heading, "body": brief})
    return features


def _inrico_parse_specs(section) -> List[Dict[str, Any]]:
    groups: List[Dict[str, Any]] = []
    seen_tech: Set[str] = set()
    if section is None:
        return groups
    for block in section.select(".list"):
        tech = _inrico_text(block.select_one(".tit"))
        if not tech or tech.lower() in seen_tech:
            continue
        seen_tech.add(tech.lower())
        items: List[Dict[str, Any]] = []
        for row in block.select(".li"):
            name = _inrico_text(row.select_one(".t"))
            value = _inrico_spec_value(row.select_one(".bri"))
            if name and value:
                items.append({"name": name, "value": value, "canonicalUid": None})
        if items:
            groups.append({"technology": tech, "items": items})
    return groups


def _inrico_parse_intro(section, origin: str) -> List[Dict[str, Any]]:
    blocks: List[Dict[str, Any]] = []
    if section is None:
        return blocks
    seen: Set[str] = set()
    for item in section.select(".li"):
        heading = _inrico_text(item.select_one(".title"))
        text = _inrico_text(item.select_one(".brief"))
        if not heading or heading.lower() in seen:
            continue
        seen.add(heading.lower())
        media: List[Dict[str, Any]] = []
        src = _inrico_img_src(item.select_one("img"))
        url = make_absolute_url(src, origin) if src else ""
        if url and not _is_skip_image(src):
...
def _inrico_parse_faq(soup: BeautifulSoup) -> List[Dict[str, Any]]:
    blocks: List[Dict[str, Any]] = []
    seen: Set[str] = set()
    acc = _inrico_first_section(soup, "products_acc")
    if acc is None:
        return blocks
    for box in acc.select(".box"):
        box_title = _inrico_text(box.select_one(".title")).lower()
        if "frequently asked" not in box_title:
            continue
        for item in box.select(".item"):
            heading = _inrico_text(item.select_one(".tit"))
            text = _inrico_text(item.select_one(".con_in"))
            key = heading.lower()
...
def _inrico_parse_accessories(
    ...
    for box in acc.select(".box"):
        box_title = _inrico_text(box.select_one(".title")).lower()
        if "standard accessories" in box_title:
            for slide in box.select(".swiper-slide"):
                if "swiper-slide-duplicate" in (slide.get("class") or []):
                    continue
                label = (
                    _inrico_text(slide.select_one(".brief"))
                    or _inrico_text(slide.select_one(".tit"))
                )
                src = _inrico_img_src(slide.find("img"))
                url = make_absolute_url(src, origin) if src else ""
                key = (label or url).lower()
                if not key or key in seen_std:
                    continue
                seen_std.add(key)
                included.append({"label": label, "image": url})
        elif "optional accessories" in box_title:
            for slide in box.select(".swiper-slide"):
                if "swiper-slide-duplicate" in (slide.get("class") or []):
                    continue
                name = (
                ...
                if href and href not in ("javascript:;", "#"):
                    product_url = make_absolute_url(href, origin)
                optional.append({
                    "name": name,
                    "title": name,
                    "url": product_url,
...
def _inrico_parse_media_links(soup: BeautifulSoup, origin: str) -> Tuple[List[str], List[Dict[str, str]]]:
    videos: List[str] = []
    files: List[Dict[str, str]] = []
    video_el = soup.select_one("[ly-video]")
    if video_el is not None:
        src = str(video_el.get("src") or video_el.get("data-src") or "").strip()
        if src:
            videos.append(make_absolute_url(src, origin))
    brochure = soup.select_one("#download_btn")
    if brochure is not None:
        href = str(brochure.get("data-href") or brochure.get("href") or "").strip()
        if href and href not in ("javascript:;", "#"):
            files.append({"title": "Brochure", "url": make_absolute_url(href, origin)})
    return videos, files


def try_parse_inrico_html(content: str, base_url: str) -> Optional[Dict[str, Any]]:
    """Parse Inrico jext CMS product pages into the unified product dict."""
    if not _is_inrico_html(content):
        return None

    origin = _inrico_origin(content, base_url)
    soup = BeautifulSoup(content, "html.parser")
    top = _inrico_first_section(soup, "products_top")
    high = _inrico_first_section(soup, "products_high")
    spe = _inrico_first_section(soup, "products_spe")
    intro = _inrico_first_section(soup, "pro_intro")

    title, subtitle, chips, hero_bg = _inrico_parse_hero(top, origin)
    if not title:
        title_tag = soup.find("title")
        title = _clean_page_title(_inrico_text(title_tag)) if title_tag else ""
    if not title:
        return None

    meta_desc = soup.find("meta", attrs={"name": "description"})
    short_description = meta_desc.get("content", "").strip() if meta_desc else ""
    faq_blocks = _inrico_parse_faq(soup)
    overview = faq_blocks[0].get("text", "") if faq_blocks else ""

    gallery = _inrico_parse_gallery(high, origin, title)
    images: List[Dict[str, str]] = []
    seen_img: Set[str] = set()
    if hero_bg:
        images.append({"url": hero_bg, "alt": title})
        seen_img.add(hero_bg)
    for img in gallery:
        if img["url"] not in seen_img:
            seen_img.add(img["url"])
            images.append(img)

    highlight_features = _inrico_parse_highlights(high)
    specifications = _inrico_parse_specs(spe)
    videos, files = _inrico_parse_media_links(soup, origin)
    included_parts, optional = _inrico_parse_accessories(soup, origin)
    intro_blocks = _inrico_parse_intro(intro, origin)
    for block in intro_blocks:
        for media in block.get("media") or []:
            url = str(media.get("url") or "")
            if url and url not in seen_img:
                seen_img.add(url)
                images.append({"url": url, "alt": block.get("heading") or title})
    ...
    if chips:
        desc_blocks.append({
            ...
            "media": [],
            "align": "left",
            "features": [{"title": chip, "body": ""} for chip in chips],
        })
    if highlight_features:
        desc_blocks.append({
            "heading": "Highlights",
            "text": "",
            "media": [{"url": img["url"], "width": 0, "height": 0} for img in gallery[:4]],
            "align": "left",
            "features": highlight_features,
        })
    desc_blocks.extend(intro_blocks)
    if included_parts:
        desc_blocks.append({
            "heading": "Standard Accessories",
            "text": ", ".join(p["label"] for p in included_parts if p.get("label")),
            "media": [
                {"url": p["image"], "width": 0, "height": 0, "alt": p.get("label") or ""}
                for p in included_parts
                if p.get("image")
            ],
            "align": "left",
    ...
    if optional:
        desc_blocks.append({
            ...
            "text": ", ".join(p["name"] for p in optional if p.get("name")),
            "media": [
                {"url": p["image"], "width": 0, "height": 0, "alt": p.get("name") or ""}
                for p in optional
                if p.get("image")
            ],
            "align": "left",
    ...
    extraction: Dict[str, Any] = {
        ...
        "uid": mpn,
        "title": title,
        "title_extended": f"{title} {subtitle}".strip() if subtitle else title,
        "short_description": short_description or overview,
        "brand": "Inrico",
        "mpn": mpn,
        "images": images,
        "files": files,
        ...
        "desc_blocks": desc_blocks,
        "videos": videos,
        "cat_leaf": "Two-way Radios",
        "cat_path_titles": ["Security Systems", "Two-way Radios"],
        "brand_path_titles": ["Inrico"],
        "category_root": "Two-way Radios",
        "bought_together": optional,
    ...
    product = normalize_from_generic(extraction, origin)
    product["included_parts"] = included_parts
    product["highlights"] = chips
    product["hero_background"] = hero_bg
    product["bought_together"] = optional
    product["brand_path_titles"] = ["Inrico"]
    product["cat_path_titles"] = ["Security Systems", "Two-way Radios"]
    product["cat_leaf"] = "Two-way Radios"
    product["videos"] = videos
    return product


def _csv_cell(row: Dict[str, str], *names: str) -> str:
    for name in names:
        if name in row and row[name] is not None:
            return str(row[name]).strip()
        for key, val in row.items():
            if key and key.strip().lower() == name.strip().lower():
                return str(val or "").strip()
    return ""


def _split_csv_list(value: str, seps: Tuple[str, ...] = (", ", ",", "|")) -> List[str]:
    text = (value or "").strip()
    if not text:
        return []
    for sep in seps:
...
def _parse_csv_rows(content: str) -> List[Dict[str, str]]:
    ...
    for row in reader:
        if not isinstance(row, dict):
            continue
        cleaned = {str(k).strip() if k else "": ("" if v is None else str(v)) for k, v in row.items()}
        if any(v.strip() for v in cleaned.values()):
            rows.append(cleaned)
    return rows


def _group_csv_product_rows(rows: List[Dict[str, str]]) -> List[List[Dict[str, str]]]:
    """Group a WooCommerce CSV into parent (+ variation) row sets."""
    groups: List[List[Dict[str, str]]] = []
    current: List[Dict[str, str]] = []
    for row in rows:
        typ = _csv_cell(row, "Type").lower()
        if typ == "variation":
            if current:
                current.append(row)
            else:
...
def _csv_attribute_values(row: Dict[str, str], wanted: str) -> List[str]:
    wanted_l = wanted.lower()
    for i in range(1, 12):
        name = _csv_cell(row, f"Attribute {i} name")
        if name.lower() != wanted_l:
            continue
        return _split_csv_list(_csv_cell(row, f"Attribute {i} value(s)"), (" | ", "|", ", "))
    return []


def normalize_from_csv_group(rows: List[Dict[str, str]]) -> Dict[str, Any]:
    """Convert a WooCommerce parent (+ variations) CSV group into the unified schema."""
    if not rows:
        raise ValueError("Empty CSV product group")
    parent = rows[0]
    variation_rows = [
        row for row in rows[1:]
        if _csv_cell(row, "Type").lower() == "variation"
    ]

    title = _csv_cell(parent, "Name", "productTitle", "title")
    short_desc = _csv_cell(parent, "Short description", "short_description")
    if short_desc.lower().startswith("ean:"):
        rest = short_desc.split("\n", 1)
        short_desc = rest[1].strip() if len(rest) > 1 else ""

    images = [
        {"url": url, "alt": title}
        for url in _split_csv_list(_csv_cell(parent, "Images"), (", ", ","))
        if url.startswith(("http://", "https://"))
    ]

    available_plugs = _csv_attribute_values(parent, "Plug")
    available_colors = _csv_attribute_values(parent, "Color")
    color_variants: List[Dict[str, Any]] = []
    for row in variation_rows:
        color = (_csv_attribute_values(row, "Color") or [""])[0]
        plug = (_csv_attribute_values(row, "Plug") or [""])[0]
        if plug and plug not in available_plugs:
            available_plugs.append(plug)
        var_images = [
            {"url": url, "alt": color or title, "color": color}
            for url in _split_csv_list(_csv_cell(row, "Images"), (", ", ","))
            if url.startswith(("http://", "https://"))
        ]
        if color:
            if color not in available_colors:
            ...
            color_variants.append({
                "color": color,
                "sku": _strip_sku_prefix(_csv_cell(row, "SKU")),
                "price": _csv_cell(row, "Regular price"),
                "stock": _csv_cell(row, "Stock"),
                "images": var_images,
            })
        elif plug and plug not in available_plugs:
            available_plugs.append(plug)

    try:
        stock = int(float(_csv_cell(parent, "Stock") or 0))
    except (TypeError, ValueError):
        stock = 0

    brand_paths = _split_csv_list(_csv_cell(parent, "Meta: _brand_paths"))
    category_paths = _split_csv_list(_csv_cell(parent, "Meta: _category_paths"))
    matching_rules = _split_csv_list(_csv_cell(parent, "Meta: _matching_rules"))
    video_urls = _split_csv_list(_csv_cell(parent, "Meta: _product_videos"))
    model_url = _csv_cell(parent, "Meta: _3d_model_url")
    ean = (
        _csv_cell(parent, "GTIN, UPC, EAN, or ISBN")
        or _csv_cell(parent, "Meta: _ean")
    )
    mpn = _csv_cell(parent, "Meta: _mpn") or _csv_cell(parent, "SKU")
    brand = _csv_cell(parent, "Brands") or _csv_cell(parent, "Meta: _brand")
    uid = _csv_cell(parent, "Meta: _getic_uid") or _strip_sku_prefix(_csv_cell(parent, "SKU"))

    categories_col = _csv_cell(parent, "Categories")
    cat_paths_from_col = _split_csv_list(categories_col)
    if not brand_paths and not category_paths and cat_paths_from_col:
        category_paths = [p for p in cat_paths_from_col if " > " in p or p]
        brand_paths = []

    product: Dict[str, Any] = {
        "id": _csv_cell(parent, "Meta: _getic_product_id") or _csv_cell(parent, "ID") or uid,
        "uid": uid,
        "title": title,
        "title_extended": "",
        ...
        "mpn": mpn,
        "ean": ean,
        "warranty": _csv_cell(parent, "Meta: _warranty_months"),
        "country_of_origin": _csv_cell(parent, "Meta: _country_of_origin"),
        "regular_price": _csv_cell(parent, "Regular price"),
        "stock": stock,
        "available_plugs": available_plugs,
        "images": images,
        "files": [],
        "specifications": [],
        "desc_blocks": (
            [{"heading": "Overview", "text": short_desc, "media": [], "align": "left"}]
            if short_desc else []
        ),
        "brand_path_titles": [],
        "category_root": "",
        ...
        "bought_together": [],
        "certifications": [],
        "has_3d_model": bool(model_url),
        "currency": "USD",
        "old_price": _csv_cell(parent, "Sale price") or None,
        "condition_options": ["new"],
        "default_plug": available_plugs[0] if available_plugs else "",
        "image_gallery": [],
        "thumbnails": [],
        "videos": video_urls,
        "model_3d": {"url": model_url} if model_url else {},
        "color_variants": color_variants,
        "available_colors": available_colors,
        "default_color": available_colors[0] if available_colors else "",
        "output_format": _csv_cell(parent, "Meta: _output_format") or "",
        "mainCategory": _csv_cell(parent, "Meta: _main_category"),
...
def load_all_products(input_path: Path, force_json: bool = False) -> List[Tuple[Dict[str, Any], str]]:
    """Parse an input file into one or more unified product records."""
    content = read_file_text(input_path)
    kind = "json" if force_json else detect_input_format(content, input_path)

    if kind == "json":
        try:
            parsed = json.loads(content)
        except json.JSONDecodeError as e:
            raise ValueError(f"Invalid JSON in {input_path.name}: {e}") from e
        records = _json_product_records(parsed)
        if not records:
            raise ValueError(f"JSON file has no product objects: {input_path.name}")
        return [(normalize_from_json(record), "raw_json") for record in records]

    if kind == "csv":
        rows = _parse_csv_rows(content)
        groups = _group_csv_product_rows(rows)
        if not groups:
            raise ValueError(f"CSV file has no product rows: {input_path.name}")
        return [(normalize_from_csv_group(group), "woocommerce_csv") for group in groups]
    ...
    parsers: List[Tuple[str, Any]] = [
        ...
        ("mikrotik", try_parse_mikrotik_html),
        ("inrico", try_parse_inrico_html),
...
def load_and_normalize(input_path: Path, force_json: bool = False) -> Tuple[Dict[str, Any], str]:
    products = load_all_products(input_path, force_json)
    return products[0]
...
PRODUCT_EXTENSIONS = (".html", ".htm", ".json", ".csv")
...
def _output_stem(path: Path) -> str:
    ...
    for ext in (".csv", ".json", ".html", ".htm", ".xlsx"):
        if lower.endswith(ext):
...
def product_image_urls(
    ...
    """Return (primary_url, additional_urls) with duplicates removed."""
    urls: List[str] = []
    seen: Set[str] = set()
...
def build_images(product: Dict[str, Any], images: Optional[List[Any]] = None) -> str:
    ...
    primary, additional = product_image_urls(product, images)
    if not primary:
        return ""
    return ", ".join([primary, *additional])
...
def _mikrotik_page_display() -> Dict[str, Dict[str, bool]]:
    display = _default_page_display()
    for key in (
        "tabs", "tabDescription", "tabSpecs", "tabDocuments", "tabOverview",
        "tabVideos", "gallery", "floatingCta", "shortDescription", "keySpecs",
    ):
        display[key] = {"enabled": True, "inherit": True}
    display["breadcrumb"] = {"enabled": True, "inherit": True}
    display["modelViewer"] = {"enabled": False, "inherit": True}
    display["tab3D"] = {"enabled": False, "inherit": True}
    return display
...
def _is_mikrotik_output(product: Dict[str, Any]) -> bool:
    return str(product.get("output_format") or "") == "mikrotik"


def _is_rich_layout_output(product: Dict[str, Any]) -> bool:
    return _is_unifi_output(product) or _is_mikrotik_output(product)
...
def build_product_json(product: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload: Dict[str, Any] = {
        ...
        "source_specifications": product.get("source_specifications") or [],
        "specifications": _json_specifications(product.get("specifications", [])),
        "canonical_specs": product.get("canonical_specs") or {},
        "unmapped_specs": product.get("unmapped_specs") or [],
        "ambiguous_specs": product.get("ambiguous_specs") or [],
        "product_domain": product.get("product_domain"),
        "documents": [
            {"title": f.get("title", ""), "url": f.get("url", "")}
        ...
        "page_display": (
            _unifi_page_display()
            if _is_unifi_output(product)
            else _mikrotik_page_display()
            if _is_mikrotik_output(product)
            else _default_page_display()
        ),
        "product_cta": {
            "enabled": False,
    ...
    if _is_mikrotik_output(product):
        payload["included_parts"] = product.get("included_parts") or []
        payload["note"] = product.get("note") or ""
        payload["wireless_tables"] = product.get("wireless_tables") or []
        payload["ethernet_test"] = product.get("ethernet_test") or {}
        payload["downloads"] = product.get("downloads") or {"banner": "", "groups": []}
        payload["brochure"] = product.get("brochure") or {}
        payload["hero_background"] = product.get("hero_background") or ""
        payload["highlights"] = product.get("highlights") or []
        payload["output_format"] = "mikrotik"
...
def build_csv_rows(product: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    def apply_meta(row: Dict[str, str]) -> None:
        ...
        canonical_specs = product.get("canonical_specs") or {}
        if canonical_specs:
            row["Meta: canonical_specs"] = json.dumps(canonical_specs, ensure_ascii=False)
            for spec_id, record in canonical_specs.items():
                if isinstance(record, dict):
                    col = "Meta: spec_" + str(spec_id).replace(".", "_")
                    row[col] = str(record.get("display") or record.get("value") or "")
...
def _csv_headers_for_rows(rows: List[Dict[str, str]]) -> List[str]:
    headers = list(load_csv_headers())
    extra: List[str] = []
    seen = set(headers)
    for row in rows:
        for key in row:
            if key.startswith("Meta: canonical_specs") or key.startswith("Meta: spec_"):
                if key not in seen:
                    extra.append(key)
                    seen.add(key)
    return headers + sorted(extra)


def write_csv(output_path: Path, rows: List[Dict[str, str]]) -> None:
    headers = _csv_headers_for_rows(rows)
    cleaned = [
...
def _resolve_output_format(output_format: str, parser_name: str, product: Dict[str, Any]) -> str:
    if output_format != "auto":
        return output_format
    incoming = str(product.get("output_format") or "")
    if parser_name == "unifi_store" or incoming == "unifi":
        return "unifi"
    if parser_name == "mikrotik" or incoming == "mikrotik":
        return "mikrotik"
    return "default"


def _write_product_pair(product: Dict[str, Any], output_path: Path) -> Path:
...
def process_single(
    ...
    *,
    ...
    if len(loaded) > 1:
        print(f"  Products in file: {len(loaded)}")

    json_paths: List[Path] = []
    for index, (product, parser_name) in enumerate(loaded, start=1):
        product = sanitize_product_text(product)
        if len(loaded) > 1:
            print(f"  [{index}/{len(loaded)}] {product.get('title') or product.get('mpn') or 'product'}")
        print(f"  Parser: {parser_name}")

        resolved_format = _resolve_output_format(output_format, parser_name, product)
        ...
        folder_brand = input_path.parent.name
        if folder_brand.lower() in NON_BRAND_FOLDER_NAMES:
            folder_brand = ""
        bname = (
            brand_name
        ...
        existing_brand_paths = list(product.get("brand_paths") or [])
        existing_category_paths = list(product.get("category_paths") or [])
        ...
        if menu is not None:
            ...
            product["brand_paths"] = brand_paths or existing_brand_paths
            product["category_paths"] = category_paths or existing_category_paths
            product["menu_category_paths"] = sorted(
                set(product["brand_paths"]) | set(product["category_paths"]),
                key=lambda p: (len(p.split(" > ")), p),
            )
            if product["brand_paths"]:
                print(
                    f"  Brand path: {max(product['brand_paths'], key=lambda p: len(p.split(' > ')))}"
                )
            if product["category_paths"]:
                print(
                    f"  Category path: {max(product['category_paths'], key=lambda p: len(p.split(' > ')))}"
                )
            for w in warns:
                print(f"  Warning: {w}")
        ...
        elif existing_brand_paths or existing_category_paths:
            product["menu_category_paths"] = sorted(
                set(existing_brand_paths) | set(existing_category_paths),
                key=lambda p: (len(p.split(" > ")), p),
            )
        ...
        apply_spec_normalization(product)
        canon_count = len(product.get("canonical_specs") or {})
        unmapped_count = len(product.get("unmapped_specs") or [])
        ambiguous_count = len(product.get("ambiguous_specs") or [])
        if canon_count:
            print(f"  Canonical specs: {canon_count}")
        if unmapped_count or ambiguous_count:
            print(f"  Spec diagnostics: {unmapped_count} unmapped, {ambiguous_count} ambiguous")

        product["slug"] = product.get("slug") or resolve_product_slug(product)
        ...
        if len(loaded) == 1:
            ...
            stem = product["slug"] or f"{_output_stem(output_path)}-{index}"
            csv_path = output_path.parent / f"{stem}.csv"

        json_paths.append(_write_product_pair(product, csv_path))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Convert product pages (HTML/JSON/CSV) to WooCommerce import CSV + JSON",
    )
    parser.add_argument(
        "input",
        help="Input file, directory, or glob (HTML, JSON, or WooCommerce CSV)",
    )
    parser.add_argument(
        "-o",
        "--output",
        required=True,
        help="Output CSV path, or a directory to write <name>.csv and <name>.json into",
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="Force JSON parsing (default: auto-detect HTML, JSON, or CSV)",
    )
    ...
    parser.add_argument(
        "--format",
        choices=["default", "unifi", "mikrotik", "auto"],
        default="auto",
        help=(
            "Output format: default (WooCommerce), unifi (rich UniFi layout), "
            "mikrotik (rich MikroTik layout), "
            "auto (unifi/mikrotik when detected; else default)"
        ),
    )
    parser.add_argument(
        "--brand",
