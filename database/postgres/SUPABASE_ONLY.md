...
| `database/postgres/10-site-settings-published-version.sql` | `SiteSettings.publishedVersion` (save vs publish) |
...
1. Run SQL migrations 01 → 10 in Supabase SQL Editor (or rely on automatic deploy — see below).
...
### Automatic schema updates (Vercel + GitHub)



Production Vercel builds run `npm run db:migrate:deploy` after `prisma generate` (see `scripts/deploy/production-build.mjs`). For Supabase this applies idempotent SQL patches (e.g. `publishedVersion`) — no manual SQL Editor step required when auto-deploy is enabled.

Optional GitHub Action (`.github/workflows/db-migrate.yml`) runs the same script on push to `main` when `prisma/` or `database/postgres/` changes. Add repository secrets:

- `DATABASE_URL` — production Postgres pooler URL
- `PRISMA_SCHEMA` — `postgresql`

Skip auto-migrate: set `SKIP_DB_MIGRATE=1` on the build environment. Preview deployments skip DB migrate unless `RUN_DB_MIGRATE_ON_PREVIEW=1`.
